...
{
	m_firstkill			= true;								//Sistema de Primera Sangre
...
inline void Battleground::_ProcessJoin(uint32 diff)
{
	...
	if(GetArenaType() == ARENA_TYPE_5v5 && GetStartDelayTime() > m_StartDelayTimes[BG_STARTING_EVENT_THIRD] && (m_PlayersCount[0] + m_PlayersCount[1]) == 2)
		SetStartDelayTime(m_StartDelayTimes[BG_STARTING_EVENT_THIRD]);
        ...
        if(isArena())
        {
            ...
            for(BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
                if(Player* plr = sObjectMgr->GetPlayer(itr->first))
                {
					...
					plr->RemoveAurasDueToSpell(SPELL_ARENA_PREPARATION2);
                    plr->ResetAllPowers();
					if(plr->HasAura(2479))
						plr->RemoveAurasDueToSpell(2479);
					DespawnCrystals();
...
void Battleground::EndBattleground(uint32 winner)
{
    ...
    for(BattlegroundPlayerMap::iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
    {
		...
		//Custom spectator, scoreboard al acabar la arena
		Map::PlayerList const &PlList = m_Map->GetPlayers();
		if (!PlList.isEmpty())
			for (Map::PlayerList::const_iterator i = PlList.begin(); i != PlList.end(); ++i)
				if (Player* _pPlayer = i->getSource())
					if (_pPlayer->isSpectator())
					{
						sBattlegroundMgr->BuildPvpLogDataPacket(&data, this);
						_pPlayer->GetSession()->SendPacket(&data);

						BlockMovement(_pPlayer);
...
void Battleground::RemovePlayerAtLeave(const uint64& guid, bool Transport, bool SendPacket)
{
    ...
    if(participant) // if the player was a match participant, remove auras, calc rating, update queue
    {
        ...
        if(pPlayer)
        {
            ...
            if(!team) team = pPlayer->GetBGTeam();
...
void Battleground::AddPlayer(Player* plr)
{
    ...
    if(isArena())
    {
        ...
        if(GetStatus() == STATUS_WAIT_JOIN)                 // not started yet
        {
			...
			plr->CastSpell(plr, SPELL_ARENA_PREPARATION2, true);
            plr->ResetAllPowers();
			if(plr->HasAura(2479) && plr->HasAura(SPELL_ARENA_PREPARATION))
				plr->RemoveAurasDueToSpell(2479);
        }
...
void Battleground::HandleKillPlayer(Player* victim, Player* killer)
{
    ...
    if(!isArena())
    {
		if(m_firstkill) //Sistema de Primera Sangre
		{
			//Creamos los iconos por faccion, asesino
			char* icono_killer;
			if(killer->GetBGTeam() == ALLIANCE)
				icono_killer = "|TInterface\\PVPFrame\\PVP-Currency-Alliance:18:18|t";
			else
				icono_killer = "|TInterface\\PVPFrame\\PVP-Currency-Horde:18:18|t";

			//Creamos los iconos por faccion, muerto
			char* icono_victim;
			if(victim->GetBGTeam() == ALLIANCE)
				icono_victim = "|TInterface\\PVPFrame\\PVP-Currency-Alliance:18:18|t";
			else
				icono_victim = "|TInterface\\PVPFrame\\PVP-Currency-Horde:18:18|t";
				
			//Creamos los colores custom
			//Colores killer
			char* color_killer;
			if(killer->getClass() == CLASS_WARRIOR)
				color_killer = "|CFFC79C6E";
			else if (killer->getClass() == CLASS_DEATH_KNIGHT)
				color_killer = "|CFFC41F3B";
			else if (killer->getClass() == CLASS_DRUID)
				color_killer = "|CFFFF7D0A";
			else if (killer->getClass() == CLASS_HUNTER)
				color_killer = "|CFFABD473";
			else if (killer->getClass() == CLASS_MAGE)
				color_killer = "|CFF69CCF0";
			else if (killer->getClass() == CLASS_PALADIN)
				color_killer = "|CFFF58CBA";
			else if (killer->getClass() == CLASS_PRIEST)
				color_killer = "|CFFFFFFFF";
			else if (killer->getClass() == CLASS_ROGUE)
				color_killer = "|CFFFFF569";
			else if (killer->getClass() == CLASS_SHAMAN)
				color_killer = "|CFF0070DE";
			else if (killer->getClass() == CLASS_WARLOCK)
				color_killer = "|CFF9482C9";
				
			//Colores victim
			char* color_victim;
			if(victim->getClass() == CLASS_WARRIOR)
				color_victim = "|CFFC79C6E";
			else if (victim->getClass() == CLASS_DEATH_KNIGHT)
				color_victim = "|CFFC41F3B";
			else if (victim->getClass() == CLASS_DRUID)
				color_victim = "|CFFFF7D0A";
			else if (victim->getClass() == CLASS_HUNTER)
				color_victim = "|CFFABD473";
			else if (victim->getClass() == CLASS_MAGE)
				color_victim = "|CFF69CCF0";
			else if (victim->getClass() == CLASS_PALADIN)
				color_victim = "|CFFF58CBA";
			else if (victim->getClass() == CLASS_PRIEST)
				color_victim = "|CFFFFFFFF";
			else if (victim->getClass() == CLASS_ROGUE)
				color_victim = "|CFFFFF569";
			else if (victim->getClass() == CLASS_SHAMAN)
				color_victim = "|CFF0070DE";
			else if (victim->getClass() == CLASS_WARLOCK)
				color_victim = "|CFF9482C9";

			SendWarningToAll(LANG_PRIMERA_SANGRE, icono_killer, color_killer, killer->GetName(), icono_victim, color_victim, victim->GetName()); 
			QueryResult inicio = CharacterDatabase.PQuery("REPLACE INTO racha(guid, muertes) VALUES ('%u', 0)", killer->GetGUID());
			killer->ModifyHonorPoints(750);
			char* msg = "|cff87CEFAHas obtenido|r |CFFFFD700750|r |cff87CEFApuntos de Honor EXTRA por conseguir la Primera Sangre y una Poción de Primera Sangre.|r";
			WorldPacket data(SMSG_NOTIFICATION, 255);
			data << msg;
			killer->ToPlayer()->GetSession()->SendPacket(&data);
			if (!killer->HasItemCount(37661, 1))
				killer->AddItem(37661, 1);
			//ChatHandler(killer).PSendSysMessage("Has obtenido |CFF87CEFA750|r puntos de Honor EXTRA por conseguir la Primera Sangre.");
			m_firstkill=false;
		}
