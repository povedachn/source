...
bool ChallengeMgr::MakeChallengeOffer(Player *challenger, Player *challengeTo, ArenaChallengeType type)
{
    ...
    {
        ChatHandler(challenger).PSendSysMessage("Alguien debe tomar una decisión.");
        return false;
    }

    if (challengeTo->challengeData->challengeTo || challengeTo->challengeData->challenger)
    {
        ChatHandler(challenger).PSendSysMessage("El ya está en cola.");
        return false;
    }
	...
	//Colores retado
	char* color_clase_retador;

	switch (challenger->getClass())
	{
		case CLASS_WARRIOR:
			color_clase_retador = "|CFFC79C6E";
		break;
		case CLASS_DEATH_KNIGHT:
			color_clase_retador = "|CFFC41F3B";
		break;
		case CLASS_DRUID:
			color_clase_retador = "|CFFFF7D0A";
		break;
		case CLASS_HUNTER:
			color_clase_retador = "|CFFABD473";
		break;
		case CLASS_MAGE:
			color_clase_retador = "|CFF69CCF0";
		break;
		case CLASS_PALADIN:
			color_clase_retador = "|CFFF58CBA";
		break;
		case CLASS_PRIEST:
			color_clase_retador = "|CFFFFFFFF";
		break;
		case CLASS_ROGUE:
			color_clase_retador = "|CFFFFF569";
		break;
		case CLASS_SHAMAN:
			color_clase_retador = "|CFF0070DE";
		break;
		case CLASS_WARLOCK:
			color_clase_retador = "|CFF9482C9";
		break;
	}
			
    ChatHandler(challengeTo).PSendSysMessage("El jugador %s%s|r te ha retado a %uc%u. Para aceptarlo, escribe .challenge aceptar", color_clase_retador, challenger->GetName(), type, type);
...
bool ChallengeWaitingAcceptEvent::Execute(uint64 /*e_time*/, uint32 /*p_time*/)
{
    ...
    ChatHandler(player).PSendSysMessage("La oferta para el reto ha sido cancelada porque se ha acabado el límite de tiempo para aceptar el reto (20 segundos).");

    if (Player *challenger = ObjectAccessor::FindPlayer(player->challengeData->challenger))
    {
        challenger->challengeData->challengeTo = 0;
        ChatHandler(challenger).PSendSysMessage("La oferta para el reto ha sido cancelada porque se ha acabado el límite de tiempo para aceptar el reto (20 segundos).");
    }
