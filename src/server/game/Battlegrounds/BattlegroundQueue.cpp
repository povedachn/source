...
GroupQueueInfo * BattlegroundQueue::AddGroup(Player* leader, Group* grp, BattlegroundTypeId BgTypeId, PvPDifficultyEntry const*  bracketEntry, uint8 ArenaType, bool isRated, bool isPremade, uint32 ArenaRating, uint32 MatchmakerRating, uint32 arenateamid)
{
	...
	/*if (isRated && GetHora() > 1373031079 && ArenaType == 2)
	{
		index = 0;
	}*/
        ...
        if(!isRated && !isPremade && sWorld->getBoolConfig(CONFIG_BATTLEGROUND_QUEUE_ANNOUNCER_ENABLE))
        {
            if(Battleground* bg = sBattlegroundMgr->GetBattlegroundTemplate(ginfo->BgTypeId))
            {
                if (sWorld->getBoolConfig(CONFIG_CROSSFACTIONBG))
                {
                    ...
                    {
                        ChatHandler(leader->GetSession()).PSendSysMessage("Estad""\xC3\xAD""sticas de Cola para %s (Nivel: %u a %u) Jugadores en cola: %u (Se necesitan al menos %u m""\xC3\xA1""s)", bgName, q_min_level, q_max_level, qPlayers, MinPlayers - qPlayers);
                    }
                    else
                    {
                        std::ostringstream ss;
                        ss << "|cffff0000[Anuncio de Cola de BG]:|r " << bgName << " -- [" << q_min_level << "-" << q_max_level << "] " << qPlayers << "/" << MinPlayers;
...
/*int32 GetHora()
{
	QueryResult resultado = WorldDatabase.PQuery("SELECT UNIX_TIMESTAMP()");
...
void BattlegroundQueue::RemovePlayer(const uint64& guid, bool decreaseInvitedCount)
{
    ...
    {
        if(Battleground* pBG = sBattlegroundMgr->GetBattleground(group->IsInvitedToBGInstanceGUID, group->BgTypeId == BATTLEGROUND_AA ? BATTLEGROUND_TYPE_NONE : group->BgTypeId))
            pBG->DecreaseInvitedCount(group->Team);
    }
    ...
    {
        if(ArenaTeam* pAT = sArenaTeamMgr->GetArenaTeamById(group->ArenaTeamId))
        {
            ...
            if(pPlayer)
                pAT->MemberLost(pPlayer, group->OpponentsMatchmakerRating);
            else
                pAT->OfflineMemberLost(guid, group->OpponentsMatchmakerRating);
            pAT->SaveToDB();
        }
    }
    ...
    else if(!group->IsInvitedToBGInstanceGUID && group->IsRated)
    {
        ...
        {
            Battleground* pBG = sBattlegroundMgr->GetBattlegroundTemplate(group->BgTypeId);
            BattlegroundQueueTypeId bgQueueTypeId = BattlegroundMgr::BGQueueTypeId(group->BgTypeId, group->ArenaType);
            uint32 queueSlot = plr2->GetBattlegroundQueueIndex(bgQueueTypeId);
            plr2->RemoveBattlegroundQueueId(bgQueueTypeId); // must be called this way, because if you move this call to
                                                            // queue->removeplayer, it causes bugs
            WorldPacket data;
            sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, pBG, queueSlot, STATUS_NONE, 0, 0, 0);
...
bool BattlegroundQueue::InviteGroupToBG(GroupQueueInfo * ginfo, Battleground* pBG, uint32 side)
{
    // set side if needed
    if(side)
        ginfo->Team = side;
    ...
    if(!ginfo->IsInvitedToBGInstanceGUID)
    {
        // not yet invited
        // set invitation
        ginfo->IsInvitedToBGInstanceGUID    = pBG->GetInstanceID();
        BattlegroundTypeId bgTypeId         = pBG->GetTypeID();
        BattlegroundQueueTypeId bgQueueTypeId = BattlegroundMgr::BGQueueTypeId(bgTypeId, pBG->GetArenaType());
        BattlegroundBracketId bracket_id    = pBG->GetBracketId();

        // set ArenaTeamId for rated matches
        if(pBG->isArena() && pBG->isRated())
            pBG->SetArenaTeamIdForTeam(ginfo->Team, ginfo->ArenaTeamId);
        ...
        for(std::map<uint64, PlayerQueueInfo*>::iterator itr = ginfo->Players.begin(); itr != ginfo->Players.end(); ++itr)
        {
            ...
            pBG->IncreaseInvitedCount(ginfo->Team);
            ...
            sLog->outDebug(LOG_FILTER_BATTLEGROUND, "Battleground: invited plr %s (%u) to BG instance %u queueindex %u bgtype %u, I can't help it if they don't press the enter battle button.", plr->GetName(), plr->GetGUIDLow(), pBG->GetInstanceID(), queueSlot, pBG->GetTypeID());

            // send status packet
            sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, pBG, queueSlot, STATUS_WAIT_JOIN, INVITE_ACCEPT_WAIT_TIME, 0, ginfo->ArenaType);
...
*/
void BattlegroundQueue::FillPlayersToBG(Battleground* pBG, BattlegroundBracketId bracket_id)
{
    int32 hordeFree = pBG->GetFreeSlotsForTeam(HORDE);
    int32 aliFree   = pBG->GetFreeSlotsForTeam(ALLIANCE);

    if (!pBG->isArena())
        if (FillXPlayersToBG(bracket_id, pBG, false))
            return;
...
bool BattlegroundQueue::CheckNormalMatch(Battleground* pBG_TEMPLATE, BattlegroundBracketId bracket_id, uint32 minPlayers, uint32 maxPlayers)
{
    ...
    if(sBattlegroundMgr->isTesting() && pBG_TEMPLATE->isBattleground() && (m_SelectionPools[BG_TEAM_ALLIANCE].GetPlayerCount() || m_SelectionPools[BG_TEAM_HORDE].GetPlayerCount()))
        return true;
...
void BattlegroundQueue::Update(BattlegroundTypeId bgTypeId, BattlegroundBracketId bracket_id, uint8 arenaType, bool isRated, uint32 arenaRating)
{
    ...
    for(itr = sBattlegroundMgr->BGFreeSlotQueue[bgTypeId].begin(); itr != sBattlegroundMgr->BGFreeSlotQueue[bgTypeId].end(); itr = next)
    {
        ...
        {
            Battleground* pBG = *itr; //we have to store battleground pointer here, because when battleground is full, it is removed from free queue (not yet implemented!!)
            ...
            FillPlayersToBG(pBG, bracket_id);
            ...
            for(GroupsQueueType::const_iterator citr = m_SelectionPools[BG_TEAM_ALLIANCE].SelectedGroups.begin(); citr != m_SelectionPools[BG_TEAM_ALLIANCE].SelectedGroups.end(); ++citr)
                InviteGroupToBG((*citr), pBG, (*citr)->Team);
    ...
    Battleground* pBG_TEMPLATE = sBattlegroundMgr->GetBattlegroundTemplate(bgTypeId);
    if(!pBG_TEMPLATE)
    {
    ...
    PvPDifficultyEntry const* bracketEntry = GetBattlegroundBracketById(pBG_TEMPLATE->GetMapId(), bracket_id);
    if(!bracketEntry)
    {
        sLog->outError("Battleground: Update: bg bracket entry not found for map %u bracket id %u", pBG_TEMPLATE->GetMapId(), bracket_id);
        return;
    }

    // get the min. players per team, properly for larger arenas as well. (must have full teams for arena matches!)
    uint32 MinPlayersPerTeam = pBG_TEMPLATE->GetMinPlayersPerTeam();
    uint32 MaxPlayersPerTeam = pBG_TEMPLATE->GetMaxPlayersPerTeam();
    ...
    if(pBG_TEMPLATE->isBattleground())
    {
        //check if there is premade against premade match
        if(CheckPremadeMatch(bracket_id, MinPlayersPerTeam, MaxPlayersPerTeam))
        {
            //create new battleground
            Battleground* pBG2 = sBattlegroundMgr->CreateNewBattleground(bgTypeId, bracketEntry, 0, false);
            if(!pBG2)
            {
            ...
            for(uint32 i = 0; i < BG_TEAMS_COUNT; i++)
                for(GroupsQueueType::const_iterator citr = m_SelectionPools[BG_TEAM_ALLIANCE + i].SelectedGroups.begin(); citr != m_SelectionPools[BG_TEAM_ALLIANCE + i].SelectedGroups.end(); ++citr)
                    InviteGroupToBG((*citr), pBG2, (*citr)->Team);
            //start bg
            pBG2->StartBattleground();
            //clear structures
            m_SelectionPools[BG_TEAM_ALLIANCE].Init();
            m_SelectionPools[BG_TEAM_HORDE].Init();
        }
    ...
    if(!isRated)
    {
        ...
        if(CheckNormalMatch(pBG_TEMPLATE, bracket_id, MinPlayersPerTeam, MaxPlayersPerTeam)
            || (pBG_TEMPLATE->isArena() && CheckSkirmishForSameFaction(bracket_id, MinPlayersPerTeam))
            || CheckCrossFactionMatch(bracket_id, pBG_TEMPLATE))
        {
            // we successfully created a pool
            Battleground* pBG2 = sBattlegroundMgr->CreateNewBattleground(bgTypeId, bracketEntry, arenaType, false);
            if(!pBG2)
            {
                sLog->outError("BattlegroundQueue::Update - Cannot create battleground: %u", bgTypeId);
                return;
...
bool BGQueueInviteEvent::Execute(uint64 /*e_time*/, uint32 /*p_time*/)
{
    ...
    Battleground* pBG = sBattlegroundMgr->GetBattleground(m_BgInstanceGUID, m_BgTypeId);
    //if battleground ended and its instance deleted - do nothing
    if(!pBG)
        return true;

    BattlegroundQueueTypeId bgQueueTypeId = BattlegroundMgr::BGQueueTypeId(pBG->GetTypeID(), pBG->GetArenaType());
    uint32 queueSlot = plr->GetBattlegroundQueueIndex(bgQueueTypeId);
    if(queueSlot < PLAYER_MAX_BATTLEGROUND_QUEUES)         // player is in queue or in battleground
    {
        ...
        if(bgQueue.IsPlayerInvited(m_PlayerGuid, m_BgInstanceGUID, m_RemoveTime))
        {
            ...
            sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, pBG, queueSlot, STATUS_WAIT_JOIN, INVITE_ACCEPT_WAIT_TIME - INVITATION_REMIND_TIME, 0, m_ArenaType);
            plr->GetSession()->SendPacket(&data);
        }
    }
...
bool BGQueueRemoveEvent::Execute(uint64 /*e_time*/, uint32 /*p_time*/)
{
    ...
    if(queueSlot < PLAYER_MAX_BATTLEGROUND_QUEUES)         // player is in queue, or in Battleground
    {
        ...
        if(bgQueue.IsPlayerInvited(m_PlayerGuid, m_BgInstanceGUID, m_RemoveTime))
        {
            ...
            if(pBG && pBG->isBattleground() && pBG->GetStatus() != STATUS_WAIT_LEAVE)
                sBattlegroundMgr->ScheduleQueueUpdate(0, 0, m_BgQueueTypeId, m_BgTypeId, pBG->GetBracketId());
