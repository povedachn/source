...
{
    m_BuffChange = true;
    BGArrayForObjects.resize(BG_EY_OBJECT_MAX);
    BGArrayForCreatures.resize(BG_EY_CREATURES_MAX);
...
void BattlegroundEY::CheckSomeoneJoinedPoint()
{
    ...
    {
        obj = HashMapHolder<GameObject>::Find(BGArrayForObjects[BG_EY_OBJECT_TOWER_CAP_FEL_REAVER + i]);
        if(obj)
        {
            uint8 j = 0;
    ...
    for(uint8 point = 0; point < EY_POINTS_MAX; ++point)
    {
        ...
        for(uint8 i = 0; i < m_PlayersNearPoint[point].size(); ++i)
        {
            ...
            if(plr)
            {
                ...
                if(point == FEL_REAVER && m_PointOwnedByTeam[point] == plr->GetBGTeam()) // hack fix for Fel Reaver Ruins
                {
...
void BattlegroundEY::RemovePlayer(Player* pPlayer, uint64 guid, uint32 /*team*/)
{
