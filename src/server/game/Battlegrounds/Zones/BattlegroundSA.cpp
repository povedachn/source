...
BattlegroundSA::BattlegroundSA()
{
    ...
    BGArrayForObjects.resize(BG_SA_MAXOBJ);
    BGArrayForCreatures.resize(BG_SA_MAXNPC + BG_SA_MAX_GY);
    TimerEnabled                = false;
    UpdateWaitTimer             = 0;
        ...
        for(BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
        {
            if(Player* pPlayer = sObjectMgr->GetPlayer(itr->first))
            {
                ...
                if(pPlayer->getFaction() == HORDE)
                    CriteriaForAchievement2200Check(pPlayer);
                else if(pPlayer->getFaction() == ALLIANCE)
                    CriteriaForAchievement1757Check(pPlayer);
            }
        }
    }
...
void BattlegroundSA::StartShips()
{
    ...
    ShipsStarted                = true;
    CriteriaForAchievement2200  = true;
    CriteriaForAchievement1757  = true;
}
...
void BattlegroundSA::EventPlayerDamagedGO(Player* /*pPlayer*/, GameObject* go, uint32 eventType)
{
    ...
    if(eventType == go->GetGOInfo()->building.destroyedEvent)
    {
        ...
        CriteriaForAchievement2200 = false;
        CriteriaForAchievement1757 = false;
    }
...
void BattlegroundSA::EventPlayerUsedGO(Player* pSource, GameObject* pGO)
{
    if(pGO->GetEntry() == BG_SA_ObjEntries[BG_SA_TITAN_RELIC] && GateStatus[BG_SA_ANCIENT_GATE] == BG_SA_GATE_DESTROYED && GateStatus[BG_SA_YELLOW_GATE] == BG_SA_GATE_DESTROYED && (GateStatus[BG_SA_PURPLE_GATE] == BG_SA_GATE_DESTROYED || GateStatus[BG_SA_RED_GATE] == BG_SA_GATE_DESTROYED) && (GateStatus[BG_SA_GREEN_GATE] == BG_SA_GATE_DESTROYED || GateStatus[BG_SA_BLUE_GATE] == BG_SA_GATE_DESTROYED))
    {
        if(pSource->GetBGTeamId() == Attackers)
        {
            if(pSource->GetBGTeamId() == TEAM_ALLIANCE)
            ...
            else if(Status == BG_SA_ROUND_TWO)
            {
                ...
                {
                    if(Player* plr = sObjectMgr->GetPlayer(itr->first))
                        if(plr->GetBGTeamId() == Attackers && RoundScores[1].winner == Attackers)
                            plr->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, 65246);
                }
...
void BattlegroundSA::EndBattleground(uint32 winner)
{
    ...
    for(BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
    {
        if(Player* pPlayer = sObjectMgr->GetPlayer(itr->first))
        {
            if(pPlayer->getFaction() == winner && pPlayer->getFaction() == HORDE)
                CriteriaForAchievement2200Check(pPlayer);
            if(pPlayer->getFaction() == winner && pPlayer->getFaction() == ALLIANCE)
                CriteriaForAchievement1757Check(pPlayer);
...
{
    if(BGArrayForObjects[BG_SA_BOAT_ONE] ||  BGArrayForObjects[BG_SA_BOAT_TWO])
    {
        UpdateData transData;
        if(BGArrayForObjects[BG_SA_BOAT_ONE])
            GetBGObject(BG_SA_BOAT_ONE)->BuildCreateUpdateBlockForPlayer(&transData, player);
        if(BGArrayForObjects[BG_SA_BOAT_TWO])
