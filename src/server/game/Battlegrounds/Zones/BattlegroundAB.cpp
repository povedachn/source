...
{
    m_BuffChange = true;
    BGArrayForObjects.resize(BG_AB_OBJECT_MAX);
    BGArrayForCreatures.resize(BG_AB_ALL_NODES_COUNT + 5);//+5 for aura triggers
...
void BattlegroundAB::_NodeDeOccupied(uint8 node)
{
    ...
    std::vector<uint64> ghost_list = m_ReviveQueue[BGArrayForCreatures[node]];
    if(!ghost_list.empty())
    {
    ...
    if(BGArrayForCreatures[node])
        DelCreature(node);
}

/* Invoked if a player used a banner as a GameObject */
void BattlegroundAB::EventPlayerClickedOnFlag(Player* pSource, GameObject* /*pGO*/)
{
    if(GetStatus() != STATUS_IN_PROGRESS)
        return;

    uint8 node = BG_AB_NODE_STABLES;
    GameObject* pGO = GetBgMap()->GetGameObject(BGArrayForObjects[node*8+7]);
    while((node < BG_AB_DYNAMIC_NODES_COUNT) && ((!pGO) || (!pSource->IsWithinDistInMap(pGO, 10))))
    {
        ++node;
        pGO = GetBgMap()->GetGameObject(BGArrayForObjects[node*8+BG_AB_OBJECT_AURA_CONTESTED]);
    }
    ...
    BattlegroundTeamId teamIndex = GetTeamIndexByTeamId(pSource->GetBGTeam());
    ...
    {
        UpdatePlayerScore(pSource, SCORE_BASES_ASSAULTED, 1);
        m_prevNodes[node] = m_Nodes[node];
        ...
        // FIXME: team and node names not localized
        if(teamIndex == 0)
            SendMessage2ToAll(LANG_BG_AB_NODE_CLAIMED, CHAT_MSG_BG_SYSTEM_ALLIANCE, pSource, _GetNodeNameId(node), LANG_BG_AB_ALLY);
        else
            SendMessage2ToAll(LANG_BG_AB_NODE_CLAIMED, CHAT_MSG_BG_SYSTEM_HORDE, pSource, _GetNodeNameId(node), LANG_BG_AB_HORDE);
    ...
    else if((m_Nodes[node] == BG_AB_NODE_STATUS_ALLY_CONTESTED) || (m_Nodes[node] == BG_AB_NODE_STATUS_HORDE_CONTESTED))
    {
        ...
        if(m_prevNodes[node] < BG_AB_NODE_TYPE_OCCUPIED)
        {
            ...
            if(teamIndex == BG_TEAM_ALLIANCE)
                SendMessage2ToAll(LANG_BG_AB_NODE_ASSAULTED, CHAT_MSG_BG_SYSTEM_ALLIANCE, pSource, _GetNodeNameId(node));
            else
                SendMessage2ToAll(LANG_BG_AB_NODE_ASSAULTED, CHAT_MSG_BG_SYSTEM_HORDE, pSource, _GetNodeNameId(node));
        }
        // If contested, change back to occupied
        else
        {
            UpdatePlayerScore(pSource, SCORE_BASES_DEFENDED, 1);
            m_prevNodes[node] = m_Nodes[node];
            ...
            // FIXME: node names not localized
            if(teamIndex == BG_TEAM_ALLIANCE)
                SendMessage2ToAll(LANG_BG_AB_NODE_DEFENDED, CHAT_MSG_BG_SYSTEM_ALLIANCE, pSource, _GetNodeNameId(node));
            else
                SendMessage2ToAll(LANG_BG_AB_NODE_DEFENDED, CHAT_MSG_BG_SYSTEM_HORDE, pSource, _GetNodeNameId(node));
        }
