...
{
    BGArrayForObjects.resize(BG_WS_OBJECT_MAX);
    BGArrayForCreatures.resize(BG_CREATURES_MAX_WS);
...
void BattlegroundWS::EventPlayerClickedOnFlag(Player* Source, GameObject* target_obj)
{
    ...
    if(Source->GetBGTeam() == HORDE && this->GetFlagState(ALLIANCE) == BG_WS_FLAG_STATE_ON_BASE
        && this->BGArrayForObjects[BG_WS_OBJECT_A_FLAG] == target_obj->GetGUID())
    {
    ...
    if(Source->GetBGTeam() == ALLIANCE && this->GetFlagState(HORDE) == BG_WS_FLAG_STATE_ON_BASE
        && this->BGArrayForObjects[BG_WS_OBJECT_H_FLAG] == target_obj->GetGUID())
    {
