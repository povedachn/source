...
{
    BGArrayForObjects.resize(MAX_NORMAL_GAMEOBJECTS_SPAWNS + MAX_AIRSHIPS_SPAWNS + MAX_HANGAR_TELEPORTERS_SPAWNS + MAX_FORTRESS_TELEPORTERS_SPAWNS);
    BGArrayForCreatures.resize(MAX_NORMAL_NPCS_SPAWNS + MAX_WORKSHOP_SPAWNS + MAX_DOCKS_SPAWNS + MAX_SPIRIT_GUIDES_SPAWNS);
...
void BattlegroundIC::DoAction(uint32 action, uint64 const& var)
{
    ...
    pPlayer->SetTransport(pPlayer->GetBGTeamId() == TEAM_ALLIANCE ? gunshipAlliance : gunshipHorde);
    ...
    pPlayer->m_movementInfo.t_guid = (pPlayer->GetBGTeamId() == TEAM_ALLIANCE ? gunshipAlliance : gunshipHorde)->GetGUID();
...
void BattlegroundIC::AddPlayer(Player* pPlayer)
{
    ...
    pPlayer->Achievement3845();
    pPlayer->Achievement3848();
    //create score and add it to map, default values are set in constructor
    BattlegroundICScore* sc = new BattlegroundICScore;

    m_PlayerScores[pPlayer->GetGUID()] = sc;

    if(nodePoint[NODE_TYPE_QUARRY].nodeState == (pPlayer->GetBGTeamId() == TEAM_ALLIANCE ? NODE_STATE_CONTROLLED_A : NODE_STATE_CONTROLLED_H))
        pPlayer->CastSpell(pPlayer, SPELL_QUARRY, true);

    if(nodePoint[NODE_TYPE_REFINERY].nodeState == (pPlayer->GetBGTeamId() == TEAM_ALLIANCE ? NODE_STATE_CONTROLLED_A : NODE_STATE_CONTROLLED_H))
...
void BattlegroundIC::HandleKillUnit(Creature* pUnit, Player* pKiller)
{
    if(GetStatus() != STATUS_IN_PROGRESS)
       return;

    if(pUnit->GetEntry() == NPC_HIGH_COMMANDER_HALFORD_WYRMBANE)
    {
        RewardHonorToTeam(WINNER_HONOR_AMOUNT, HORDE);
        EndBattleground(HORDE);
    } else if(pUnit->GetEntry() == NPC_OVERLORD_AGMAR) {
    ...
    if(pUnit->IsVehicle())
    {
        pKiller->CastSpell(pKiller, SPELL_DESTROYED_VEHICLE_ACHIEVEMENT, true);
        pKiller->T12059 = true;
        pKiller->Achievement3845Check();
    }
}
...
void BattlegroundIC::HandleKillPlayer(Player* pVictim, Player* pKiller)
{
    ...
    factionReinforcements[pVictim->GetBGTeamId()] -= 1;

    UpdateWorldState((pVictim->GetBGTeamId() == TEAM_ALLIANCE ? BG_IC_ALLIANCE_RENFORT : BG_IC_HORDE_RENFORT), factionReinforcements[pVictim->GetBGTeamId()]);
    ...
    if(factionReinforcements[pVictim->GetBGTeamId()] < 1)
        EndBattleground(pKiller->GetBGTeam());
}
...
void BattlegroundIC::RealocatePlayers(ICNodePointType nodeType)
{
    ...
    std::vector<uint64> ghost_list = m_ReviveQueue[BGArrayForCreatures[BG_IC_NPC_SPIRIT_GUIDE_1+nodeType-2]];
    if(!ghost_list.empty())
    {
...
void BattlegroundIC::EventPlayerClickedOnFlag(Player* player, GameObject* pGO)
{
    if(GetStatus() != STATUS_IN_PROGRESS)
        return;
    ...
    for(uint8 i = 0; i < MAX_NODE_TYPES; i++)
    {
        if(nodePoint[i].gameobject_entry == pGO->GetEntry())
        {
                ...
                nodePoint[i].banners[BANNER_H_CONTESTED] == nextBanner)
            {
                ...
                if(nodePoint[i].nodeType != NODE_TYPE_REFINERY && nodePoint[i].nodeType != NODE_TYPE_QUARRY)
                    if(BGArrayForCreatures[BG_IC_NPC_SPIRIT_GUIDE_1+(nodePoint[i].nodeType)-2])
                        DelCreature(BG_IC_NPC_SPIRIT_GUIDE_1+(nodePoint[i].nodeType)-2);

                UpdatePlayerScore(player, SCORE_BASES_ASSAULTED, 1);
                player->T11487          = true;
                player->Achievement3845Check();
