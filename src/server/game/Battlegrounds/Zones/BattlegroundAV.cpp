...
{
    BGArrayForObjects.resize(BG_AV_OBJECT_MAX);
    BGArrayForCreatures.resize(AV_CPLACE_MAX+AV_STATICCPLACE_MAX);
...
void BattlegroundAV::HandleKillPlayer(Player* pVictim, Player* pKiller)
{
    ...
    UpdateScore(pVictim->GetBGTeam(), -1);
    pKiller->T425 = true;
...
void BattlegroundAV::EventPlayerDestroyedPoint(BG_AV_Nodes node)
{
    ...
    if(IsTower(node))
    {
        ...
        //despawn marshal
        if(BGArrayForCreatures[AV_CPLACE_A_MARSHAL_SOUTH + tmp])
...
void BattlegroundAV::DePopulateNode(BG_AV_Nodes node)
{
    uint32 c_place = AV_CPLACE_DEFENSE_STORM_AID + (4 * node);
    for(uint8 i=0; i<4; i++)
        if(BGArrayForCreatures[c_place+i])
            DelCreature(c_place+i);
    //spiritguide
    if(!IsTower(node) && BGArrayForCreatures[node])
        DelCreature(node);
...
void BattlegroundAV::EventPlayerClickedOnFlag(Player* pSource, GameObject* pGO)
{
    if(GetStatus() != STATUS_IN_PROGRESS)
        return;
    int32 object = GetObjectType(pGO->GetGUID());
    sLog->outDebug(LOG_FILTER_BATTLEGROUND, "BG_AV using gameobject %i with type %i", pGO->GetEntry(), object);
    if(object < 0)
        return;
    switch(pGO->GetEntry())
    {
        ...
        case BG_AV_OBJECTID_BANNER_SNOWFALL_N:
            EventPlayerAssaultsPoint(pSource, object);
            break;
        ...
        case BG_AV_OBJECTID_BANNER_CONT_H_B:
            EventPlayerDefendsPoint(pSource, object);
            break;
        default:
            break;
    }
...
void BattlegroundAV::EventPlayerDefendsPoint(Player* pPlayer, uint32 object)
{
    ...
    if(owner == pPlayer->GetBGTeam() || m_Nodes[node].State != POINT_ASSAULTED)
        return;
    ...
    if(m_Nodes[node].TotalOwner != AV_NEUTRAL_TEAM)
    {
        ...
        else
        {
            ...
            if(!ghost_list.empty())
            {
                ...
                m_ReviveQueue[BGArrayForCreatures[node]].clear();
            }
        }
        DePopulateNode(node);
    }
