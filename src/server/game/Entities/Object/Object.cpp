...
void Object::_SetUpdateBits(UpdateMask *updateMask, Player* /*pTarget*/) const
{
...
void Object::_SetCreateBits(UpdateMask *updateMask, Player* /*pTarget*/) const
{
    uint32 *value = m_uint32Values;
...
{
//    if(!this) /* ROLLBACK THAT IF CRASH HAPPEN */
//        return false;
...
TempSummon* WorldObject::SummonCreature(uint32 entry, const Position &pos, TempSummonType spwtype, uint32 duration, uint32 /*vehId*/) const
{
    if(this)
    {
        if(Map* pMap = FindMap())
        {
            if(TempSummon* pSummon = pMap->SummonCreature(entry, pos, NULL, duration, isType(TYPEMASK_UNIT) ? (Unit*)this : NULL))
            {
                pSummon->SetTempSummonType(spwtype);
                return pSummon;
            }
        }
    }
...
void WorldObject::DestroyForNearbyPlayers()
{
    ...
    for(std::list<Player*>::const_iterator iter = targets.begin(); iter != targets.end(); ++iter)
    {
        ...
        if(pPlayer == this)
            continue;
        if(!pPlayer->HaveAtClient(this))
            continue;
        if(isType(TYPEMASK_UNIT) && ((Unit*)this)->GetCharmerGUID() == pPlayer->GetGUID()) // TODO: this is for puppet
            continue;

        DestroyForPlayer(pPlayer);
        pPlayer->m_clientGUIDs.erase(GetGUID());
    }
}
