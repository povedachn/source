...
bool Unit::HasBreakableByDamageAuraType(AuraType type, uint32 excludeAura) const
{
    ...
    for(AuraEffectList::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
        if((!excludeAura || excludeAura != (*itr)->GetSpellProto()->Id) && //Avoid self interrupt of channeled Crowd Control spells like Seduction
           ((*itr)->GetSpellProto()->Attributes & SPELL_ATTR0_BREAKABLE_BY_DAMAGE || (*itr)->GetSpellProto()->AuraInterruptFlags & AURA_INTERRUPT_FLAG_TAKE_DAMAGE))
            return true;
    return false;
}
...
void Unit::CastSpell(Unit* Victim, uint32 spellId, bool triggered, Item* castItem, AuraEffect const* triggeredByAura, uint64 originalCaster)
{
    ...
    CastSpell(Victim, pSpellEntry, triggered, castItem, triggeredByAura, originalCaster);
}

void Unit::CastSpell(Unit* Victim, SpellEntry const* pSpellEntry, bool triggered, Item* castItem, AuraEffect const* triggeredByAura, uint64 originalCaster)
{
    if(!pSpellEntry)
    {
    ...
    if(castItem)
        sLog->outDebug(DebugLogFilters(LOG_FILTER_SPELLS_AURAS | LOG_FILTER_PLAYER_ITEMS), "WORLD: cast Item spellId - %i", pSpellEntry->Id);

    if(!originalCaster && triggeredByAura)
        originalCaster = triggeredByAura->GetCasterGUID();

    Spell* pSpell = new Spell(this, pSpellEntry, triggered, originalCaster);
...
void Unit::AttackerStateUpdate(Unit* pVictim, WeaponAttackType attType, bool extra)
{
    ...
    if((GetTypeId() == TYPEID_PLAYER) && (attType == BASE_ATTACK || attType == OFF_ATTACK) && !this->IsWithinLOSInMap(pVictim) && !isPet())
        return;
...
SpellMissInfo Unit::MagicSpellHitResult(Unit* pVictim, SpellEntry const* spell)
{
   ...
   // Roll chance
	...
	//Custom, esto antes no estaba
	// Resist Chance
	/*if(!IsPositiveSpell(spell->Id))
	{
		if (pVictim->GetTypeId() == TYPEID_PLAYER && sSpellMgr->GetSpellCustomAttr(spell->Id) & SPELL_ATTR0_CU_CAN_RESIST)
		{      
			int32 resistChance = pVictim->GetMechanicResistChance(spell);
			if (resistChance)
			{
...
void Unit::RemoveAurasDueToSpell(uint32 spellId, uint64 casterGUID, uint8 reqEffMask, AuraRemoveMode removeMode)
{
    for(AuraApplicationMap::iterator iter = m_appliedAuras.lower_bound(spellId); iter != m_appliedAuras.upper_bound(spellId);)
    {
        ...
        if(pAura && ((pAura->GetEffectMask() & reqEffMask) == reqEffMask) && (!casterGUID || pAura->GetCasterGUID() == casterGUID))
        {
            RemoveAura(iter, removeMode);
...
void Unit::RemoveAuraFromStack(uint32 spellId, uint64 casterGUID, AuraRemoveMode removeMode)
{
    for(AuraMap::iterator iter = m_ownedAuras.lower_bound(spellId); iter != m_ownedAuras.upper_bound(spellId);)
    {
        ...
        if((pAura->GetType() == UNIT_AURA_TYPE) && (!casterGUID || pAura->GetCasterGUID() == casterGUID))
        {
...
void Unit::RemoveArenaAuras()
{
    ...
    for(AuraApplicationMap::iterator iter = m_appliedAuras.begin(); iter != m_appliedAuras.end();)
    {
        ...
        if(!(aura->GetSpellProto()->AttributesEx4 & SPELL_ATTR4_UNK21) // don't remove stances, shadowform, pally/hunter auras
			...
			&& aura->GetSpellProto()->Id!=3408 //No venenos solo ultimo rango
			&& aura->GetSpellProto()->Id!=5761 //No venenos solo ultimo rango
			&& aura->GetSpellProto()->Id!=57968 //No venenos solo ultimo rango
			&& aura->GetSpellProto()->Id!=57973 //No venenos solo ultimo rango
			&& aura->GetSpellProto()->Id!=57978 //No venenos solo ultimo rango
			&& aura->GetSpellProto()->Id!=57982) //No venenos solo ultimo rango
...
int Unit::GetDiseasesCountOnTarget()
{
    static const AuraType diseaseAuraTypes[] =
    {
        SPELL_AURA_PERIODIC_DAMAGE, // Frost Fever and Blood Plague
    ...
    for(AuraType const* itr = &diseaseAuraTypes[0]; itr && itr[0] != SPELL_AURA_NONE; ++itr)
    {
        for(AuraEffectList::iterator i = m_modAuras[*itr].begin(); i != m_modAuras[*itr].end();)
        {
            ...
            if((*i)->GetSpellProto()->Dispel == DISPEL_DISEASE)
            {
                ++DiseasesCountOnTarget;
            }
            ++i;
        }
    }
    return DiseasesCountOnTarget;
}
            ...
            if(dummySpell->SpellFamilyFlags[1] & 0x80000)
            {
                if(procFlag & PROC_FLAG_TAKEN_SPELL_MAGIC_DMG_CLASS_POS)
                {
                    if(procSpell->SpellFamilyName == SPELLFAMILY_PALADIN && (procSpell->SpellFamilyFlags[0] & 0x40000000))
                    {
                        ...
                        if(basepoints0 && (this->HasSpell(53569) || this->HasSpell(53576)))
                ...
                if(procSpell->SpellFamilyFlags[0] & 0x40000000 && procSpell->SpellIconID == 242)
                {
                    *handled = true;
                    if(victim && victim->HasAura(53601))
                    {
						...
						if(this->HasSpell(53569) || this->HasSpell(53576))
...
void Unit::SetCharm(Unit* charm, bool apply)
{
    ...
    else
    {
		...
		if (charm->IsWalking() != _isWalkingBeforeCharm)
        {
            ...
            charm->SendMovementFlagUpdate(true);
        }
...
int32 Unit::DealHeal(Unit* victim, uint32 addhealth)
{
    ...
    if(Player* player = unit->ToPlayer())
    {
		...
		sScriptMgr->OnPVPHeal(player, victim, uint32(gain));
...
//        modOwner->ApplySpellMod(spellProto->Id, SPELLMOD_CRIT_DAMAGE_BONUS, crit_bonus);
    if((int32)damage > crit_bonus)
    {
        //sLog->outError("DAMAGE value: %u , CRIT_BONUS value: %u", damage, crit_bonus);
        // adds additional damage to critBonus (from talents)
        if(Player* pOwner = GetSpellModOwner())
            pOwner->ApplySpellMod(spellProto->Id, SPELLMOD_CRIT_DAMAGE_BONUS, crit_bonus);
    }
    crit_bonus += damage;
...
bool Unit::isAlwaysVisibleFor(WorldObject const* seer) const
{
    ...
    uint64 guid = GetCharmerOrOwnerGUID();
    if(guid && seer->GetGUID() == guid)
        return true;

    if(Player const* seerPlayer = seer->ToPlayer())
    {
        if(Unit* pOwner = GetOwner())
            if(Player* ownerPlayer = pOwner->ToPlayer())
...
void Unit::setDeathState(DeathState s)
{
    ...
    if(s == JUST_DIED)
    {
        ...
        if(Creature* pCreature = ToCreature())
        {
            if(IsAIEnabled && pCreature->AI()->IsEscorted())
                DisableSpline();
        }
...
Unit* Creature::SelectVictim()
{
    ...
    if(!tauntAuras.empty())
    {
        ...
        {
            target = getVictim();
            if(checkCanReachTargetAndRemoveIfNot(target))
                return target;
        }
    ...
    if(HasReactState(REACT_AGGRESSIVE))
    {
        ...
        if(target && _IsTargetAcceptable(target) && isDestinationReachable(target))
            return target;
    }
...
int32 Unit::ModSpellDuration(SpellEntry const* spellProto, Unit const* target, int32 duration, bool positive, uint32 effectMask)
{
    ...
    else
    {
        ...
        // mixology s:53042 duration bonus
        if(target->GetTypeId() == TYPEID_PLAYER && target->HasAura(53042))
            if(target->ToPlayer()->IsSpellAffectedByMixology(spellProto))
                duration *= 2;
    }
...
{
	if (spellProto->Effect[0] == SPELL_EFFECT_ENCHANT_ITEM || spellProto->Effect[0] == SPELL_EFFECT_APPLY_GLYPH || spellProto->Effect[0] == SPELL_EFFECT_ENCHANT_ITEM_PRISMATIC)
		castTime = 0;
...
void Unit::SetHealth(uint32 val)
{
    ...
    if(Player* plr = ToPlayer())
    {
		if(plr->HaveSpectators())
        {
            SpectatorAddonMsg msg;
            msg.SetPlayer(plr->GetName());
            msg.SetCurrentHP(val);
            plr->SendSpectatorAddonMsgToBG(msg);
        }
...
void Unit::RemoveFromWorld()
{
    ...
    if(IsInWorld())
    {
        ...
        if(GetCharmerGUID())
        {
            ...
            sLog->outErrorDb("CHECK unit.cpp void Unit::RemoveFromWorld(),  void Unit::CleanupsBeforeDelete(bool finalCleanup) GUID: %u , ENTRY: %u ", GetGUID(), GetEntry());
            ASSERT(!GetCharmerGUID());
        }

        if(Unit* pOwner = GetOwner())
        {
            if(pOwner->m_Controlled.find(this) != pOwner->m_Controlled.end())
            {
                sLog->outCrash("Unit %u is in controlled list of %u when removed from world", GetEntry(), pOwner->GetEntry());
                ASSERT(false);
            }
        }
            ...
            for(uint8 effIndex = 0; effIndex<MAX_SPELL_EFFECTS; ++effIndex)
            {
                ...
                switch(triggeredByAura->GetAuraType())
                {
                    ...
                    case SPELL_AURA_TRANSFORM:
                    {
                        ...
                        if(spellInfo->Category == 1215 && procSpell && procSpell->SpellFamilyName == SPELLFAMILY_MAGE && procSpell->SpellFamilyFlags[1] == 0x00010000)
                            break;
...
void Unit::Kill(Unit* pVictim, bool durabilityLoss)
{
    ...
    // find pPlayer: owner of controlled `this` or `this` itself maybe
    ...
    if(pCreature)
    {
        isRewardAllowed = pCreature->IsDamageEnoughForLootingAndReward();
        if(!isRewardAllowed)
            pCreature->SetLootRecipient(NULL);
    }

    if(isRewardAllowed && pCreature && pCreature->GetLootRecipient())
        pPlayer = pCreature->GetLootRecipient();

    // Reward pPlayer, his pets, and group/raid members
    // call kill spell proc event (before real die and combat stop to triggering auras removed at death/combat stop)
    if(isRewardAllowed && pPlayer && pPlayer != pVictim)
    {
        WorldPacket data(SMSG_PARTYKILLLOG, (8+8)); // send event PARTY_KILL
        data << uint64(pPlayer->GetGUID()); // pPlayer with killing blow
        data << uint64(pVictim->GetGUID()); // pVictim

        Player* pLooter = pPlayer;

        if(Group* pGroup = pPlayer->GetGroup())
        {
            pGroup->BroadcastPacket(&data, pGroup->GetMemberGroup(pPlayer->GetGUID()));

            if(pCreature)
            {
                pGroup->UpdateLooterGuid(pCreature, true);
                if(pGroup->GetLooterGuid())
                {
                    if(pLooter = sObjectMgr->GetPlayer(pGroup->GetLooterGuid()))
                    {
                        pCreature->SetLootRecipient(pLooter);   // update pCreature loot recipient to the allowed looter.
                        pGroup->SendLooter(pCreature, pLooter);
                    } else pGroup->SendLooter(pCreature, NULL);
                } else pGroup->SendLooter(pCreature, NULL);

                pGroup->UpdateLooterGuid(pCreature);
            }
        }
        else
        {
            ...
            if(pCreature)
            {
                ...
                data2 << uint64(pCreature->GetGUID());
                data2 << uint8(0); // unk1
                data2 << uint8(0); // no group looter
                pPlayer->SendMessageToSet(&data2, true);
            }
        }
        ...
        if(pCreature)
        {
            ...
            if(pCreature->lootForPickPocketed)
                pCreature->lootForPickPocketed = false;
    ...
    if(pPlayer)
        pPlayer->GetAchievementMgr().UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_GET_KILLING_BLOWS, 1, 0, pVictim);

    // if talent known but not triggered (check priest class for speedup check)
    bool spiritOfRedemption = false;
    if(pVictim->GetTypeId() == TYPEID_PLAYER && pVictim->getClass() == CLASS_PRIEST)
    {
        AuraEffectList const& dummyAuras = pVictim->GetAuraEffectsByType(SPELL_AURA_DUMMY);
        for(AuraEffectList::const_iterator itr = dummyAuras.begin(); itr != dummyAuras.end(); ++itr)
        {
            if((*itr)->GetSpellProto()->SpellIconID == 1654)
            {
                ...
                pVictim->CastSpell(pVictim, 27792, true);
    ...
    // Inform pets (if any) when pPlayer kills target), after victim->setDeathState(JUST_DIED) else will cause problems on next target selection
    if(Player* pPlayer = ToPlayer())
    {
       if(Pet* pPet = pPlayer->GetPet())
            if(pPet->isAlive() && pPet->isControlled())
    ...
    if(Player* plrVictim = pVictim->ToPlayer())
    {
        ...
        plrVictim->SetPvPDeath(pPlayer != NULL);

        // only if not player and not controlled by player pet. And not at BG
        if((durabilityLoss && !pPlayer && !pVictim->ToPlayer()->InBattleground()) || (pPlayer && sWorld->getBoolConfig(CONFIG_DURABILITY_LOSS_IN_PVP)))
        {
        ...
        if(!pCreature->isPet())
        {
            ...
            CreatureTemplate const* cInfo = pCreature->GetCreatureInfo();
        ...
        if(pCreature->IsAIEnabled)
            pCreature->AI()->JustDied(this);

        if(TempSummon* pSummon = pCreature->ToTempSummon())
            if(Unit* pSummoner = pSummon->GetSummoner())
                if(pSummoner->ToCreature() && pSummoner->IsAIEnabled)
                    pSummoner->ToCreature()->AI()->SummonedCreatureDies(pCreature, this);

        // Dungeon specific stuff, only applies to players killing creatures
        if(pCreature->GetInstanceId())
        {
            Map* instanceMap = pCreature->GetMap();
    ...
    {
        if(this != pVictim)
            if(OutdoorPvP* pOPvP = pPlayer->GetOutdoorPvP())
                pOPvP->HandleKill(pPlayer, pVictim);
        ...
        if(pPlayer->InBattleground())
        {
            ...
            {
                if(pVictim->GetTypeId() == TYPEID_PLAYER)
                    pBG->HandleKillPlayer((Player*)pVictim, pPlayer);
                else
                    pBG->HandleKillUnit(pVictim->ToCreature(), pPlayer);
            }
        }
    }
    // achievement stuff
    if(pVictim->GetTypeId() == TYPEID_PLAYER)
    {
        if(GetTypeId() == TYPEID_UNIT)
            ...
            pVictim->ToPlayer()->GetAchievementMgr().UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_KILLED_BY_PLAYER, 1, ToPlayer()->GetTeam());
    }
    ...
    if(pVictim->GetVehicle())
        pVictim->ExitVehicle();
    if(Vehicle* pVehicle = pVictim->GetVehicleKit())
...
bool Unit::IsInPartyWith(Unit const* pUnit) const
{
    if(pUnit)
    {
        if(this == pUnit)
        ...
        const Unit* u2 = pUnit->GetCharmerOrOwnerOrSelf();
        if(u1 == u2)
            return true;
...
bool Unit::IsInRaidWith(Unit const* pUnit) const
{
    if(pUnit)
    {
        if(this == pUnit)
...
{
    if(Unit* pOwner = GetCharmerOrOwnerOrSelf())
    {
        Group* pGroup = NULL;
        ...
        if(pGroup)
        {
            uint8 subgroup = pOwner->ToPlayer()->GetSubGroup();
            ...
            if(pOwner->isAlive() && (pOwner == this || IsWithinDistInMap(pOwner, radius)))
                TagUnitMap.push_back(pOwner);
            if(Guardian* pet = pOwner->GetGuardianPet())
                if(pet->isAlive() && (pet == this || IsWithinDistInMap(pet, radius)))
                    TagUnitMap.push_back(pet);
        }
    }
...
void Unit::GetPartyMembers(std::list<Unit*> &TagUnitMap)
{
    if(Unit* pOwner = GetCharmerOrOwnerOrSelf())
    {
        ...
        if(pGroup)
        {
            ...
            if(pOwner->isAlive() && (pOwner == this || IsInMap(pOwner)))
                TagUnitMap.push_back(pOwner);
            if(Guardian* pet = pOwner->GetGuardianPet())
    ...
    switch(type)
    {
        ...
        case CR_CRIT_TAKEN_SPELL:
            ...
            if(source && damage)
            {
				...
				{
					if (isCrit)
						*damage -= target->GetSpellWarlockCritDamageReduction(*damage);
...
void Unit::_EnterVehicle(Vehicle* pVehicle, int8 seatId, AuraApplication const* aurApp)
{
    if(!pVehicle || !isAlive() || GetVehicleKit() == pVehicle || pVehicle->GetBase()->IsOnVehicle(this))
        return;
    ...
    if(m_vehicle)
    {
        if(m_vehicle == pVehicle)
        {
            ...
            sLog->outDebug(LOG_FILTER_VEHICLES, "EnterVehicle: %u exit %u and enter %u.", GetEntry(), m_vehicle->GetBase()->GetEntry(), pVehicle->GetBase()->GetEntry());
            ExitVehicle();
        }
    }
    ...
    if(Player* pPlayer = ToPlayer())
    {
        if(pVehicle->GetBase()->GetTypeId() == TYPEID_PLAYER && pPlayer->isInCombat())
            return;

        InterruptNonMeleeSpells(false);
        pPlayer->StopCastingCharm();
        pPlayer->StopCastingBindSight();
        Unmount();
        RemoveAurasByType(SPELL_AURA_MOUNTED);

        // drop flag at invisible in bg
        if(Battleground* pBG = pPlayer->GetBattleground())
            pBG->EventPlayerDroppedFlag(pPlayer);
...
bool Unit::isDestinationReachable(WorldObject* destObj) const
{
    if(!destObj->IsInWorld())
        return false;

    return isDestinationReachable(destObj->GetPositionX(), destObj->GetPositionY(), destObj->GetPositionZ());
}
...
void Unit::SetVisibleAura(uint8 slot, AuraApplication* pAuraApplication)
{
    if(Aura* aura = pAuraApplication->GetBase())
        if(Player* player = ToPlayer())
            if(player->HaveSpectators() && slot < MAX_AURAS)
            {
                ...
                msg.CreateAura(casterID, aura->GetSpellProto()->Id,
                    pAuraApplication->IsPositive(), aura->GetSpellProto()->Dispel,
                    aura->GetDuration(), aura->GetMaxDuration(),
                    aura->GetStackAmount(), false);
                player->SendSpectatorAddonMsgToBG(msg);
            }

    m_visibleAuras[slot] = pAuraApplication;
