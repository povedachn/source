...
#include "../../../scripts/World/Custom/Transmogrification.h"
#include "Common.h"
#include "Language.h"
...
#pragma warning(default:4355)
    ...
    moveChecker = new PlayerMovementChecker(this);
}
...
uint32 Player::EnvironmentalDamage(EnviromentalDamage type, uint32 damage)
{
    ...
    if(!isAlive())
    {
        ...
        {
            sLog->outDebug(LOG_FILTER_UNITS, "We are fall to death, loosing 10 percents durability");
...
/*int IsVIP(Player* player)
{
	QueryResult account = LoginDatabase.PQuery("SELECT gmlevel FROM account_access WHERE id = '%u'", player->GetSession()->GetAccountId());
...
int OchoHoras(Player* player)
{
	QueryResult jugadas = CharacterDatabase.PQuery("SELECT 8_horas FROM characters WHERE guid = '%u'", player->GetGUID());

	if (!jugadas)
		return 0;
	else
	{
		Field *fields = jugadas->Fetch();
			return fields[0].GetUInt32();
	}
}

int Expulsar_Jugadores_Eventos()
{
	QueryResult Expulsar_Jugadores_evento = WorldDatabase.PQuery("SELECT Expulsar_Jugadores FROM eventos_custom WHERE id = 1");

	if (!Expulsar_Jugadores_evento)
		return 0;
	else
	{
		Field *fields = Expulsar_Jugadores_evento->Fetch();
			return fields[0].GetUInt32();
	}
}
...
{
	//Aquí está el script de revivir auto al morir. Si quieres usar las coordenadas aleatorias para revivir, quita el "/*" y el "*/" que ves a continuación, si no, dejalo comentado y el jugador revivirá sobre su cadaver
	/*if ((GetZoneId() == 40 || GetZoneId() == 0) && GetMapId() == 0 && !isAlive())
	{
		ResurrectPlayer(1.0, false);
		int32 RespawnPos = urand(0, MAX_PLAYER_SPAWN_POINTS_EVENTO_LABERINTO - 1);
        TeleportTo(1, playerSpawnPointEventoLaberinto[RespawnPos].GetPositionX(), playerSpawnPointEventoLaberinto[RespawnPos].GetPositionY(), playerSpawnPointEventoLaberinto[RespawnPos].GetPositionZ(), playerSpawnPointEventoLaberinto[RespawnPos].GetOrientation());
        ResetAllPowers();
		RemoveArenaSpellCooldowns();
		RemoveAllSpellCooldown();
		CastSpell(this, 8611, false);
	}*/

	//Custom, script phase duel
	//Zona de duelos, por defecto
	/*if(GetAreaId() == 3893 && GetPhaseMask() == PHASEMASK_NORMAL && !(m_serverSideVisibilityDetect.GetValue(SERVERSIDE_VISIBILITY_GM) == 1) && !isGameMaster())
	{
		m_serverSideVisibilityDetect.SetValue(SERVERSIDE_VISIBILITY_GM, 1);
		m_serverSideVisibility.SetValue(SERVERSIDE_VISIBILITY_GM, 1);
		UpdateObjectVisibility();
	}
	else if(GetAreaId() != 3893 && GetPhaseMask() == PHASEMASK_NORMAL && !(m_serverSideVisibilityDetect.GetValue(SERVERSIDE_VISIBILITY_GM) == 0) && !isGameMaster())
	{
		m_serverSideVisibilityDetect.SetValue(SERVERSIDE_VISIBILITY_GM, 0);
		m_serverSideVisibility.SetValue(SERVERSIDE_VISIBILITY_GM, 0);
		UpdateObjectVisibility();
	}*/

	//Evento laberinto, expulsar jugadores del evento haciendo .event start 94
	if ((GetZoneId() == 267 || GetZoneId() == 45/*Escalada1*/
	...
	if (GetAreaId() != 3431 && (HasItemOrGemWithIdEquipped(1404,1) || HasItemOrGemWithIdEquipped(13209,1)))
	{
		ItemPosCountVec off_dest;
		Item* FirstItem = GetItemByPos(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_TRINKET1);
		InventoryResult off_msg_2 = CanStoreItem(NULL_BAG, NULL_SLOT, off_dest, FirstItem, false);
		if(off_msg_2 == EQUIP_ERR_OK)
		{
			RemoveItem(INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_TRINKET1, true);
			StoreItem(off_dest, FirstItem, true);
		}
		else
			SendEquipError(off_msg_2, FirstItem, NULL);
	}
	...
	//Expulsar del si el player esta en la Zona PvP con un grupo mayor a 3 personas
	if (GetAreaId() == 368 && group && group->IsCreated() && group->GetMembersCount() > 3)
	{
		ChatHandler(this).PSendSysMessage("|CFFFF0000Has sido expulsado del grupo porque el m""\xC3\xA1""ximo n""\xC3\xBA""mero de personas en grupo en la Zona PvP es de 3.|R");
		uint32 guid = GUID_LOPART(GetGUIDLow());
		if(QueryResult resultGroup = CharacterDatabase.PQuery("SELECT guid FROM group_member WHERE memberGuid=%u", guid))
			if(Group* group = sGroupMgr->GetGroupByDbStoreId((*resultGroup)[0].GetUInt32()))
				RemoveFromGroup(group, GetGUIDLow());
	}

	//Expulsar del si el player esta en el Evento Laberinto con un grupo mayor a 2 personas
	/*if ((GetZoneId() == 40 || GetZoneId() == 0) && GetMapId() == 0 && group && group->IsCreated() && group->GetMembersCount() > 2)
	{
		ChatHandler(this).PSendSysMessage("|CFFFF0000Has sido expulsado del grupo porque el m""\xC3\xA1""ximo n""\xC3\xBA""mero de personas en grupo en el Evento Laberinto es de 2.|R");
		uint32 guid = GUID_LOPART(GetGUIDLow());
		if(QueryResult resultGroup = CharacterDatabase.PQuery("SELECT guid FROM group_member WHERE memberGuid=%u", guid))
			if(Group* group = sGroupMgr->GetGroupByDbStoreId((*resultGroup)[0].GetUInt32()))
				RemoveFromGroup(group, GetGUIDLow());
	...
	//Expulsar del grupo si el player esta en Liza con un grupo
	if (GetAreaId() == 2177 && GetZoneId() == 33 && GetMapId() == 0 && group)
	{
		ChatHandler(this).PSendSysMessage("|CFFFF0000Se te ha expulsado del grupo por entrar en Liza estando en un grupo.|R");
		uint32 guid = GUID_LOPART(GetGUIDLow());
		if(QueryResult resultGroup = CharacterDatabase.PQuery("SELECT guid FROM group_member WHERE memberGuid=%u", guid))
			if(Group* group = sGroupMgr->GetGroupByDbStoreId((*resultGroup)[0].GetUInt32()))
				RemoveFromGroup(group, GetGUIDLow());
	...
	//Limites de la zona PvP, te teletransporta si sales de esos limites
	if (GetZoneId() == 14 || GetZoneId() == 17)
	{
		if (!isGameMaster() && (GetAreaId() == 393 || GetAreaId() == 391 || GetAreaId() == 373 || GetAreaId() == 2320 || (GetAreaId() == 2319 && GetZoneId() == 17 && GetMapId() == 1)))
		{
			ChatHandler(this).PSendSysMessage("Has sido teletransportado a la Zona PvP por salir de ella.");
			TeleportTo(1, -949.171f, -5179.04f, 2.11f, 3.98f);
		}
	}

	//Limites Terrallende sin contar duel zone
	/*if(GetMapId() == 530 && !(GetAreaId() == 3893 || GetAreaId() == 3431) && !isGameMaster())
	{
		ChatHandler(this).PSendSysMessage("Lo siento, no puedes salir de los límites. Has sido teletransportado a la Shop.");
		TeleportTo(1, -2315.19951, -413.394012, -6.3581, 1.916640);
	}*/

	//Limites Northrend
	/*if(GetMapId() == 571 && !isGameMaster())
	{
		ChatHandler(this).PSendSysMessage("Lo siento, no puedes salir de los límites. Has sido teletransportado a la Shop.");
		TeleportTo(1, -2315.19951, -413.394012, -6.3581, 1.916640);
	}*/

	//Limites Reinos del Este, menos zona monturas y Guru
	/*if(GetMapId() == 0 && !(GetZoneId() == 40 || GetZoneId() == 0 || GetAreaId() == 10 || GetAreaId() == 2177 || GetAreaId() == 1741 || GetAreaId() == 19 || GetAreaId() == 477) && !isGameMaster())
	{
		ChatHandler(this).PSendSysMessage("Lo siento, no puedes salir de los límites. Has sido teletransportado a la Shop.");
		TeleportTo(1, -2315.19951, -413.394012, -6.3581, 1.916640);
	}*/

	//Limites Kalimdor, menos Zona PvP y alrededores
	/*if(GetMapId() == 1 && !(GetAreaId() == 976 || GetAreaId() == 876 || GetAreaId() == 368 || GetAreaId() == 393 || GetAreaId() == 391 || GetAreaId() == 373 || GetAreaId() == 2320 || (GetAreaId() == 2319 && GetZoneId() == 17)) && !isGameMaster())
	{
		ResurrectPlayer(1.0, false);
        ResetAllPowers();
		RemoveArenaSpellCooldowns();
		RemoveAllSpellCooldown();
		CastSpell(this, 8611, false);
		TeleportTo(1, -949.171, -5179.04, 2.11, 3.98);
		if (getClass() == CLASS_HUNTER || getClass() == CLASS_WARLOCK)
			CastSpell(this, 883, false);
	}*/

	//Borrar racha de sanacion y muertes si no esta en la Zona PvP o en BG
	if ((InBattleground() && !GetBattleground()->Is1vs1()) || GetAreaId() == 368)
	{
		//No hacer nada
	}
	else
	{
		QueryResult result = CharacterDatabase.PQuery("SELECT guid FROM racha WHERE guid = '%u'", GetGUID());
		if (result)
			CharacterDatabase.PExecute("DELETE FROM racha WHERE guid='%u'", GetGUID());

		QueryResult result2 = CharacterDatabase.PQuery("SELECT guid FROM racha_sanacion WHERE guid = '%u'", GetGUID());
		if (result2)
				CharacterDatabase.PExecute("DELETE FROM racha_sanacion WHERE guid='%u'", GetGUID());
	}

	//Premio a quien lleva 8 horas de /played
	//static bool OchoHoras = true;

	if(m_Played_time[PLAYED_TIME_TOTAL] == 28800)
	{
		if(OchoHoras(this) == 0)
		{
			ModifyArenaPoints(480);
			ModifyHonorPoints(9000);
			ChatHandler(this).PSendSysMessage("Por estar 8 horas jugando, has obtenido 9000 Honor Puntos de Honor y 480 Puntos de Arena.");
			CharacterDatabase.PExecute("UPDATE characters SET 8_horas = 1 WHERE guid = '%u'", GetGUID());
		}
	}

	//Quitar el efecto de las camisas de ilusion en Arenas
	/*if(InArena())
	{
		if(HasAura(37810))
			RemoveAurasDueToSpell(37810);
		if(HasAura(37807))
			RemoveAurasDueToSpell(37807);
		if(HasAura(37809))
			RemoveAurasDueToSpell(37809);
		if(HasAura(37805))
			RemoveAurasDueToSpell(37805);
		if(HasAura(37811))
			RemoveAurasDueToSpell(37811);
		if(HasAura(35466))
			RemoveAurasDueToSpell(35466);
		if(HasAura(37808))
			RemoveAurasDueToSpell(37808);
		if(HasAura(37806))
			RemoveAurasDueToSpell(37806);
		if(HasAura(41983))
			RemoveAurasDueToSpell(41983);
		if(HasAura(39505))
			RemoveAurasDueToSpell(39505);
		if(HasAura(34850))
			RemoveAurasDueToSpell(34850);
		if(HasAura(34847))
			RemoveAurasDueToSpell(34847);
		if(HasAura(34845))
			RemoveAurasDueToSpell(34845);
		if(HasAura(34848))
			RemoveAurasDueToSpell(34848);
		if(HasAura(34849))
			RemoveAurasDueToSpell(34849);
		if(HasAura(34851))
			RemoveAurasDueToSpell(34851);
		if(HasAura(44372))
			RemoveAurasDueToSpell(44372);
	}*/

	// Shop Vip
	if(((HasItemCount(1404, 1) || HasItemCount(13209, 1)) && (GetAreaId() == 3431 && GetZoneId() == 3430 && GetMapId() == 530)) || isGameMaster())
	{
		//No hacer nada si tiene los items, esta en el sitio y es gm
	}
	...
	// Shop neutral
	if(GetZoneId() == 440 && GetAreaId() == 976 && GetMapId() == 1)
	{
		if(!isGameMaster())
		{
	...
	//Custom
	//uint32 tiempo_mensaje = 1 * MINUTE;
	/*QueryResult contar_area = CharacterDatabase.PQuery("SELECT COUNT(area) FROM characters WHERE `area` = 368");
	QueryResult contar_area_jugador = CharacterDatabase.PQuery("SELECT area FROM characters WHERE `guid` = '%u'", GetGUID());

	Field* fields = contar_area->Fetch();
	uint32 contador = fields[0].GetUInt32();
	Field* fields2 = contar_area_jugador->Fetch();
	uint32 valor_area = fields2[0].GetUInt32();

	//Custom
	if(GetAreaId() == 368)
	{
		if(valor_area != 368)
			CharacterDatabase.PExecute("UPDATE characters SET area = 368 WHERE guid = '%u'", GetGUID());

		if(p_time >= tiempo_mensaje)
		{
			ChatHandler(this).PSendSysMessage("|CFFFFA500En este momento hay |CFFFF00FF%u|R|CFFFFA500 %s en la Zona PvP. Este mensaje se repetir""\xC3\xA1"" cada 3 minutos.|R", contador, contador != 1 ? "jugadores" : "jugador");
			tiempo_mensaje = 180000;
		}
		else
			tiempo_mensaje -= p_time;
	}
	else if(GetAreaId() != 368)
	{
		if (valor_area == 368)
			CharacterDatabase.PExecute("UPDATE characters SET area = 0 WHERE guid = '%u'", GetGUID());
	}*/
    ...
    if(m_nextSave > 0)
    {
        if(p_time >= m_nextSave)
        {
            ...
            {
				//short valor = urand(0,4);
				//QueryResult result = CharacterDatabase.PQuery("SELECT zone FROM characters WHERE online > 1");
				//if(result)
				//{
				//CharacterDatabase.PExecute("UPDATE characters SET zone = (FLOOR(50 * RAND()) + 1) WHERE online > 1");
						//Field* fields = result->Fetch();
						//uint32 zona = fields[0].GetUInt32();
						//if (/*valor == 0 && */zona == 3707) // Campamento Ali
						//	CharacterDatabase.PExecute("UPDATE characters SET zone = 3519 WHERE online > 1 AND zone = 3707");
						//else if (/*valor == 1*/zona == 3708) // Campamento Horda
						//	CharacterDatabase.PExecute("UPDATE characters SET zone = 3707 WHERE online > 1 AND zone = 3708");
						//else if (/*valor == 2*/zona == 33) // Guru
						//	CharacterDatabase.PExecute("UPDATE characters SET zone = 3708 WHERE online > 1 AND zone = 33");
						//else if (/*valor == 3*/zona == 4395) // Shop Neutral
						//	CharacterDatabase.PExecute("UPDATE characters SET zone = 33 WHERE online > 1 AND zone = 4395");
						//else if (/*valor == 4*/zona == 3519) // Zona de duelos (Bosque de Terokkar)
						//	CharacterDatabase.PExecute("UPDATE characters SET zone = 4395 WHERE online > 1 AND zone = 3519");
				//}
				CharacterDatabase.PExecute("UPDATE characters SET zone = (SELECT zona FROM Fake_players ORDER BY RAND() LIMIT 1) WHERE online > 1");
...
bool Player::TeleportTo(uint32 mapid, float x, float y, float z, float orientation, uint32 options)
{
   ...
   //|| (mEntry->MapID == 571 && GetSession()->GetPlayer()->getLevel() < 68
   //|| (mEntry->MapID == 530 && GetSession()->GetPlayer()->getLevel() < 58)))
    {
    ...
    else
    {
        ...
        if(!map || map->CanEnter(this))
        {
            ...
            if(Battleground const* pBG = GetBattleground())
            {
                // Note: at battleground join battleground id set before teleport
                // and we already will found "current" battleground
                // just need check that this is targeted map or leave
                if(pBG->GetMapId() != mapid)
...
bool Player::IsInSameGroupWith(Player const* pPlayer) const
{
    if(!pPlayer)
        return false;
    return pPlayer == this || (GetGroup() != NULL &&
        GetGroup() == pPlayer->GetGroup() &&
        GetGroup()->SameSubGroup(this, pPlayer));
}
...
bool Player::AddTalent(uint32 spell_id, uint8 spec, bool learning)
{
	///Guerrero
	// Golpe Mortal
	if(HasSpell(12294) && !HasSpell(47486))
		learnSpell(47486, false);
	// Devastar
	if(HasSpell(20243) && !HasSpell(47498))
		learnSpell(47498, false);

	///Paladin
	// Choque Sagrado
	if(HasSpell(20473) && !HasSpell(48825))
		learnSpell(48825, false);
	// Escudo Sagrado
	if(HasSpell(20925) && !HasSpell(48952))
		learnSpell(48952, false);
	// Bendicion de Salvaguardia
	if(HasSpell(20911) && !HasSpell(25899))
		learnSpell(25899, false);
	// Escudo del Vengador
	if(HasSpell(31935) && !HasSpell(48827))
		learnSpell(48827, false);

	///Cazador
	// Disparo de Punteria
	if(HasSpell(19434) && !HasSpell(49050))
		learnSpell(49050, false);
	// Contraataque
	if(HasSpell(19306) && !HasSpell(48999))
		learnSpell(48999, false);
	// Picadura de Dracoleon
	if(HasSpell(19386) && !HasSpell(49012))
		learnSpell(49012, false);
	// Flecha Negra
	if(HasSpell(3674) && !HasSpell(63672))
		learnSpell(63672, false);
	// Disparo Explosivo
	if(HasSpell(53301) && !HasSpell(60053))
		learnSpell(60053, false);

	///Picaro
	// Mutilar
	if(HasSpell(1329) && !HasSpell(48666))
		learnSpell(48666, false);
	// Hemorragia
	if(HasSpell(16511) && !HasSpell(48660))
		learnSpell(48660, false);

	///Sacerdote
	// Penitencia
	if(HasSpell(47540) && !HasSpell(53007))
		learnSpell(53007, false);
	// Tortura Mental
	if(HasSpell(15407) && !HasSpell(48156))
		learnSpell(48156, false);
	// Toque Vampirico
	if(HasSpell(34914) && !HasSpell(48160))
		learnSpell(48160, false);
	// Rezo Desesperado
	if(HasSpell(19236) && !HasSpell(48173))
		learnSpell(48173, false);
	// Pozo de Luz
	if(HasSpell(724) && !HasSpell(48087))
		learnSpell(48087, false);
	// Circulo de Sanacion
	if(HasSpell(34861) && !HasSpell(48089))
		learnSpell(48089, false);

	///Caballero de la Muerte
	// Golpe de Escarcha
	if(HasSpell(49143) && !HasSpell(55268))
		learnSpell(55268, false);
	// Explosion Aullante
	if(HasSpell(49184) && !HasSpell(51411))
		learnSpell(51411, false);
	// Deflagracion de Cadaver
	if(HasSpell(49158) && !HasSpell(51328))
		learnSpell(51328, false);
	// Golpe de la Plaga
	if(HasSpell(55090) && !HasSpell(55271))
		learnSpell(55271, false);
	// Golpe en el Corazon
	if(HasSpell(55050) && !HasSpell(55262))
		learnSpell(55262, false);

	///Chaman
	// Totem de Colera
	if(HasSpell(30706) && !HasSpell(57722))
		learnSpell(57722, false);
	// Tormenta de Truenos
	if(HasSpell(51490) && !HasSpell(59159))
		learnSpell(59159, false);
	// Escudo de Tierra
	if(HasSpell(974) && !HasSpell(49284))
		learnSpell(49284, false);
	// Mareas Vivas
	if(HasSpell(61295) && !HasSpell(61301))
		learnSpell(61301, false);

	///Mago
	// Tromba Arcana
	if(HasSpell(44425) && !HasSpell(44781))
		learnSpell(44781, false);
	// Barrera de Hielo
	if(HasSpell(11426) && !HasSpell(43039))
		learnSpell(43039, false);
	// Piroexplosion
	if(HasSpell(11366) && !HasSpell(42891))
		learnSpell(42891, false);
	// Ola Explosiva
	if(HasSpell(11113) && !HasSpell(42945))
		learnSpell(42945, false);
	// Aliento de Dragon
	if(HasSpell(31661) && !HasSpell(42950))
		learnSpell(42950, false);
	// Bomba Viva
	if(HasSpell(44457) && !HasSpell(55360))
		learnSpell(55360, false);

	///Brujo
	// Pacto Oscuro
	if(HasSpell(18220) && !HasSpell(59092))
		learnSpell(59092, false);
	// Afliccion Inestable
	if(HasSpell(30108) && !HasSpell(30108))
		learnSpell(30108, false);
	// Poseer
	if(HasSpell(48181) && !HasSpell(59164))
		learnSpell(59164, false);
	// Quemadura de las Sombras
	if(HasSpell(17877) && !HasSpell(47827))
		learnSpell(47827, false);
	// Furia de las Sombras
	if(HasSpell(30283) && !HasSpell(47847))
		learnSpell(47847, false);
	// Descarga de Caos
	if(HasSpell(50796) && !HasSpell(59172))
		learnSpell(59172, false);

	///Druida
	// Destrozar (oso)
	if(HasSpell(33917) && !HasSpell(48564))
		learnSpell(48564, false);
	// Destrozar (felino)
	if(HasSpell(33917) && !HasSpell(48566))
		learnSpell(48566, false);
	// Enjambre de Insectos
	if(HasSpell(5570) && !HasSpell(48468))
		learnSpell(48468, false);
	// Tifon
	if(HasSpell(50516) && !HasSpell(61384))
		learnSpell(61384, false);
	// Lluvia de Estrellas
	if(HasSpell(48505) && !HasSpell(53201))
		learnSpell(53201, false);
	// Crecimiento Salvaje
	if(HasSpell(48438) && !HasSpell(53251))
		learnSpell(53251, false);
...
void Player::removeSpell(uint32 spell_id, bool disabled, bool learn_low_rank)
{
    ...
    if(spell_id == 46917 && m_canTitanGrip)
    {
        ...
        RemoveAurasDueToSpell(49152); // Remove Titan's Grip damage penalty now
    }
...
bool Player::resetTalents(bool no_cost)
{
	...
	{
		ChatHandler(this).PSendSysMessage("|CFFFF0000No puedes reiniciarte los talentos mientras estás en cola de Arenas 1v1.|r");
		return false;
	}
...
void Player::DeleteFromDB(uint64 playerguid, uint32 accountId, bool updateRealmChars, bool deleteFinally)
{
    ...
    switch(charDelete_method)
    {
        ...
        case CHAR_DELETE_UNLINK:
            CharacterDatabase.PExecute("UPDATE `characters` SET `at_login` = at_login|0x100,  `deleteInfos_Name` = name, `deleteInfos_Account` = account, `deleteDate` ='" UI64FMTD "', `name` ='', `account` = 0 WHERE `guid` = %u", uint64(time(NULL)), guid);
            break;
        default:
...
/**
 * Elimina todos los equipos de arenas cuando acaba la Season
 */
void Player::DeleteArenaSeasons() //Custom Khanx
{
	uint32 season_reset = sWorld->getIntConfig(CONFIG_ARENA_SEASON_RESET);

	if(!season_reset) //Si el valor es 0 no ejecuta nada
        return;

	QueryResult start = CharacterDatabase.PQuery("SELECT season FROM arena_season WHERE start IS NOT NULL AND start < '%u'", uint32(time(NULL) - time_t(season_reset * DAY)));

	if(start)
		{
			sLog->outString("Player::DeleteArenaSeasons: Otorgando Premios");



			sLog->outString("Player::DeleteArenaSeasons: Eliminando equipos de arenas");
				QueryResult limpieza = CharacterDatabase.PQuery("TRUNCATE TABLE arena_team, arena_team_member");
...
void Player::BuildPlayerRepop()
{
	...
	// Se quita el aura
	if(HasAura(2479))
		RemoveAurasDueToSpell(2479);

	// 350% de velocidad estando muerto
	/*if(HasAura(8326))
	{
		if(!InArena() || !InBattleground())
		{
			SetSpeed(MOVE_RUN, 3.5, true);
			SetSpeed(MOVE_WALK, 3.5, true);
			SetSpeed(MOVE_SWIM, 3.5, true);
		}
	}*/
...
void Player::ResurrectPlayer(float restore_percent, bool applySickness)
{
	...
	CastSpell(this, 2479, true); 	// Objetivo sin honor
    ...
    if(InBattleground())
    {
        if(Battleground* pBG = GetBattleground())
            pBG->HandlePlayerResurrect(this);
    }
    ...
    if(cost)
    {
        ...
        if(LostDurability>0)
        {
            ...
            if(guildBank)
            {
                if(GetGuildId() == 0)
                {
                    sLog->outDebug(LOG_FILTER_UNITS, "You are not member of a guild");
                    return TotalCost;
                }
            ...
            {
                sLog->outDebug(LOG_FILTER_UNITS, "You do not have enough money");
                return TotalCost;
            }
            else
...
void Player::RepopAtGraveyard()
{
    ...
    if(Battleground* pBG = GetBattleground())
        ClosestGrave = pBG->GetClosestGraveYard(this);
    else
...
bool Player::RewardHonor(Unit* uVictim, uint32 groupsize, int32 honor, bool pvptoken)
{
    ...
    {
        if(Battleground* pBG = GetBattleground())
            pBG->UpdatePlayerScore(this, SCORE_BONUS_HONOR, honor, false); //false: prevent looping
    }
...
uint32 Player::GetAreaIdFromDB(uint64 guid)
{
    uint32 guidLow = GUID_LOPART(guid);
    QueryResult result = CharacterDatabase.PQuery("SELECT area FROM characters WHERE guid='%u'", guidLow);
    if(!result)
        return 0;
    Field* fields = result->Fetch();
    uint32 area = fields[0].GetUInt16();

    if(!area)
    {
        // stored area is zero, use generic and slow area detection
        ...
        area = sMapMgr->GetAreaId(map, posx, posy, posz);

        if(area > 0)
            CharacterDatabase.PExecute("UPDATE characters SET area='%u' WHERE guid='%u'", area, guidLow);
    }
...
void Player::UpdateArea(uint32 newArea)
{
    ...
    if(area && area->IsSanctuary())    // in sanctuary
    {
		...
		if(!duel)	//Fix salir de combate shadowstep (paso de las sombras) en zonas santuario
			CombatStopWithPets();
    }
    else
...
void Player::DuelComplete(DuelCompleteType type)
{
    ...
    switch(type)
    {
        case DUEL_FLED:
			...
			//Custom, script phase duel
			/*if (duel->initiator->GetAreaId() == 3893)
			{
				duel->initiator->m_serverSideVisibility.SetValue(SERVERSIDE_VISIBILITY_GM, 1);
				duel->initiator->m_serverSideVisibilityDetect.SetValue(SERVERSIDE_VISIBILITY_GM, 1);
				duel->initiator->SetPhaseMask(3, true);
				if(duel->initiator->GetPet())
				{
					duel->initiator->GetPet()->m_serverSideVisibility.SetValue(SERVERSIDE_VISIBILITY_GM, 1);
					duel->initiator->GetPet()->m_serverSideVisibilityDetect.SetValue(SERVERSIDE_VISIBILITY_GM, 1);
					duel->initiator->GetPet()->SetPhaseMask(3, true);
				}

				duel->opponent->m_serverSideVisibility.SetValue(SERVERSIDE_VISIBILITY_GM, 1);
				duel->opponent->m_serverSideVisibilityDetect.SetValue(SERVERSIDE_VISIBILITY_GM, 1);
				duel->opponent->SetPhaseMask(3, true);
				if(duel->opponent->GetPet())
				{
					duel->opponent->GetPet()->m_serverSideVisibility.SetValue(SERVERSIDE_VISIBILITY_GM, 1);
					duel->opponent->GetPet()->m_serverSideVisibilityDetect.SetValue(SERVERSIDE_VISIBILITY_GM, 1);
					duel->opponent->GetPet()->SetPhaseMask(3, true);
				}
			}
			else
			{
				duel->initiator->m_serverSideVisibility.SetValue(SERVERSIDE_VISIBILITY_GM, SEC_PLAYER);
				duel->initiator->m_serverSideVisibilityDetect.SetValue(SERVERSIDE_VISIBILITY_GM, SEC_PLAYER);
				duel->initiator->SetPhaseMask(3, true);
				if(duel->initiator->GetPet())
				{
					duel->initiator->GetPet()->m_serverSideVisibility.SetValue(SERVERSIDE_VISIBILITY_GM, SEC_PLAYER);
					duel->initiator->GetPet()->m_serverSideVisibilityDetect.SetValue(SERVERSIDE_VISIBILITY_GM, SEC_PLAYER);
					duel->initiator->GetPet()->SetPhaseMask(3, true);
				}

				duel->opponent->m_serverSideVisibility.SetValue(SERVERSIDE_VISIBILITY_GM, SEC_PLAYER);
				duel->opponent->m_serverSideVisibilityDetect.SetValue(SERVERSIDE_VISIBILITY_GM, SEC_PLAYER);
				duel->opponent->SetPhaseMask(3, true);
				if(duel->opponent->GetPet())
				{
					duel->opponent->GetPet()->m_serverSideVisibility.SetValue(SERVERSIDE_VISIBILITY_GM, SEC_PLAYER);
					duel->opponent->GetPet()->m_serverSideVisibilityDetect.SetValue(SERVERSIDE_VISIBILITY_GM, SEC_PLAYER);
					duel->opponent->GetPet()->SetPhaseMask(3, true);
				}
			}*/
            break;
...
void Player::ApplyEquipSpell(SpellEntry const* spellInfo, Item* item, bool apply, bool form_change)
{
    if(apply)
    {
        ...
        sLog->outDebug(DebugLogFilters(LOG_FILTER_SPELLS_AURAS | LOG_FILTER_PLAYER_ITEMS), "WORLD: cast %s Equip spellId - %i", (item ? "item" : "itemset"), spellInfo->Id);
        ...
        if(go->getLootState() == GO_READY)
        {
            ...
            if((go->GetEntry() == BG_AV_OBJECTID_MINE_N || go->GetEntry() == BG_AV_OBJECTID_MINE_S))
                if(Battleground* pBG = GetBattleground())
                {
                    ...
                    {
                        if(!(((BattlegroundAV*)pBG)->PlayerCanDoMineQuest(go->GetEntry(), GetTeam())))
                        {
                            SendLootRelease(guid);
                            return;
                        }
...
void Player::SendInitWorldStates(uint32 zoneid, uint32 areaid)
{
    ...
    Battleground* pBG = GetBattleground();
    ...
    switch(zoneid)
    {
        ...
        case 2597:                                          // Alterac Valley
            if(pBG && pBG->GetTypeID(true) == BATTLEGROUND_AV)
                pBG->FillInitialWorldStates(data);
            else
            {
        ...
        case 3277:                                          // Warsong Gulch
            if(pBG && pBG->GetTypeID(true) == BATTLEGROUND_WS)
                pBG->FillInitialWorldStates(data);
            else
            {
        ...
        case 3358:                                          // Arathi Basin
            if(pBG && pBG->GetTypeID(true) == BATTLEGROUND_AB)
                pBG->FillInitialWorldStates(data);
            else
            {
        ...
        case 3820:                                          // Eye of the Storm
            if(pBG && pBG->GetTypeID(true) == BATTLEGROUND_EY)
                pBG->FillInitialWorldStates(data);
            else
            {
        ...
        case 3698:                                          // Nagrand Arena
            if(pBG && pBG->GetTypeID(true) == BATTLEGROUND_NA)
                pBG->FillInitialWorldStates(data);
            else
            {
        ...
        case 3702:                                          // Blade's Edge Arena
            if(pBG && pBG->GetTypeID(true) == BATTLEGROUND_BE)
                pBG->FillInitialWorldStates(data);
            else
            {
        ...
        case 3968:                                          // Ruins of Lordaeron
            if(pBG && pBG->GetTypeID(true) == BATTLEGROUND_RL)
                pBG->FillInitialWorldStates(data);
            else
            {
        ...
        case 4378:                                          // Dalaran Sewers
            if(pBG && pBG->GetTypeID(true) == BATTLEGROUND_DS)
                pBG->FillInitialWorldStates(data);
            else
            {
        ...
        case 4384:                                          // Strand of the Ancients
            if(pBG && pBG->GetTypeID(true) == BATTLEGROUND_SA)
                pBG->FillInitialWorldStates(data);
            else
            {
        ...
        case 4406:                                          // Ring of Valor
            if(pBG && pBG->GetTypeID(true) == BATTLEGROUND_RV)
                pBG->FillInitialWorldStates(data);
            else
            {
            ...
            break;
        case 4710:
            if(pBG && pBG->GetTypeID(true) == BATTLEGROUND_IC)
                pBG->FillInitialWorldStates(data);
            else
            {
...
{
    if(HasFlag(PLAYER_FLAGS, PLAYER_ALLOW_ONLY_ABILITY) && !HasAura(46924))
       return EQUIP_ERR_CANT_DO_RIGHT_NOW;

	if(isSpectator())
	{
		char* msg = "|CFF87CEFANo puedes equiparte objetos siendo un espectador|R";
	...
	if((pItem->GetEntry() == 1404 || pItem->GetEntry() == 13209) && !(GetMapId() == 530 && GetZoneId() == 3430 && GetAreaId() == 3431))
	{
		char* msg = "|CFF87CEFANecesitas estar en la Shop VIP para poder equiparte ese objeto|R";
		WorldPacket data(SMSG_NOTIFICATION, 255);
		data << msg;
		GetSession()->SendPacket(&data);
		return EQUIP_ERR_NONE;
        ...
        if(pProto)
        {
            ...
            if(not_loading)
            {
                ...
                if(!pProto->CanChangeEquipStateInCombat())
                {
                    ...
                    if(Battleground* pBG = GetBattleground())
                        if(pBG->isArena() && pBG->GetStatus() == STATUS_IN_PROGRESS)
			...
			//VIP solo se puede equipar en ranura 1, mas facil de quitarselo al irse
			if(pItem->GetEntry() == 1404 && eslot != EQUIPMENT_SLOT_TRINKET1)
			{
				char* msg = "|CFF87CEFAEse objeto solo se puede equipar en la primera ranura de los Abalorios|R";
				WorldPacket data(SMSG_NOTIFICATION, 255);
				data << msg;
				GetSession()->SendPacket(&data);
				return EQUIP_ERR_NONE;
			...
			//VIP solo se puede equipar en ranura 1, mas facil de quitarselo al irse
			if(pItem->GetEntry() == 13209 && eslot != EQUIPMENT_SLOT_TRINKET1)
			{
				char* msg = "|CFF87CEFAEse objeto solo se puede equipar en la primera ranura de los Abalorios|R";
				WorldPacket data(SMSG_NOTIFICATION, 255);
				data << msg;
...
InventoryResult Player::CanUseItem(ItemTemplate const *pProto) const
{
    ...
    if(pProto)
    {
		//Evitar usar la Pocion de Primera Sangre fuera de BG (por si ya la tienen por lo que sea)
		if (pProto->ItemId == 37661 && !InBattleground())
		{
			char* msg = "|CFF87CEFANecesitas estar un Campo de Batalla para usar este objeto|R";
			WorldPacket data(SMSG_NOTIFICATION, 255);
			data << msg;
			GetSession()->SendPacket(&data);
			return EQUIP_ERR_NONE;
...
void Player::PrepareGossipMenu(WorldObject* source, uint32 menuId /*= 0*/, bool showQuests /*= false*/)
{
    ...
    for(GossipMenuItemsMap::const_iterator itr = menuItemBounds.first; itr != menuItemBounds.second; ++itr)
    {
        ...
        if(source->GetTypeId() == TYPEID_UNIT)
        {
            ...
            switch(itr->second.OptionType)
            {
                ...
                case GOSSIP_OPTION_VENDOR:
                {
					...
					// Multivendor
					VendorItemData const* vendorItems = itr->second.ActionMenuId ? sObjectMgr->GetNpcVendorItemList(itr->second.ActionMenuId) : pCreature->GetVendorItems();
        ...
        {
            GameObject* pGO = source->ToGameObject();
...
#define RelocateToHomebind(){ mapId = m_homebindMapId; instanceId = 0; Relocate(m_homebindX, m_homebindY, m_homebindZ); }
    ...
    for(uint32 arena_slot = 0; arena_slot < MAX_ARENA_SLOT; ++arena_slot)
    {
        ...
        if(ArenaTeam* pAT = sArenaTeamMgr->GetArenaTeamById(arena_team_id))
            if(pAT->IsMember(GetGUID()))
                continue;
...
void Player::_LoadInventory(PreparedQueryResult result, uint32 timeDiff)
{
    ...
    if(result)
    {
        ...
        while(!problematicItems.empty())
        {
            ...
            MailDraft draft(subject, "Hay problemas equipando este/estos objeto/objetos.");
...
bool Player::_LoadHomeBind(PreparedQueryResult result)
{
    ...
    sLog->outDebug(DebugLogFilters(LOG_FILTER_UNITS | LOG_FILTER_MAPS), "Setting player home position - mapid: %u, areaid: %u, X: %f, Y: %f, Z: %f",
...
void Player::SaveToDB()
{
    ...
    ss << (IsInWorld() && !GetSession()->PlayerLogout() ? 1 : 0) << ", ";
...
void Player::_SaveQuestStatus(SQLTransaction& trans)
{
    ...
    for(saveItr = m_QuestStatusSave.begin(); saveItr != m_QuestStatusSave.end(); ++saveItr)
    {
        if(saveItr->second)
        {
            ...
            {
					trans->PAppend("REPLACE INTO character_queststatus (guid, quest, status, explored, timer, mobcount1, mobcount2, mobcount3, mobcount4, itemcount1, itemcount2, itemcount3, itemcount4, itemcount5, itemcount6, playercount) "
                    "VALUES ('%u', '%u', '%u', '%u', '" UI64FMTD "', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u', '%u')",
                    GetGUIDLow(), statusItr->first, statusItr->second.m_status, statusItr->second.m_explored, uint64(statusItr->second.m_timer / IN_MILLISECONDS+ sWorld->GetGameTime()), statusItr->second.m_creatureOrGOcount[0], statusItr->second.m_creatureOrGOcount[1], statusItr->second.m_creatureOrGOcount[2], statusItr->second.m_creatureOrGOcount[3], statusItr->second.m_itemcount[0], statusItr->second.m_itemcount[1], statusItr->second.m_itemcount[2], statusItr->second.m_itemcount[3], statusItr->second.m_itemcount[4], statusItr->second.m_itemcount[5], statusItr->second.m_playercount);
            }
        }
        else
...
{
    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_LOAD_CHAR_PLAYERBYTES2);
    ...
    stmt = CharacterDatabase.GetPreparedStatement(CHAR_UPDATE_GENDER_PLAYERBYTES);
...
bool Player::BuyItemFromVendorSlot(uint64 vendorguid, uint32 vendorslot, uint32 item, uint8 count, uint8 bag, uint8 slot)
{
    ...
    if(pProto->RequiredReputationFaction && (uint32(GetReputationRank(pProto->RequiredReputationFaction)) < pProto->RequiredReputationRank))
    {
        SendBuyError(BUY_ERR_REPUTATION_REQUIRE, NULL, item, 0);
        return false;
    }
...
uint32 Player::GetMaxPersonalArenaRatingRequirement(uint32 minarenaslot) const
{
    ...
    for(uint8 i = minarenaslot; i < MAX_ARENA_SLOT; ++i)
    {
        ...
        if(ArenaTeam* pAT = sArenaTeamMgr->GetArenaTeamById(GetArenaTeamId(i)))
        {
            uint32 p_rating = GetArenaPersonalRating(i);
            uint32 t_rating = pAT->GetRating();
...
bool Player::IsSpellAffectedByMixology(SpellEntry const *spellProto) const
{
    return (HasSkill(SKILL_ALCHEMY) && spellProto->SpellFamilyName == SPELLFAMILY_POTION && (
        ...
        HasSpell(spellProto->EffectTriggerSpell[0]));
}
...
void Player::LeaveBattleground(bool teleportToEntryPoint)
{
    if(Battleground* pBG = GetBattleground())
    {
        pBG->RemovePlayerAtLeave(GetGUID(), teleportToEntryPoint, true);

        // call after remove to be sure that player resurrected for correct cast
        if(pBG->isBattleground() && !isGameMaster() && sWorld->getBoolConfig(CONFIG_BATTLEGROUND_CAST_DESERTER))
        {
            if(pBG->GetStatus() == STATUS_IN_PROGRESS || pBG->GetStatus() == STATUS_WAIT_JOIN)
            {
...
void Player::ReportedAfkBy(Player* reporter)
{
    ...
    if(!pBG || pBG != reporter->GetBattleground() || GetTeam() != reporter->GetTeam() || pBG->GetStatus() != STATUS_IN_PROGRESS)
        return;
...
bool Player::canSeeAlways(WorldObject const* pWO) const
{
    if(Unit::canSeeAlways(pWO))
        return true;
...
bool Player::isAlwaysDetectableFor(WorldObject const* pSeer) const
{
    if(Unit::isAlwaysDetectableFor(pSeer))
        return true;

    if(const Player* seerPlayer = pSeer->ToPlayer())
            ...
            for(Unit::VisibleAuraMap::const_iterator itr = visibleAuras->begin(); itr != visibleAuras->end(); ++itr)
            {
                ...
                if(Aura* aura = auraApp->GetBase())
                {
                    ...
                    msg.CreateAura(casterID, aura->GetSpellProto()->Id,
                                    auraApp->IsPositive(), aura->GetSpellProto()->Dispel,
...
bool Player::GetBGAccessByLevel(BattlegroundTypeId bgTypeId) const
{
    ...
    Battleground* pBG = sBattlegroundMgr->GetBattlegroundTemplate(bgTypeId);
    if(!pBG)
        return false;
    ...
    if(level < pBG->GetMinLevel() || level > pBG->GetMaxLevel())
        return false;
...
void Player::SummonIfPossible(bool agree)
{
    ...
    if(Battleground* pBG = GetBattleground())
        pBG->EventPlayerDroppedFlag(this);
...
void Player::AutoUnequipOffhandIfNeed(bool force /*= false*/)
{
    ...
    else
    {
        ...
        std::string subject = GetSession()->GetTrinityString(LANG_NOT_EQUIPPED_ITEM);
        MailDraft(subject, "Hay varios problemas equipando uno o varios objetos.").AddItem(offItem).SendMailTo(trans, this, MailSender(this, MAIL_STATIONERY_GM), MAIL_CHECK_MASK_COPIED);
...
void Player::UpdateCharmedAI()
{
  ...
  Creature* pCreature = GetCharmer()->ToCreature();
    ...
    if(pCreature->IsInEvadeMode())
    {
        ...
        {
            if((*iter)->GetCasterGUID() == pCreature->GetGUID() && (*iter)->GetBase()->IsPermanent())
            {
                pCreature->DealDamage(this, GetHealth(), NULL, DIRECT_DAMAGE, SPELL_SCHOOL_MASK_NORMAL, NULL, false);
                return;
            }
        }
    }
    ...
    Unit* pTarget = getVictim();
    if(!pTarget || !pCreature->canAttack(pTarget))
    {
        pTarget = pCreature->SelectNearestTarget();
        if(!pTarget)
            return;
...
void Player::StoreLootItem(uint8 lootSlot, Loot* loot)
{
    ...
    if(msg == EQUIP_ERR_OK)
    {
        ...
        item->AddAlreadyLootedPlayer(this);
...
void Player::HandleFall(MovementInfo const& movementInfo)
{
        ...
        !HasAuraType(SPELL_AURA_FLY) && !IsImmunedToDamage(SPELL_SCHOOL_MASK_NORMAL))
    {
        ...
        {
            uint32 damage;
			//Custom para el evento de escalada
			if (GetZoneId() == 267 || GetZoneId() == 45 || GetZoneId() == 2037 || GetZoneId() == 214)
				damage = 0;
			else
            ...
            sLog->outDebug(LOG_FILTER_UNITS, "FALLDAMAGE z=%f sz=%f pZ=%f FallTime=%d mZ=%f damage=%d SF=%d", movementInfo.pos.GetPositionZ(), height, GetPositionZ(), movementInfo.fallTime, height, damage, safe_fall);
        }
    }
...
void Player::LearnTalent(uint32 talentId, uint32 talentRank)
{
	if(InBattlegroundQueueForBattlegroundQueueType(BATTLEGROUND_QUEUE_5v5))
	{
		ChatHandler(this).PSendSysMessage("|CFFFF0000No puedes gastar puntos de talentos mientras estás en cola de Arenas 1v1.|r");
		return;
	}
...
void Player::ActivateSpec(uint8 spec)
{
	if(InBattlegroundQueueForBattlegroundQueueType(BATTLEGROUND_QUEUE_5v5))
	{
		ChatHandler(this).PSendSysMessage("|CFFFF0000No puedes cambiarte la spec mientras estás en cola de Arenas 1v1.|r");
		return;
	}
...
void Player::SetSelection(const uint64 &guid)
{
    m_curSelection = guid;
...
void Player::FitPlayerInTeam(bool action)
{
    ...
    if (GetBattleground() && action)
        ChatHandler(GetSession()).PSendSysMessage("%sAhora mismo est""\xC3\xA1""s jugando por la %s%s en %s", MSG_COLOR_WHITE, GetBGTeam() == ALLIANCE ? MSG_COLOR_DARKBLUE"Alianza" : MSG_COLOR_RED"Horda", MSG_COLOR_WHITE, GetBattleground()->GetName());
}
...
{
    sLog->outDebug(LOG_FILTER_BATTLEGROUND, "BattleGroundQueue::CheckCrossFactionMatch");
    ...
    {
        sLog->outDebug(LOG_FILTER_BATTLEGROUND, "Not enough players. Has: %u Need: %u", m_SelectionPools[TEAM_ALLIANCE].GetPlayerCount() + m_SelectionPools[TEAM_HORDE].GetPlayerCount(), MPT * 2);
        return false;
    }
    return true;
}
    ...
    if (sWorld->getBoolConfig(CONFIG_CROSSFACTIONBG))
    {
        ...
        for (GroupsQueueType::iterator itr = m_QueuedGroups[bracket_id][BG_QUEUE_MIXED].begin(); itr != m_QueuedGroups[bracket_id][BG_QUEUE_MIXED].end(); ++itr)
        {
            ...
            if (m_SelectionPools[alliance ? TEAM_ALLIANCE : TEAM_HORDE].AddGroup((*itr), alliance ? aliFree : hordeFree))
            {
                ...
                sLog->outDebug(LOG_FILTER_BATTLEGROUND, "Size: %u", GSize);
