        ...
        default:
        {
            switch(GetEntry())
            {
                ...
                case 29264: // Feral Spirit
                {
                    ...
                    SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, float(petlevel * 6.4 - petlevel));
                    SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, float(petlevel * 6.4 + petlevel));
                ...
                case 27829: // Ebon Gargoyle
                {
                    ...
                    {
                        SetCreateMana(28 + 10*petlevel);
                        SetCreateHealth(28 + 25*petlevel);
                    }
                    if(Player* pSummoner = m_owner->ToPlayer()) // get 100% of owning player's physical (melee) haste
                    {
                        ...
                        SetCreateHealth(uint32(pSummoner->GetMaxHealth()*0.5f)); // hp must be 0.8x of DK hp
                    }
                    SetBonusDamage(int32(m_owner->GetTotalAttackPowerValue(BASE_ATTACK) * 0.5f)); // attack power coefficient is 0.4
                    SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, float(petlevel - (petlevel / 4)));
                    SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, float(petlevel + (petlevel / 4)));
    ...
    switch(getPetType())
    {
        ...
        case HUNTER_PET:
            ...
            /*else if(getLevel() + 5 < owner->getLevel())
                GivePetLevel(owner->getLevel() - 5);*/ //Custom, al domesticar la pet, nivel maximo
            break;
        default:
            break;
    }
