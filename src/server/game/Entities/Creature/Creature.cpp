...
{
	/*if (GetDBTableGUIDLow() == 1330370)	//GUID de la criatura a la que afectará
	{
		if(GetAreaId() != 368)	//ID del área de la cual no pueden salir
		{
			uint32 lowguid = GetDBTableGUIDLow();
			///Recordar que si se cambian las coordenadas las "comas" ponerlas con puntos, si no, dará error
			float x =	-1141.11f;	//Coordenada X, cambiar los numeros dejando la "f"
			float y =	-5072.19f;	//Coordenada Y, cambiar los numeros dejando la "f"
			float z =	1.7f;		//Coordenada Z, cambiar los numeros dejando la "f"
			float o =	1.09f;		//Coordenada o (Orientación), cambiar los numeros dejando la "f"
...
void Creature::Respawn(bool force)
{
    ...
    if(getDeathState() == DEAD)
    {
        ...
        sLog->outDebug(LOG_FILTER_UNITS, "Respawning...");
        m_respawnTime = 0;
...
bool Creature::checkCanReachTargetAndRemoveIfNot(Unit* pTarget)
{
    if(!pTarget)
        return false;
    ...
    if(!GetMotionMaster()->top()->IsReachable())
    {
        ...
        {
            getHostileRefManager().deleteReference(pTarget);
            m_ThreatManager.modifyThreatPercent(pTarget, -101);
