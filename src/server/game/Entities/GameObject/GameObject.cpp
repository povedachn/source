...
bool GameObject::Create(uint32 guidlow, uint32 name_id, Map *map, uint32 phaseMask, float x, float y, float z, float ang, float rotation0, float rotation1, float rotation2, float rotation3, uint32 animprogress, GOState go_state, uint32 artKit)
{
    ...
    //SetGoState(go_state); // CHECK NEED CHECK, POSSIBLE GO ISSUES! NEED HELP, WAIT vGaming
                    ...
                    if(ok)
                    {
                        ...
                        if(IsBattlegroundTrap && ok->GetTypeId() == TYPEID_PLAYER)
                        {
                            ...
                            if(ok->ToPlayer()->InBattleground())
                                if(Battleground* pBG = ok->ToPlayer()->GetBattleground())
                                    pBG->HandleTriggerBuff(GetGUID());
                        }
                    }
                }
        ...
        case GO_JUST_DEACTIVATED:
        {
            ...
            if(GetGoType() == GAMEOBJECT_TYPE_GOOBER)
            {
                ...
                if(spellId)
                {
                    ...
                    for(std::set<uint32>::const_iterator it = m_unique_users.begin(); it != m_unique_users.end(); ++it)
                    {
                        if(Player* pOwner = ObjectAccessor::GetPlayer(*this, *it))
                            pOwner->CastSpell(pOwner, spellId, false);
                    }
...
bool GameObject::isAlwaysVisibleFor(WorldObject const* seer) const
{
    ...
    if(uint64 guid = GetOwnerGUID())
    {
        ...
        if(pOwner && seer->isType(TYPEMASK_UNIT) && pOwner->IsFriendlyTo(((Unit*)seer)))
            return true;
    }
        ...
        case GAMEOBJECT_TYPE_CHEST:
        {
            if(LootTemplates_Gameobject.HaveQuestLootForPlayer(GetGOInfo()->GetLootId(), pTarget))
            {
                ...
                if(GetEntry() == BG_AV_OBJECTID_MINE_N || GetEntry() == BG_AV_OBJECTID_MINE_S)
                    if(Battleground* pBG = pTarget->GetBattleground())
                        if(pBG->GetTypeID(true) == BATTLEGROUND_AV && !(((BattlegroundAV*)pBG)->PlayerCanDoMineQuest(GetEntry(), pTarget->GetTeam())))
                            return false;
                return true;
            }
            break;
        ...
        case GAMEOBJECT_TYPE_GOOBER:                        //10
        {
            ...
            if(user->GetTypeId() == TYPEID_PLAYER)
            {
                ...
                if(Battleground* pBG = player->GetBattleground())
                    pBG->EventPlayerUsedGO(player, this);
        ...
        case GAMEOBJECT_TYPE_FISHINGNODE:                   //17
        {
            ...
            switch(getLootState())
            {
                case GO_READY:                              // ready for loot
                {
                    ...
                    sLog->outDebug(LOG_FILTER_PLAYER_SKILLS, "Fishing check (skill: %i zone min skill: %i chance %i roll: %i", skill, zone_skill, chance, roll);
        ...
        case GAMEOBJECT_TYPE_FLAGSTAND:                     // 24
        {
            ...
            if(player->CanUseBattlegroundObject())
            {
                ...
                if(Battleground* pBG = player->GetBattleground())
                {
                    ...
                    // 15004
                    // 15005
                    pBG->EventPlayerClickedOnFlag(player, this);
                }
...
void GameObject::SetDestructibleState(GameObjectDestructibleState state, Player* eventInvoker /*= NULL*/, bool setHealth /*= false*/)
{
    ...
    switch(state)
    {
        ...
        case GO_DESTRUCTIBLE_DAMAGED:
        {
            ...
            if(eventInvoker)
                if(Battleground* pBG = eventInvoker->GetBattleground())
                    pBG->EventPlayerDamagedGO(eventInvoker, this, m_goInfo->building.damagedEvent);
        ...
        case GO_DESTRUCTIBLE_DESTROYED:
        {
            ...
            if(eventInvoker)
            {
                if(Battleground* pBG = eventInvoker->GetBattleground())
                {
                    pBG->EventPlayerDamagedGO(eventInvoker, this, m_goInfo->building.destroyedEvent);
                    pBG->DestroyGate(eventInvoker, this);
                }
            }
