...
/*uint32 Seleccionar_Id_Cuenta()
{
	WorldSession* session = GetSession();
	Player* player;
	QueryResult Seleccionar_Id_Cuenta = LoginDatabase.PQuery("SELECT id FROM account WHERE username = '%u'", player->GetName());

	if (!Seleccionar_Id_Cuenta)
		return 0;
	else
	{
		Field* fields = Seleccionar_Id_Cuenta->Fetch();
			return fields[0].GetUInt32();
	}
}*/

/*uint32 Seleccionar_Nivel_Cuenta()
{
	QueryResult Seleccionar_Nivel_Cuenta = LoginDatabase.PQuery("SELECT gmlevel FROM account_access WHERE id = '%u'", Seleccionar_Id_Cuenta());

	if (!Seleccionar_Nivel_Cuenta)
		return 0;
	else
	{
		Field* fields = Seleccionar_Nivel_Cuenta->Fetch();
			return fields[0].GetUInt32();
	}
}*/
...
bool ChatHandler::HandleNameAnnounceCommand(const char* args)
{
	...
	QueryResult Seleccionar_Id_Cuenta = CharacterDatabase.PQuery("SELECT account FROM characters WHERE NAME = '%s'", name.c_str());
	Field* fields = Seleccionar_Id_Cuenta->Fetch();
	uint32 Seleccionar_Id_Cuenta_valor = fields[0].GetUInt32();

	QueryResult Seleccionar_Nivel_Cuenta = LoginDatabase.PQuery("SELECT gmlevel FROM account_access WHERE id = '%u'", Seleccionar_Id_Cuenta_valor);
	Field* fields2 = Seleccionar_Nivel_Cuenta->Fetch();
	uint32 Seleccionar_Nivel_Cuenta_valor = fields2[0].GetUInt32();
	
	char* gm;
	if (Seleccionar_Nivel_Cuenta_valor == 2)
		gm = "GM - Rango 2";
	else if (Seleccionar_Nivel_Cuenta_valor == 3)
		gm = "GM - Rango 3";
	else if (Seleccionar_Nivel_Cuenta_valor == 4)
		gm = "GM - Rango 4";
	else if (Seleccionar_Nivel_Cuenta_valor == 5)
		gm = "GM - Rango 5";	
	else if (Seleccionar_Nivel_Cuenta_valor == 6)
		gm = "GM - Rango 6";
	else if (Seleccionar_Nivel_Cuenta_valor == 7)
		gm = "Sub-Admin";			
	else
		gm = "Administrador";

    sWorld->SendWorldText(LANG_ANNOUNCE_COLOR, gm, name.c_str(), args);
    return true;
}
...
bool ChatHandler::HandleGMNameAnnounceCommand(const char* args)
{
    ...
    sWorld->SendGMText(LANG_GM_ANNOUNCE_COLOR, gm, name.c_str(), args);
    return true;
}
