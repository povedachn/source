...
{
    sLog->outString( "PELIGRO: Reiniciando gameobject_template" );
    sObjectMgr->LoadGameObjectTemplate();
    SendGlobalSysMessage("PELIGRO: Tabla `gameobject_template` reiniciada.");
    return true;
}

bool ChatHandler::HandleReloadItemTemplateCommand(const char*)
{
    sLog->outString( "PELIGRO: Reiniciando item_template" );
    sObjectMgr->LoadItemTemplates();
    SendGlobalSysMessage("PELIGRO: Tabla `item_template` reiniciada.");
    return true;
}

bool ChatHandler::HandleReloadCreatureTemplateCommand(const char*)
{
    sLog->outString( "PELIGRO: Reiniciando creature_template" );
    sObjectMgr->LoadCreatureTemplates();
    SendGlobalSysMessage("PELIGRO: Tabla `creature_template` reiniciada.");
    return true;
}

bool ChatHandler::HandleBGStartCommand(const char*)
{
    if(m_session->GetPlayer() && m_session->GetPlayer()->GetBattleground())
        m_session->GetPlayer()->GetBattleground()->SetStartDelayTime(BG_START_DELAY_NONE);
 
    PSendSysMessage("BG/Arena empezada");
 
    return true;
}

bool ChatHandler::HandleReloadBattlegroundTemplateCommand(const char*)
{
    sLog->outString( "PELIGRO: Reiniciando battleground_template" );
    sBattlegroundMgr->CreateInitialBattlegrounds();
    SendGlobalSysMessage("PELIGRO: Tabla `battleground_template` reiniciada.");
    return true;
}
