...
bool AchievementCriteriaData::Meets(uint32 criteria_id, Player const* source, Unit const* target, uint32 miscvalue1 /*= 0*/) const
{
    switch(dataType)
    {
        ...
        case ACHIEVEMENT_CRITERIA_DATA_TYPE_BG_LOSS_TEAM_SCORE:
        {
            Battleground* pBG = source->GetBattleground();
            if(!pBG)
                return false;
            return pBG->IsTeamScoreInRange(source->GetTeam() == ALLIANCE ? HORDE : ALLIANCE, bg_loss_team_score.min_score, bg_loss_team_score.max_score);
        }
        case ACHIEVEMENT_CRITERIA_DATA_INSTANCE_SCRIPT:
        {
            if(!source->IsInWorld())
        ...
        switch(type)
        {
            ...
            case ACHIEVEMENT_CRITERIA_TYPE_DEATH:
            {
                ...
                for(int j = 0; j < MAX_ARENA_SLOT; ++j)
                {
                    ...
                    {
                        Battleground* pBG = GetPlayer()->GetBattleground();
                        if(!pBG || !pBG->isArena() || ArenaTeam::GetSlotByType(pBG->GetArenaType()) != j)
                            notfit = true;
