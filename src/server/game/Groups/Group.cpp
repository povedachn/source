...
bool Group::ChangeLeader(const uint64 &guid)
{
    member_witerator slot = _getMemberWSlot(guid);
...
void Group::BroadcastGroupUpdate(void)
{
    ...
    for(member_citerator citr = m_memberSlots.begin(); citr != m_memberSlots.end(); ++citr)
    {
        ...
        if(pp && pp->IsInWorld())
        {
            ...
            sLog->outDebug(LOG_FILTER_UNITS, "-- Forced group value update for '%s'", pp->GetName());
        }
    }
}
...
void Group::SetBattlegroundGroup(Battleground* pBG)
{
    m_bgGroup = pBG;
}
