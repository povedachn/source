...
m_Warden(NULL), timeLastWhoCommand(0), m_CurrentVendor(0)
{
    if(sock)
    {
            ...
            try
            {
                switch(opHandle.status)
                {
                    ...
                    case STATUS_AUTHED:
						...
						//Posible fix para el script que tira servers
						/*if(packet->GetOpcode() == CMSG_CHAR_ENUM)
						{
							if(m_playerRecentlyLogout == false)
...
void WorldSession::LogoutPlayer(bool Save)
{
    ...
    if(_player)
    {
        ...
        else if(!_player->getAttackers().empty())
        {
            ...
            if(!aset.empty())
                if(Battleground* pBG = _player->GetBattleground())
                    pBG->HandleKillPlayer(_player, *aset.begin());
        }
        ...
        if(Battleground* pBG = _player->GetBattleground())
            pBG->EventPlayerLoggedOut(_player);
