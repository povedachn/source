...
void WorldSession::HandleMessagechatOpcode(WorldPacket &recv_data)
{
	...
	//Quitado porque causa problemas y no puedes escribir en el chat los comandos
    ...
    {
		//Custom - LANG_UNIVERSAL en Zona de Duelos y Zona de Monturas y Shop
		if(_player->GetZoneId() == 3519 || (_player->GetAreaId() == 19 && _player->GetZoneId() == 33) ||  (_player->GetMapId() == 617 && _player->GetAreaId() == 4378 && _player->GetZoneId() == 4378) || (_player->GetMapId() == 532 && _player->GetAreaId() == 3457 && _player->GetZoneId() == 3457) && !sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_CHAT))
			lang = LANG_UNIVERSAL;
    ...
    if (sender->isGMChat() && sWorld->getBoolConfig(CONFIG_GM_CHAT_COLOR_HABILITADO))
		// |cff2998ff = Azul
        msg = "|cff2998ff" + msg + "|r";
		
	if(_player->GetSession()->GetSecurity() == SEC_VIP)
		msg = "|TInterface\\CHATFRAME\\UI-ChatWhisperIcon:15:15:0:-1|t|CFFC0C0C0[Premium]|r: " + msg;
        ...
        case CHAT_MSG_WHISPER:
        {
            ...
            if(!player || (tSecurity == SEC_PLAYER && pSecurity > SEC_PLAYER && !player->isAcceptWhispers()))
            {
                ...
                if (sWorld->getBoolConfig(CONFIG_FAKE_WHO_LIST))
                {
                    ...
                    if (result)
                    {
                        ChatHandler(this).SendSysMessage("El jugador desea no ser molestado y no puede recibir mensajes privados: Ocupado");
                        return;
                    }
                }
                SendPlayerNotFoundNotice(to);
...
{
    if(!GetPlayer()->isAlive() || GetPlayer()->HasUnitState(UNIT_STAT_DIED) || GetPlayer()->isSpectator())
        return;
