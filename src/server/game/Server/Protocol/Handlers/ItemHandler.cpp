...
void WorldSession::HandleSellItemOpcode(WorldPacket &recv_data)
{
    ...
    if(pItem)
    {
        ...
        {
            _player->SendSellError(SELL_ERR_CANT_SELL_ITEM, NULL, itemguid, 0);
            return;
        }
    ...
    _player->SendSellError(SELL_ERR_CANT_FIND_ITEM, NULL, itemguid, 0);
    return;
}
...
void WorldSession::HandleBuybackItem(WorldPacket &recv_data)
{
    ...
    if(pItem)
    {
        ...
        {
            _player->SendBuyError(BUY_ERR_NOT_ENOUGHT_MONEY, NULL, pItem->GetEntry(), 0);
            return;
        }
        ...
        _player->SendBuyError(BUY_ERR_CANT_FIND_ITEM, NULL, 0, 0);
}
...
void WorldSession::SendListInventory(uint64 vendorGuid, uint32 vendorEntry)
{
	...
	if(!vendor && !itemk)
    {
        sLog->outDebug(LOG_FILTER_NETWORKIO, "WORLD: SendListInventory - Unit\Item (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(vendorGuid)));
        _player->SendSellError(SELL_ERR_CANT_FIND_VENDOR, NULL, 0, 0);
        return;
    }
    ...
    if(vendor && vendor->HasUnitState(UNIT_STAT_MOVING))
        vendor->StopMoving();
