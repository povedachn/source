...
void WorldSession::HandleWhoOpcode(WorldPacket &recv_data)
{
    ...
    for(HashMapHolder<Player>::MapType::const_iterator itr = m.begin(); itr != m.end(); ++itr)
    {
		...
		//Custom, principio
		if(itr->second->isGameMaster() && itr->second->GetSession()->GetSecurity() > 1)
		{
    ...
    if (sWorld->getBoolConfig(CONFIG_FAKE_WHO_LIST) && matchcount < 49)
    {
        ...
        if (result)
        {
            do
            {
                ...
                if (Player::TeamForRace(uint8(race)) != team)
                    continue;
...
void WorldSession::HandleAreaTriggerOpcode(WorldPacket &recv_data)
{
    ...
    {
        if(Battleground* pBG = GetPlayer()->GetBattleground())
            if(pBG->GetStatus() == STATUS_IN_PROGRESS)
                pBG->HandleAreaTrigger(GetPlayer(), Trigger_ID);

        return;
    }

    if(OutdoorPvP* pPVP = GetPlayer()->GetOutdoorPvP())
    {
        if(pPVP->HandleAreaTrigger(_player, Trigger_ID))
            return;
    }

    // NULL if all values default (non teleport trigger)
    AreaTrigger const* pATeam = sObjectMgr->GetAreaTrigger(Trigger_ID);
    if(!pATeam)
        return;
    ...
    if(GetPlayer()->GetMapId() != pATeam->target_mapId && !sMapMgr->CanPlayerEnter(pATeam->target_mapId, GetPlayer(), false))
        return;

    // Check if we are in LfgGroup and trying to get out the dungeon
    if(GetPlayer()->GetGroup() && GetPlayer()->GetGroup()->isLFGGroup() && GetPlayer()->GetMap()->IsDungeon() && pATeam->target_mapId != GetPlayer()->GetMapId())
        GetPlayer()->TeleportToBGEntryPoint();
    else
        GetPlayer()->TeleportTo(pATeam->target_mapId, pATeam->target_X, pATeam->target_Y, pATeam->target_Z, pATeam->target_Orientation, TELE_TO_NOT_LEAVE_TRANSPORT);
}
...
void WorldSession::HandleInspectOpcode(WorldPacket &recv_data)
{
    uint64 guid;
    recv_data >> guid;
    sLog->outDebug(LOG_FILTER_NETWORKIO, "Inspected guid is " UI64FMTD, guid);
...
void WorldSession::HandleWorldTeleportOpcode(WorldPacket &recv_data)
{
    ...
    sLog->outDebug(LOG_FILTER_NETWORKIO, "Time %u sec, map=%u, x=%f, y=%f, z=%f, orient=%f", time/1000, mapid, PositionX, PositionY, PositionZ, Orientation);
