...
void WorldSession::HandleMoveWorldportAckOpcode()
{
    ...
    if(_player->InBattleground())
    {
        ...
        // join to bg case
        else if(Battleground* pBG = _player->GetBattleground())
        {
            if(_player->IsInvitedForBattlegroundInstance(_player->GetBattlegroundId()))
                pBG->AddPlayer(_player);
        }
    }
...
void WorldSession::HandleMoveTeleportAck(WorldPacket &recv_data)
{
    ...
    sLog->outDebug(LOG_FILTER_UNITS, "Guid " UI64FMTD, guid);
    sLog->outDebug(LOG_FILTER_UNITS, "Flags %u, time %u", flags, time/IN_MILLISECONDS);
...
void WorldSession::HandleMovementOpcodes(WorldPacket &recv_data)
{
    ...
    if(plMover)                                            // nothing is charmed, or player charmed
    {
        ...
        if(movementInfo.pos.GetPositionZ() < underMapValueZ)
        {
			...
			if (underMapValueZ != -500) // For Custom Arena Zone //thanks to Saqirmdev
			{
				// Hackfix for ArenaUnderZ -> Teleport WHY TRINITY BREAK IT!
				if (plMover->GetMapId() == 572) // Lordaeron Arena 
					plMover->TeleportTo(572, 1286.14868f, 1667.32f, 41.0f, 1.6f); //

				if (plMover->GetMapId() == 559) // Nagrand Arena 
					plMover->TeleportTo(559, 4052.79868f, 2926.32f, 16.0f, 1.6f); 

				if (plMover->GetMapId() == 562) // Blade Edge arena 6237.797363 261.146088 2 0.7
					plMover->TeleportTo(562, 6237.79768f, 261.142f, 2.0f, 4.0f); 
 
				if (plMover->GetMapId() == 617) // Dalaran Arena
					plMover->TeleportTo(617, 1286.14868f, 1667.32f, 41.0f, 1.6f); 
