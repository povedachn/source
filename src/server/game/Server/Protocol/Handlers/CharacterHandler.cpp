...
#include "../../../../scripts/World/Custom/itemgossiphelp.h"
    ...
    */
    switch(createInfo->Stage)
    {
        ...
        case 3:
        {
			...
			sObjectMgr->SetHighestGuids();		//CUSTOM MIGRACIONES
            ...
            sWorld->AddCharacterNameData(pNewChar->GetGUIDLow(), std::string(pNewChar->GetName()), pNewChar->getGender(), pNewChar->getRace(), pNewChar->getClass());
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder * holder)
{
	...
	pCurrChar->CastSpell(pCurrChar, 2479, true);
    ...
    if(pCurrChar->m_deathState != ALIVE)
    {
		...
		if(pCurrChar->HasAura(2479))
			pCurrChar->RemoveAurasDueToSpell(2479);
    ...
    if(pCurrChar->HasAtLoginFlag(AT_LOGIN_UPDATE_NAME))
    {
        ...
        if(result)
        {
            ...
            sWorld->AddCharacterNameData(pCurrChar->GetGUIDLow(), nameUpdate_AT_LOGIN_UPDATE_NAME, pCurrChar->getGender(), pCurrChar->getRace(), pCurrChar->getClass());
        }
	...
	if(pCurrChar->GetTotalPlayedTime() < 1)
	{
		if(!pCurrChar->HasItemCount(6948, 1))
			pCurrChar->AddItem(6948, 1);
		if(!pCurrChar->HasItemCount(6283, 1))
			pCurrChar->AddItem(6283, 1);
		if(!pCurrChar->HasItemCount(33781, 1))
			pCurrChar->AddItem(33781, 1);
		aprender_habilidad_clase(pCurrChar);
	}
	
	//Custom, mensaje de recordar borrar cache al logear
	if(pCurrChar->GetTotalPlayedTime() < 30)
	{
		sWorld->SendPlayerText("|CFF800080Es necesario borrar caché si no lo has hecho ya para visualizar todo correctamente|R", pCurrChar);
		ChatHandler handler = ChatHandler(pCurrChar);
		handler.PSendSysMessage("|CFFFF0000Si no has borrado la carpeta Caché aún, es necesario que la borres para visualizar correctamente todos los cambios que el servidor ofrece y evitar ""bugs"" visuales en objetos y criaturas.|R");
	}
}
...
void WorldSession::HandleCharCustomize(WorldPacket &recv_data)
{
    ...
    PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_LOAD_CHARACTER_AT_LOGIN);
    ...
    stmt = CharacterDatabase.GetPreparedStatement(CHAR_LOAD_CHARACTER_NAME);
    stmt->setUInt32(0, GUID_LOPART(guid));
...
void WorldSession::HandleCharFactionOrRaceChange(WorldPacket &recv_data)
{
    ...
    if(recv_data.GetOpcode() == CMSG_CHAR_FACTION_CHANGE)
    {
        ...
        {
            stmt->setUInt16(1, 1);
            stmt->setUInt16(2, 215);
            stmt->setFloat (3, -2306.250977f);
            stmt->setFloat (4, -446.046448f);
            stmt->setFloat (5, -5.338296f);
            Player::SavePositionInDB(1, -2306.250977f, -446.046448f, -5.338296f, 1.790976f, 215, lowGuid);
        }
        else
        {
            stmt->setUInt16(1, 1);
        ...
        // Quest conversion
        if(QueryResult resultQuest = WorldDatabase.Query("SELECT `alliance_id`, `horde_id` FROM `player_factionchange_quests`"))
        {
            do
            {
                Field* fields2 = resultQuest->Fetch();
                ...
                trans->PAppend("UPDATE `character_queststatus_rewarded` SET quest = '%u' WHERE quest = '%u' AND guid = '%u'",
                    team == BG_TEAM_ALLIANCE ? quest_alliance : quest_horde, team == BG_TEAM_ALLIANCE ? quest_horde : quest_alliance, lowGuid);
            } while (resultQuest->NextRow());
        }
    }
