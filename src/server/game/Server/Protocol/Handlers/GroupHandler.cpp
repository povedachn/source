...
void WorldSession::HandleGroupInviteOpcode(WorldPacket &recv_data)
{
	...
	//GetPlayer() = el que invita, player = el invitado
	
	/*if ((GetPlayer()->GetAreaId() != 368 && player->GetAreaId() == 368))
    {
		ChatHandler handler(GetPlayer());
		handler.PSendSysMessage("|CFFFF0000No puedes invitar a un jugador que est""\xC3\xA9"" en la Zona PvP.|R");
		//SendPartyResult(PARTY_OP_UNINVITE, membername, ERR_GROUP_FULL);
		return;
    }	
	...
	{
		ChatHandler handler(GetPlayer());
		handler.PSendSysMessage("|CFFFF0000No puedes invitar a un jugador que no est""\xC3\xA9"" en la Zona PvP mientras est""\xC3\xA9""s en la Zona PvP.|R");
	    //SendPartyResult(PARTY_OP_UNINVITE, membername, ERR_GROUP_FULL);
		return;
	}*/
	
	if (GetPlayer()->GetAreaId() == 368)
	{
		Group* plrgroupcheck = GetPlayer()->GetGroup();
		if (plrgroupcheck && plrgroupcheck->GetMembersCount() > 3)
		{
			//ChatHandler handler(player);
			ChatHandler handler2(GetPlayer());
			//handler.PSendSysMessage("|CFFFF0000Un jugador te ha intentado invitar a un grupo pero ya tiene un grupo con las máximas personas permitidas en la Zona PvP (3 jugadores).|R");
			handler2.PSendSysMessage("|CFFFF0000Tu grupo no puede tener m""\xC3\xA1""s de 3 miembros en la Zona PvP.|R");
	        //SendPartyResult(PARTY_OP_UNINVITE, membername, ERR_GROUP_FULL);
			return;
		}
	}
	
	//Prohibido grupos en Liza
	if ((player->GetAreaId() == 2177 && player->GetZoneId() == 33 && player->GetMapId() == 0))
    {
		ChatHandler handler(GetPlayer());
		handler.PSendSysMessage("|CFFFF0000No puedes invitar a un jugador que est""\xC3\xA9"" en Liza.|R");
		return;
    }
	if (GetPlayer()->GetAreaId() == 2177 && GetPlayer()->GetZoneId() == 33 && GetPlayer()->GetMapId() == 0)
	{
		ChatHandler handler(GetPlayer());
		handler.PSendSysMessage("|CFFFF0000No puedes invitar a un jugador estando en Liza.|R");
		return;
    }
