...
void WorldSession::HandleTabardVendorActivateOpcode(WorldPacket &recv_data)
{
	...
	if(IS_ITEM_GUID(guid))
		...
		unit = GetPlayer()->GetNPCIfCanInteractWith(guid, UNIT_NPC_FLAG_TABARDDESIGNER);

    if(!unit && !itemk)
    {
        sLog->outDebug(LOG_FILTER_NETWORKIO, "WORLD: HandleTabardVendorActivateOpcode - Unit\Item (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(guid)));
        return;
    }
...
void WorldSession::HandleBankerActivateOpcode(WorldPacket &recv_data)
{
	...
	if(IS_ITEM_GUID(guid))
		...
		unit = GetPlayer()->GetNPCIfCanInteractWith(guid, UNIT_NPC_FLAG_BANKER);
    if(!unit && !item)
    {
        sLog->outDebug(LOG_FILTER_NETWORKIO, "WORLD: HandleBankerActivateOpcode - Unit/Item (GUID: %u) not found or you can not interact with him.", uint32(GUID_LOPART(guid)));
        return;
    }
...
void WorldSession::HandleGossipHelloOpcode(WorldPacket &recv_data)
{
    ...
    if(unit->isSpiritGuide())
    {
        ...
        {
            pBG->AddPlayerToResurrectQueue(unit->GetGUID(), _player->GetGUID());
            sBattlegroundMgr->SendAreaSpiritHealerQueryOpcode(_player, pBG, unit->GetGUID());
            return;
        }
    }
...
void WorldSession::HandleListStabledPetsOpcode(WorldPacket &recv_data)
{
    ...
    if(!CheckStableMaster(npcGUID) && !IS_ITEM_GUID(npcGUID))
        return;
...
void WorldSession::HandleRepairItemOpcode(WorldPacket &recv_data)
{
	...
	if(IS_ITEM_GUID(npcGUID))
		item2 = GetPlayer()->GetItemByGuid(npcGUID);
	else
		unit = GetPlayer()->GetNPCIfCanInteractWith(npcGUID, UNIT_NPC_FLAG_REPAIR);
	...
	if(unit)
		discountMod = _player->GetReputationPriceDiscount(unit);
