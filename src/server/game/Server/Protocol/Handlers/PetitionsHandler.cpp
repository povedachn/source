...
void WorldSession::HandlePetitionBuyOpcode(WorldPacket &recv_data)
{
    ...
    // prevent cheating
	if(IS_ITEM_GUID(guidNPC))
		itemk = GetPlayer()->GetItemByGuid(guidNPC);
	else
		pCreature = GetPlayer()->GetNPCIfCanInteractWith(guidNPC, UNIT_NPC_FLAG_PETITIONER);

    if(!pCreature && !itemk)
    {
        sLog->outDebug(LOG_FILTER_NETWORKIO, "WORLD: HandlePetitionBuyOpcode - Unit\Item (GUID: %u) not found or you can't interact with him.", GUID_LOPART(guidNPC));
        return;
    }
    ...
    if((itemk)||(pCreature && pCreature->isTabardDesigner()))
    {
    ...
    {                                                       //player hasn't got enough money
        _player->SendBuyError(BUY_ERR_NOT_ENOUGHT_MONEY, NULL, charterid, 0);
        return;
    }
...
void WorldSession::SendPetitionShowList(uint64 guid)
{
    ...
    if (pCreature && pCreature->isTabardDesigner())
    {
        data << uint8(1);                                   // count
        data << uint32(1);                                  // index
