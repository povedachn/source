...
#include "../../../../scripts/World/Custom/npc_arena1v1.h"
...
void WorldSession::HandleBattlemasterJoinOpcode(WorldPacket &recv_data)
{
    ...
    if(instanceId)
        pBG = sBattlegroundMgr->GetBattlegroundThroughClientInstance(instanceId, bgTypeId);
    if(!pBG)
        pBG = sBattlegroundMgr->GetBattlegroundTemplate(bgTypeId);
    if(!pBG)
        return;
    ...
    if(!joinAsGroup)
    {
        ...
        sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, pBG, queueSlot, STATUS_WAIT_QUEUE, avgTime, 0, ginfo->ArenaType);
        SendPacket(&data);
    ...
    else
    {
        ...
        if(grp->GetLeaderGUID() != _player->GetGUID())
            return;
        err = grp->CanJoinBattlegroundQueue(pBG, bgQueueTypeId, 0, pBG->GetMaxPlayersPerTeam(), false, 0);
        isPremade = (grp->GetMembersCount() >= pBG->GetMinPlayersPerTeam());
...
void WorldSession::HandlePVPLogDataOpcode(WorldPacket & /*recv_data*/)
{
    ...
    WorldPacket data;
    sBattlegroundMgr->BuildPvpLogDataPacket(&data, pBG);
    SendPacket(&data);
...
void WorldSession::HandleBattleFieldPortOpcode(WorldPacket &recv_data)
{
    ...
    Battleground* pBG = (GetPlayer()->challengeData->bg) ? GetPlayer()->challengeData->bg : sBattlegroundMgr->GetBattleground(ginfo.IsInvitedToBGInstanceGUID, bgTypeId);
    ...
    if(action == 1 && ginfo.ArenaType == 0)
    {
        ...
        if(_player->getLevel() > pBG->GetMaxLevel())
        {
            sLog->outError("Battleground: Player %s (%u) has level (%u) higher than maxlevel (%u) of battleground (%u)! Do not port him to battleground!",
                _player->GetName(), _player->GetGUIDLow(), _player->getLevel(), pBG->GetMaxLevel(), pBG->GetTypeID());
            action = 0;
        }
    }

	if (_player->GetAreaId() == 368 || _player->GetAreaId() == 19)
	{
		ChatHandler(_player->GetSession()).SendSysMessage("No puedes entrar en Arena o BG estando en la Isla PvP o en la Shop Monturas Custom.");
		return;
	}
    ...
    switch(action)
    {
        case 1:                                         // port to battleground
            ...
            sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, pBG, queueSlot, STATUS_IN_PROGRESS, 0, pBG->GetStartTime(), pBG->GetArenaType());
            _player->GetSession()->SendPacket(&data);
            ...
            _player->SetBattlegroundId(pBG->GetInstanceID(), bgTypeId);
            // set the destination team
            _player->SetBGTeam(ginfo.Team);
            // pBG->HandleBeforeTeleportToBattleground(_player);
            (_player->challengeData->bg) ? sBattlegroundMgr->SendToBattleground(_player, _player->challengeData->bg) :
                                       sBattlegroundMgr->SendToBattleground(_player, ginfo.IsInvitedToBGInstanceGUID, bgTypeId);
            // add only in HandleMoveWorldPortAck()
            sLog->outDebug(LOG_FILTER_BATTLEGROUND, "Battleground: player %s (%u) joined battle for bg %u, bgtype %u, queue type %u.", _player->GetName(), _player->GetGUIDLow(), pBG->GetInstanceID(), pBG->GetTypeID(), bgQueueTypeId);
            break;
        case 0:                                         // leave queue
            // if player leaves rated arena match before match start, it is counted as he played but he lost
            if(ginfo.IsRated && ginfo.IsInvitedToBGInstanceGUID)
            {
                if(ArenaTeam* pAT = sArenaTeamMgr->GetArenaTeamById(ginfo.Team))
                {
                    ...
                    pAT->MemberLost(_player, ginfo.OpponentsMatchmakerRating);
                    pAT->SaveToDB();
                }
            }
            ...
            SendPacket(&data);
            sLog->outDebug(LOG_FILTER_BATTLEGROUND, "Battleground: player %s (%u) left queue for bgtype %u, queue type %u.", _player->GetName(), _player->GetGUIDLow(), pBG->GetTypeID(), bgQueueTypeId);
            break;
        default:
...
void WorldSession::HandleBattlefieldStatusOpcode(WorldPacket & /*recv_data*/)
{
    ...
    for(uint8 i = 0; i < PLAYER_MAX_BATTLEGROUND_QUEUES; ++i)
    {
        ...
        {
            pBG = _player->GetBattleground();
            //i cannot check any variable from player class because player class doesn't know if player is in 2v2 / 3v3 or 5v5 arena
            //so i must use bg pointer to get that information
            if(pBG && pBG->GetArenaType() == arenaType)
            {
                // this line is checked, i only don't know if GetStartTime is changing itself after bg end!
                // send status in Battleground
                sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, pBG, i, STATUS_IN_PROGRESS, pBG->GetEndTime(), pBG->GetStartTime(), arenaType);
                SendPacket(&data);
                continue;
            }
        }
        ...
        {
            pBG = sBattlegroundMgr->GetBattleground(ginfo.IsInvitedToBGInstanceGUID, bgTypeId);
            if(!pBG)
                continue;
            uint32 remainingTime = getMSTimeDiff(getMSTime(), ginfo.RemoveInviteTime);
            // send status invited to Battleground
            sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, pBG, i, STATUS_WAIT_JOIN, remainingTime, 0, arenaType);
            SendPacket(&data);
        }
        else
        {
            ...
            sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, pBG, i, STATUS_WAIT_QUEUE, avgTime, getMSTimeDiff(ginfo.JoinTime, getMSTime()), arenaType);
            SendPacket(&data);
        }
    }
}
...
void WorldSession::HandleAreaSpiritHealerQueryOpcode(WorldPacket &recv_data)
{
    ...
    if(pBG)
        sBattlegroundMgr->SendAreaSpiritHealerQueryOpcode(_player, pBG, guid);
    else
...
void WorldSession::HandleBattlemasterJoinArena(WorldPacket &recv_data)
{
	...
	if (arenaslot == 2)
	{
		_player->GetSession()->SendNotification("Las arenas 5c5 est""\xC3\xA1""n deshabilitadas.");
		return;
	}
		
	if (Habilitada2() == 0) //Arenas Habilitadas, se modifica en SmartAI.cpp, la hora se pone por game_event (evento 83)
	{
		...
		else
		{
			...
			if(grp->GetLeaderGUID() != _player->GetGUID())
				return;
			err = grp->CanJoinBattlegroundQueue(pBG, bgQueueTypeId, arenatype, arenatype, (bool)isRated, arenaslot);
		}
		...
		if(asGroup)
		{
			...
			for(GroupReference *itr = grp->GetFirstMember(); itr != NULL; itr = itr->next())
			{
				...
				// send status packet (in queue)
				sBattlegroundMgr->BuildBattlegroundStatusPacket(&data, pBG, queueSlot, STATUS_WAIT_QUEUE, avgTime, 0, arenatype);
				member->GetSession()->SendPacket(&data);
				sBattlegroundMgr->BuildGroupJoinedBattlegroundPacket(&data, err);
				member->GetSession()->SendPacket(&data);
	...
	else if (Habilitada2() == 1) //Arenas deshabilitadas, se modifica en SmartAI.cpp, la hora se pone por game_event (evento 83)
	{
