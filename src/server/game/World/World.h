...
enum WorldBoolConfigs
{
	...
	CONFIG_GM_CHAT_COLOR_HABILITADO,
...
public:
	...
	void SendAreaMessage(uint32 area, WorldPacket* packet, .../*, WorldSession* self, uint32 team*/);
    void SendAreaText(uint32 area, const char* text, .../*WorldSession* self, uint32 team*/);
	void SendAllWorldMessage(WorldPacket* packet, ...);
	void SendAllWorldText(const char* text, ...);
	void SendPlayerMessage(WorldPacket* packet, ...);
	void SendPlayerText(const char* text, Player* player, ...);
