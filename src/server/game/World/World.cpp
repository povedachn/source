...
void World::LoadConfigSettings(bool reload)
{
    ...
    m_bool_configs[CONFIG_CROSSFACTIONBG] = sConfig->GetBoolDefault("MixedBGs", true);
    ...
    m_bool_configs[CONFIG_ALLOW_TWO_SIDE_INTERACTION_CALENDAR]= sConfig->GetBoolDefault("AllowTwoSide.Interaction.Calendar", false);
    ...
    m_int_configs[CONFIG_ARENA_RATED_UPDATE_TIMER]                   = sConfig->GetIntDefault ("Arena.RatedUpdateTimer", 5 * IN_MILLISECONDS);
    ...
    m_int_configs[CONFIG_ARENA_PROGRESSIVE_MMR_TIMER]                = sConfig->GetIntDefault ("Arena.ProgressiveMMRTimer", 30000);
    m_int_configs[CONFIG_ARENA_PROGRESSIVE_MMR_STEPSIZE]             = sConfig->GetIntDefault ("Arena.ProgressiveMMRStepSize", 50);
	...
	m_bool_configs[CONFIG_ARENA_1V1_ENABLE]							 = sConfig->GetBoolDefault("Arena.1v1.Enable", true);
	m_bool_configs[CONFIG_ARENA_1V1_ANNOUNCER]						 = sConfig->GetBoolDefault("Arena.1v1.Announcer", false);
	m_int_configs[CONFIG_ARENA_1V1_MIN_LEVEL]						 = sConfig->GetIntDefault("Arena.1v1.MinLevel", 80);
	m_int_configs[CONFIG_ARENA_1V1_COSTS]							 = sConfig->GetIntDefault("Arena.1v1.Costs", 400000);
	m_bool_configs[CONFIG_ARENA_1V1_VENDOR_RATING]					 = sConfig->GetBoolDefault("Arena.1v1.VendorRating", false);
	m_float_configs[CONFIG_ARENA_1V1_ARENAPOINTS_MULTI]				 = sConfig->GetFloatDefault("Arena.1v1.ArenaPointsMulti", 0.64f);
	m_bool_configs[CONFIG_ARENA_1V1_BLOCK_FORBIDDEN_TALENTS]		 = sConfig->GetBoolDefault("Arena.1v1.BlockForbiddenTalents", true);
    ...
    if(int32 clientCacheId = sConfig->GetIntDefault("ClientCacheVersion", 0))
    {
        ...
        } else sLog->outError("ClientCacheVersion can't be negative %d, ignored.", clientCacheId);
    }
    ...
    m_int_configs[CONFIG_CHARDELETE_KEEP_DAYS] = sConfig->GetIntDefault("Arena.KeepDays", 30);

	m_int_configs[CONFIG_ARENA_SEASON_RESET] = sConfig->GetIntDefault("Arena.season_reset", 30); //CUSTOM KHANX
    ...
    m_bool_configs[CONFIG_MOVEMENT_CHECKS_FLYING]       = sConfig->GetBoolDefault("MovementChecker.flying", true);
	...
	//Anuncio al gm al loguear
	m_bool_configs[CONFIG_GMLOGIN_ENABLED] = sConfig->GetBoolDefault("Gm.Login.Enable", true);

	// Gm chat azul
    m_bool_configs[CONFIG_GM_CHAT_COLOR_HABILITADO] = sConfig->GetBoolDefault("Gm.Chat.Color.Habilitado", false);
    ...
    m_bool_configs[CONFIG_DUEL_RESET_COOLDOWN_ONLY_IN_ZONA_DE_DUELOS] = sConfig->GetBoolDefault("DuelReset.Cooldown.Only.in.zona.de.duelos", false);
    ...
    m_bool_configs[CONFIG_DUEL_RESET_COOLDOWN_MAX_ENERGY_ON_START] = sConfig->GetBoolDefault("DuelReset.Cooldown.Max.Energy.OnStart", false);
...
void World::SetInitialWorldSettings()
{
	...
	// Delete arena seasons
	Player::DeleteArenaSeasons();	//Custom Khanx
	...
	//Custom
	/*CharacterDatabase.PExecute("UPDATE gameobjects_pvp_zone SET curar_1 = 0 WHERE Unica = 1");
	CharacterDatabase.PExecute("UPDATE gameobjects_pvp_zone SET curar_2 = 0 WHERE Unica = 1");
	CharacterDatabase.PExecute("UPDATE gameobjects_pvp_zone SET curar_3 = 0 WHERE Unica = 1");
	CharacterDatabase.PExecute("UPDATE gameobjects_pvp_zone SET curar_4 = 0 WHERE Unica = 1");
	CharacterDatabase.PExecute("UPDATE gameobjects_pvp_zone SET rabiar_1 = 0 WHERE Unica = 1");
	CharacterDatabase.PExecute("UPDATE gameobjects_pvp_zone SET rabiar_2 = 0 WHERE Unica = 1");
	CharacterDatabase.PExecute("UPDATE gameobjects_pvp_zone SET rabiar_3 = 0 WHERE Unica = 1");
	CharacterDatabase.PExecute("UPDATE gameobjects_pvp_zone SET rabiar_4 = 0 WHERE Unica = 1");
	CharacterDatabase.PExecute("UPDATE gameobjects_pvp_zone SET rabiar_5 = 0 WHERE Unica = 1");
	CharacterDatabase.PExecute("UPDATE gameobjects_pvp_zone SET rabiar_6 = 0 WHERE Unica = 1");
	CharacterDatabase.PExecute("UPDATE gameobjects_pvp_zone SET caldero_1 = 0 WHERE Unica = 1");
	CharacterDatabase.PExecute("UPDATE gameobjects_pvp_zone SET caldero_2 = 0 WHERE Unica = 1");
	CharacterDatabase.PExecute("UPDATE gameobjects_pvp_zone SET caldero_3 = 0 WHERE Unica = 1");*/
	...
	CharacterDatabase.PExecute("DELETE FROM racha");
	CharacterDatabase.PExecute("DELETE FROM racha_sanacion");
...
void World::Update(uint32 diff)
{
    ...
    if(m_timers[WUPDATE_PINGDB].Passed())
    {
        ...
        sLog->outDebug(LOG_FILTER_NETWORKIO, "Ping MySQL to keep connection alive");
...
void World::SendAreaMessage(uint32 area, WorldPacket* packet, .../*, WorldSession* self, uint32 team*/)
{
    SessionMap::const_iterator itr;
    for(itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
    {
...
/// Send a System Message to all players
void World::SendPlayerText(const char* text, Player* player, ...)
{
    char str[1024];
    va_list ap;
    va_start(ap, text);
...
void World::ShutdownMsg(bool show, Player* player)
{
        ...
        (m_ShutdownTimer > 12 * HOUR && (m_ShutdownTimer % (12 * HOUR)) == 0)) // > 12 h ; every 12 h
    {
        ...
        sLog->outDebug(LOG_FILTER_NETWORKIO, "Server is %s in %s", (m_ShutdownMask & SHUTDOWN_MASK_RESTART ? "restart" : "shuttingdown"), str.c_str());
    }
}
...
void World::ShutdownCancel()
{
    ...
    sLog->outDebug(LOG_FILTER_NETWORKIO, "Server %s cancelled.", (m_ShutdownMask & SHUTDOWN_MASK_RESTART ? "restart" : "shuttingdown"));
