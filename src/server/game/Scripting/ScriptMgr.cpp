...
bool ScriptMgr::OnCriteriaCheck(AchievementCriteriaData const* data, Player* pSource, Unit* target)
{
    ASSERT(pSource);
    // target can be NULL.

    GET_SCRIPT_RET(AchievementCriteriaScript, data->ScriptId, tmpscript, false);
    return tmpscript->OnCheck(pSource, target);
}
...
//Custom
void ScriptMgr::OnPVPHeal(Player* healer, Unit* healed, uint32 healAmount)
{
	FOREACH_SCRIPT(PlayerScript)->OnPVPHeal(healer, healed, healAmount);
}
