    ...
    for(uint32 i = 0; i < sSpellStore.GetNumRows(); ++i)
    {
        ...
        for(uint8 j = 0; j < MAX_SPELL_EFFECTS; ++j)
        {
            switch(spellInfo->Effect[j])
            {
                ...
                //case SPELL_AURA_PERIODIC_MANA_LEECH:
                    mSpellCustomAttr[i] |= SPELL_ATTR0_CU_CAN_RESIST;
                    break;
                case SPELL_EFFECT_POWER_DRAIN:
                case SPELL_EFFECT_POWER_BURN:
                    mSpellCustomAttr[i] |= SPELL_ATTR0_CU_NO_INITIAL_THREAT;
        ...
        switch(i)
        {
			/////////////////////////// 	CAMISAS DE ILUSIÓN - INICIO		///////////////////////////
			case 34845: // Ilusión Orca
				spellInfo->EffectMiscValue[0] = 400000;
				spellInfo->EffectImplicitTargetA[0] = TARGET_UNIT_CASTER;
                spellInfo->AttributesEx3 |= SPELL_ATTR3_DEATH_PERSISTENT;
			break;
			case 34848: // Ilusión Enano
				spellInfo->EffectMiscValue[0] = 400001;
				spellInfo->EffectImplicitTargetA[0] = TARGET_UNIT_CASTER;
                spellInfo->AttributesEx3 |= SPELL_ATTR3_DEATH_PERSISTENT;
			break;
			case 34849: // Ilusión Elfo de la Noche
				spellInfo->EffectMiscValue[0] = 400002;
				spellInfo->EffectImplicitTargetA[0] = TARGET_UNIT_CASTER;
                spellInfo->AttributesEx3 |= SPELL_ATTR3_DEATH_PERSISTENT;
			break;
			case 34851: // Ilusión Trol
				spellInfo->EffectMiscValue[0] = 400003;
				spellInfo->EffectImplicitTargetA[0] = TARGET_UNIT_CASTER;
                spellInfo->AttributesEx3 |= SPELL_ATTR3_DEATH_PERSISTENT;
			break;
			case 44372: // Ilusión Draenei Hembra
				spellInfo->EffectMiscValue[0] = 400004;
				spellInfo->EffectImplicitTargetA[0] = TARGET_UNIT_CASTER;
                spellInfo->AttributesEx3 |= SPELL_ATTR3_DEATH_PERSISTENT;
			break;
			...
			case 41983:
                ...
                spellInfo->AttributesEx3 |= SPELL_ATTR3_DEATH_PERSISTENT;
            break;
			/////////////////////////// 	CAMISAS DE ILUSIÓN - FINAL		///////////////////////////
			/*case 53595: // Martillo del honrado
				spellInfo->EffectBasePoints[0] = 645;
			break;*/
			case 51425: // Asolar
				spellInfo->EffectBasePoints[0] = 645;
			break;
			case 55268: // Golpe de escarcha
				spellInfo->EffectBasePoints[0] = 290;
			break;
			case 48657: // Puñalada
				spellInfo->EffectBasePoints[0] = 389;
				spellInfo->EffectBasePoints[1] = 149;
			...
			break;*/
			/*case 21651: // Cofre de Gurubashi
				spellInfo->CastingTimeIndex = 6;
			break;*/
			case 35395: // Golpe de Cruzado (buffeo)
				spellInfo->EffectBasePoints[0] = 84;
			break;
			case 48572: // Triturar (buffeo)
				spellInfo->EffectBasePoints[0] = 350;
				mSpellCustomAttr[i] |= SPELL_ATTR0_CU_REQ_CASTER_BEHIND_TARGET;
			break;
			case 48668: // Eviscerar (buffeo)
				spellInfo->EffectBasePoints[0] = 610;
			break;
			case 47471: // Ejecutar (buffeo)
				spellInfo->EffectBasePoints[0] = 2055;
			break;
			case 7384: // Abrumar
				spellInfo->EffectBasePoints[0] = 170;
			break;
			case 48691: // Emboscada
				/*spellInfo->EffectBasePoints[0] = 409;
				*/mSpellCustomAttr[i] |= SPELL_ATTR0_CU_REQ_CASTER_BEHIND_TARGET;
			break;
			case 55271: // Golpe de Plaga
				spellInfo->EffectBasePoints[0] = 609;
			break;
			case 53209: // Disparo de Quimera (Buffeo, antes 124 (125% daño arma))
				spellInfo->EffectBasePoints[2] = 129;
			break;
			case 55857: // Aura Piedra VIP, phase a la que te cambia
				spellInfo->EffectMiscValue[0] = 5;
			break;
			case 8690: // Piedra de Hogar
				spellInfo->Category = 0;
				spellInfo->RecoveryTime = 0;
				spellInfo->CategoryRecoveryTime = 900000;
			break;
			case 8611: //Aura de la Zona PvP al resucitar, duracion = 15 seg y quitado el efecto de no moverte
            ...
            case 8178: // Grounding Totem - [Shaman]
                ...
                break;
			// Custom: spells de totems y filotormenta sin visual
			/*case 58735: // Tótem de magma
			case 58736: // Tótem de magma
			case 8187:	// Tótem de magma
			case 8188:	// Tótem de magma
				Unit* target;
				Unit const* caster;
				if(!caster->IsWithinLOSInMap(target))
					spellInfo->SpellVisual[0] = 0;
				else
					spellInfo->SpellVisual[0] = 2107;
				break;*/
			/*case 58702: // Tótem lengua de fuego
				if(!caster->IsWithinLOSInMap(target))
					spellInfo->SpellVisual[0] = 0;
				else
					spellInfo->SpellVisual[0] = 67;
				break;*/
			/*case 3600: // Tótem nexo de tierra
				if(!caster->IsWithinLOSInMap(target))
					spellInfo->SpellVisual[0] = 0;
				else
					spellInfo->SpellVisual[0] = 68;
				break;*/
			/*case 50622: // Torbellino (Filotormenta)
				spellInfo->AttributesEx4 = 0;
				spellInfo->SpellVisual[0] = 223;
				break;*/
			/*case 46924:
				spellInfo->SpellVisual[0] = 0;
				spellInfo->SpellVisual[1] = 0;
				break;*/
           ...
           case 69839: // big ooze explode cast
			...
			case 49575: // death grip (leap effect)
                spellInfo->AttributesEx |= !SPELL_ATTR1_CANT_BE_REFLECTED;
                break;
			case 49576: // death grip (no se puede reflejar)
				spellInfo->AttributesEx += SPELL_ATTR1_CANT_BE_REFLECTED;
				break;
