...
void Spell::AddUnitTarget(Unit* pVictim, uint32 effIndex)
{
    ...
    if(m_originalCaster)
    {
		// Fix reflejar disipar magia del sacer si el sacer es aliado del warr
		/*if((m_spellInfo->Id == 988 || m_spellInfo->Id == 527) && !m_originalCaster->IsHostileTo(pVictim))
		{
    ...
    if(m_spellInfo->speed > 0.0f && m_caster != pVictim)
    {
        ...
        if(dist < 5.0f)
            dist = 5.0f;
        target.timeDelay = (uint64)floor(dist / m_spellInfo->speed * 900.0f);
		
        // Calculate minimum incoming time
        if(m_delayMoment == 0 || m_delayMoment>target.timeDelay)
            m_delayMoment = target.timeDelay;
			
		// Fix Delay hecho por Ray
		if(m_spellInfo->Id == 7922 		//Carga aturdidora (stun del warr, no se si es necesario o no)
		|| m_spellInfo->Id == 11578		//Cargar, rango 3
		|| m_spellInfo->Id == 6178		//Cargar, rango 2
		|| m_spellInfo->Id == 100		//Cargar, rango 1
		|| m_spellInfo->Id == 20253		//Interceptar (stun del interceptar)
		|| m_spellInfo->Id == 20252		//Interceptar
		|| m_spellInfo->Id == 3355		//Efecto de la trampa congelante, no la trampa en si. Rango 1
		|| m_spellInfo->Id == 14308		//Efecto de la trampa congelante, no la trampa en si. Rango 2
		|| m_spellInfo->Id == 14309		//Efecto de la trampa congelante, no la trampa en si. Rango 3
		|| m_spellInfo->Id == 43448		//Trampa congelante (no es la del hunter)
		|| m_spellInfo->Id == 43415		//Trampa congelante, rango 3 (no es la del hunter)
		|| m_spellInfo->Id == 55041		//Efecto de la trampa congelante, no la trampa en si.
		|| m_spellInfo->Id == 10890		//Alarido psiquico, rango 4
		|| m_spellInfo->Id == 10888		//Alarido psiquico, rango 3
		|| m_spellInfo->Id == 8124		//Alarido psiquico, rango 2
		|| m_spellInfo->Id == 8122		//Alarido psiquico, rango 1
		|| m_spellInfo->Id == 6215		//Miedo, rango 3
		|| m_spellInfo->Id == 12826		//Polimorfia, rango 4
		|| m_spellInfo->Id == 12825		//Polimorfia, rango 3
		|| m_spellInfo->Id == 12824		//Polimorfia, rango 2
		|| m_spellInfo->Id == 118		//Polimorfia, rango 1
		|| m_spellInfo->Id == 61721		//Polimorfia, conejo
		|| m_spellInfo->Id == 61305		//Polimorfia, felino negro
		|| m_spellInfo->Id == 28272		//Polimorfia, cerdo
		|| m_spellInfo->Id == 28271		//Polimorfia, tortuga
		|| m_spellInfo->Id == 45438		//Bloque de Hielo
		|| m_spellInfo->Id == 41425
		|| m_spellInfo->Id == 5246		//Grito intimidador
		|| m_spellInfo->Id == 36554		//Shadowstep
		|| m_spellInfo->Id == 36563		//Shadowstep
		|| m_spellInfo->Id == 1833		//Golpe bajo
		|| m_spellInfo->Id == 8643		//Golpe en los riñones Rank 2
		|| m_spellInfo->Id == 408		//Golpe en los riñones Rank 1
		|| m_spellInfo->Id == 31228		//Burlar a la muerte, talent del picaro
		|| m_spellInfo->Id == 31229		//Burlar a la muerte, talent del picaro
		|| m_spellInfo->Id == 31230		//Burlar a la muerte, talent del picaro
		)		
		{
			target.timeDelay = 0;
			if(m_delayMoment == 0 || m_delayMoment>target.timeDelay)
...
SpellMissInfo Spell::DoSpellHitOnUnit(Unit* unit, const uint32 effectMask, bool scaleAura)
{
	...
	//Fix abrumar que no se quitaba si se absorvía, etc
	if(m_spellInfo->Id == 7384 && m_caster->ToPlayer()->HasAura(60503))
		m_caster->ToPlayer()->RemoveAurasDueToSpell(60503);
	
	//Fix sabañon del dk que no se quita al tirarse escudo divino
	if(m_spellInfo->Id == 642 && m_caster->ToPlayer()->HasAura(50436))
		m_caster->ToPlayer()->RemoveAurasDueToSpell(50436);
	
	//Quitar Bomba viva al lanzar bloque de hielo
	if(m_spellInfo->Id == 45438)
	{
		if(m_caster->HasAura(44457))
			m_caster->RemoveAurasDueToSpell(44457);
		else if (m_caster->HasAura(55359))
			m_caster->RemoveAurasDueToSpell(55359);
		else if (m_caster->HasAura(55360))
			m_caster->RemoveAurasDueToSpell(55360);
	}

    if(m_caster != unit)
    {
		//Quitar Filotormenta al lanzar mano de proteccion del paladin
		if(m_spellInfo->Id==10278 && unit->HasAura(46924))
			unit->RemoveAurasDueToSpell(46924);
			
		//Quitar sigilo al lanzar raices del chaman
		if(m_spellInfo->Id==64695)
			unit->RemoveAurasByType(SPELL_AURA_MOD_STEALTH);
		
		//Quitar Sentencia de Justicia cuando se usa Fear (todas las spells con fear) para evitar casteos en fear
		if(m_spellInfo->Id == 6215 || m_spellInfo->Id == 6213 || m_spellInfo->Id == 5782 || m_spellInfo->Id == 17928 || m_spellInfo->Id == 5484 || m_spellInfo->Id == 47860 || m_spellInfo->Id == 47859 || m_spellInfo->Id == 27223 || m_spellInfo->Id == 17926 || m_spellInfo->Id == 17925 || m_spellInfo->Id == 6789)
			if(unit->HasAura(20184))
				unit->RemoveAurasDueToSpell(20184);
				
		//Quitar Sentencia de Justicia cuando se usa Alarido Psiquico de Sacerdote para evitar bug de casteo en fear
		if(m_spellInfo->Id == 10890 || m_spellInfo->Id == 10888 || m_spellInfo->Id == 8124 || m_spellInfo->Id == 8122)
			if(unit->HasAura(20184))
				unit->RemoveAurasDueToSpell(20184);
			
		/*if(m_spellInfo->Id == 1833 && unit->HasAuraType(SPELL_AURA_DEFLECT_SPELLS))
		...
		//Earthbind Totem Fix and magma totem
		/*if(m_spellInfo->Id==6474 || m_spellInfo->Id==58702 || m_spellInfo->Id==3600 || m_spellInfo->Id==51485 || m_spellInfo->Id==51483 || m_spellInfo->Id==16043 || m_spellInfo->Id==16130 || m_spellInfo->Id==53019 || m_spellInfo->Id==56425 || m_spellInfo->Id==64695 || m_spellInfo->Id==8377 || m_spellInfo->Id==8187 || m_spellInfo->Id==8188 || m_spellInfo->Id==58735 || m_spellInfo->Id==58736)
		{
			if (!m_caster->IsWithinLOSInMap(unit))
			{
				m_damage = 0;
		...
		// Fix Filotormenta (mirar el de mas arriba que es el bueno)
		/*if (m_spellInfo->Id == 50622)
		{
			if (!m_caster->IsWithinLOSInMap(unit))
			{
				/*char* msg = "No puedes ver al objetivo";
        ...
        {				
			if(unit->HasAura(6215))
			{
				...
				if(unit->HasAura(6215) && unit->GetAura(6215)->GetDuration() > tiempo*0.5)
					...
					{
						char* msg = "Ya est""\xC3\xA1"" activo un hechizo m""\xC3\xA1""s poderoso";
						WorldPacket data(SMSG_NOTIFICATION, 255);
						data << msg;
						m_caster->ToPlayer()->GetSession()->SendPacket(&data);
						return SPELL_MISS_NONE;
	...
	if(GetCaster() && GetSpellInfo())
    {
        if(Player* tmpPlayer = GetCaster()->ToPlayer())
        {
            if(tmpPlayer->HaveSpectators())
            {
                ...
                msg.CastSpell(m_spellInfo->Id, m_spellInfo->CastingTimeIndex); //CHECK THIS
...
void Spell::cast(bool skipCheck)
{
    ...
    //Custom
	//if(((m_spellInfo->speed > 0.0f || GetCCDelay(m_spellInfo) > 0) && !IsChanneledSpell(m_spellInfo)) || m_spellInfo->Id == 14157 || m_spellInfo->Id == 70802)
...
void Spell::TakeAmmo()
{
    if(m_attackType == RANGED_ATTACK && m_caster->GetTypeId() == TYPEID_PLAYER)
    {
		...
		//Custom no gastan ammo los cazas
...
#define SPELLATTR0_PASSIVE_AURA (SPELL_ATTR0_ABILITY|SPELL_ATTR0_PASSIVE|SPELL_ATTR0_HIDDEN_CLIENTSIDE|SPELL_ATTR0_HIDE_IN_COMBAT_LOG)

bool Spell::isCorpseExplosion(uint32 SpellId)
{
    switch(SpellId)
    {
        case 49158:
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if(m_spellInfo->AttributesEx6 & SPELL_ATTR6_UNK28 && m_caster->HasUnitMovementFlag(MOVEMENTFLAG_FALLING|MOVEMENTFLAG_FALLING_SLOW|MOVEMENTFLAG_JUMPING|MOVEMENTFLAG_SWIMMING|MOVEMENTFLAG_ASCENDING|MOVEMENTFLAG_PITCH_DOWN|MOVEMENTFLAG_HOVER))
        return SPELL_FAILED_NOT_READY;
    if(m_spellInfo->Id == 36554 && !m_caster->CanFreeMove())
        return SPELL_FAILED_NOT_READY;
    if(m_caster->getClass() == CLASS_DEATH_KNIGHT && isCorpseExplosion(m_spellInfo->Id))
    {
    ...
    // Anti-WPE block prevents stacking of a passive aura
    if(m_caster->GetTypeId() == TYPEID_PLAYER && (m_spellInfo->Attributes & SPELLATTR0_PASSIVE_AURA) == SPELLATTR0_PASSIVE_AURA && m_caster->HasAura(m_spellInfo->Id))
        return SPELL_FAILED_DONT_REPORT; // the m_caster player either voluntarily uses .cast command or is a cheater!
    ...
    if(!m_IsTriggeredSpell && m_caster->GetTypeId() == TYPEID_PLAYER)
        if(Battleground* pBG = m_caster->ToPlayer()->GetBattleground())
	...
	/*if(m_caster->ToPlayer()->InBattleground() && m_spellInfo->Id == 605)
           return SPELL_FAILED_SPELL_UNAVAILABLE;*/
	...
	//Arqueta de Gurubashi
	if (m_caster->GetAreaId() == 2177 && m_spellInfo->Id == 21651 && (m_caster->HasAura(642) || m_caster->HasAura(10278) || m_caster->HasAura(58597) || m_caster->HasAura(48066) || m_caster->HasAura(48065) || m_caster->HasAura(25218) || m_caster->HasAura(25217) || m_caster->HasAura(10901) || m_caster->HasAura(10900) || m_caster->HasAura(10899) || m_caster->HasAura(592) || m_caster->HasAura(17)))
	{
		char* msg = "No puedes lootear la Arqueta de Gurubashi con inmunidades o escudos";
		WorldPacket data(SMSG_NOTIFICATION, 255);
		data << msg;
		m_caster->ToPlayer()->GetSession()->SendPacket(&data);
	...
	// Fix castear cosas mientras fear + sentencia de justicia (roja)
	/*if ((m_caster->HasAura(6215) || m_caster->HasAura(6213) || m_caster->HasAura(5782)) && m_caster->HasAura(20184) && !(m_spellInfo->AttributesEx5 & SPELL_ATTR5_USABLE_WHILE_FEARED))
		return SPELL_FAILED_FLEEING;*/

	/*if ((m_caster->HasAura(6215) || m_caster->HasAura(6213) || m_caster->HasAura(5782)) && !(m_spellInfo->AttributesEx5 & SPELL_ATTR5_USABLE_WHILE_FEARED))
		return SPELL_FAILED_FLEEING;*/

	// Fix áreas como Lluvia de Fuego o Ventisca
	if (m_targets.HasDst())
	{
		float x, y, z;
		m_targets.GetDst()->GetPosition(x, y, z);
	
		if (!(m_spellInfo->AttributesEx2 & 0x00000004) && !(m_customAttr & SPELL_ATTR0_CU_IGNORE_LOS) && !m_caster->IsWithinLOS(x, y, z))
			return SPELL_FAILED_LINE_OF_SIGHT;
	}
	
	//Earthbind Totem Fix and magma totem
	/*if(m_spellInfo->Id==6474 || m_spellInfo->Id==3600 || m_spellInfo->Id==58702 || m_spellInfo->Id==51485 || m_spellInfo->Id==51483 || m_spellInfo->Id==16043 || m_spellInfo->Id==16130 || m_spellInfo->Id==53019  || m_spellInfo->Id==56425 || m_spellInfo->Id==64695 || m_spellInfo->Id==8377 || m_spellInfo->Id==8187 || m_spellInfo->Id==8188 || m_spellInfo->Id==58735 || m_spellInfo->Id==58736)
	{
    ...
    {			
		...
		/*// Fix Lluvia de Fuego
		if (m_spellInfo->Id == 47818)
			if (!m_caster->IsWithinLOSInMap(target))
				return SPELL_FAILED_LINE_OF_SIGHT;
		
		// Fix Lluvia de Fuego
		if (m_spellInfo->Id == 47820)
			if (!m_caster->IsWithinLOSInMap(target))
				return SPELL_FAILED_LINE_OF_SIGHT;*/
		...
		/*if(m_caster->IsFriendlyTo(target))
        {
			if (IsPositiveSpell(m_spellInfo->Id))
			{
				if (target != m_caster && target->ToPlayer()->duel != NULL && target->GetTypeId() == TYPEID_PLAYER)
					target = m_caster;
				else
        ...
        if(non_caster_target)
        {
            ...
            if(m_spellInfo->AttributesEx & SPELL_ATTR1_NOT_IN_COMBAT_TARGET && target->isInCombat())
             ...
             //    if(!(m_spellInfo->AttributesEx2 & 0x00000004) && VMAP::VMapFactory::checkSpellForLoS(m_spellInfo->Id) && !m_caster->IsWithinLOSInMap(target))
             //        return SPELL_FAILED_LINE_OF_SIGHT;
    ...
    for(int i = 0; i < MAX_SPELL_EFFECTS; i++)
    {
        ...
        switch(m_spellInfo->Effect[i])
        {
            case SPELL_EFFECT_DUMMY:
            {
                if(m_spellInfo->SpellIconID == 33 && m_spellInfo->SpellFamilyName == SPELLFAMILY_SHAMAN && m_spellInfo->SpellFamilyFlags[0] & SPELLFAMILYFLAG_SHAMAN_FIRE_NOVA)
                {
            ...
            case SPELL_EFFECT_TELEPORT_UNITS_FACE_CASTER:
            {
              //Do not allow to cast it before BG starts.
                if(m_caster->GetTypeId() == TYPEID_PLAYER)
                    if(Battleground const* pBG = m_caster->ToPlayer()->GetBattleground())
                        if(pBG->GetStatus() != STATUS_IN_PROGRESS)
                            return SPELL_FAILED_TRY_AGAIN;
                break;
            }
    ...
    for(int i = 0; i < MAX_SPELL_EFFECTS; i++)
    {
        switch(m_spellInfo->EffectApplyAuraName[i])
        {
            ...
            case SPELL_AURA_MOUNTED:
            {
                ...
                if(m_caster->GetTypeId() == TYPEID_PLAYER && !AllowMount && !m_IsTriggeredSpell && !m_spellInfo->AreaGroupId || m_caster->GetMapId() == 13)
...
/*uint32 Spell::GetCCDelay(SpellEntry const* _spell)
{
	...
	const uint32 delayForInstantSpells = 0;
	...
	switch(_spell->SpellFamilyName)
	{
        ...
        case SPELLFAMILY_WARRIOR:
            ...
            if (_spell->Id == 20253)
                return delayForInstantSpells;
            // Charge
            if (_spell->Id == 7922 || _spell->Id == 65929 || _spell->Id == 11578)
                return 0;
            break;
	...
	{
		if(_spell->EffectApplyAuraName[auraWithCCD[i]])
			return delayForInstantSpells;
	}
	return 0;
}*/
...
{
    // Heroism & Bloodlust over LOS
    if(m_spellInfo->Id == 32182 || m_spellInfo->Id == 2825)
        return true;
		
	// Furia de las Sombras sobre LOS, el Dst esta puesto, pero lo que es la spell atraviesa, como debe ser
	if(m_spellInfo->Id == 47847 || m_spellInfo->Id == 47846 || m_spellInfo->Id == 30414 || m_spellInfo->Id == 30413 || m_spellInfo->Id == 30283)
		return true;
	...
	//Fix Llamas infernales
	if(m_spellInfo->Id == 47823 || m_spellInfo->Id == 47822 || m_spellInfo->Id == 27214 || m_spellInfo->Id == 27213 || m_spellInfo->Id == 11682 || m_spellInfo->Id == 11684 || m_spellInfo->Id == 11683 || m_spellInfo->Id == 11681 || m_spellInfo->Id == 1949 || m_spellInfo->Id == 5857)
		if(!m_caster->IsWithinLOSInMap(pTarget) && !(m_spellInfo->AttributesEx2 & SPELL_ATTR2_UNK2))
			return false;
	
	//Custom fix filotormenta
	if((m_spellInfo->EffectImplicitTargetA[0] == TARGET_SRC_CASTER) && (m_spellInfo->EffectImplicitTargetB[0] == TARGET_UNIT_AREA_ENEMY_SRC))
		if(!m_caster->IsWithinLOSInMap(pTarget) && !(m_spellInfo->AttributesEx2 & SPELL_ATTR2_UNK2))
			return false;
			
	//Custom fix tifon
	if((m_spellInfo->EffectImplicitTargetA[0] == TARGET_UNIT_AREA_PATH))
		if(!m_caster->IsWithinLOSInMap(pTarget) && !(m_spellInfo->AttributesEx2 & SPELL_ATTR2_UNK2))
			return false;
...
void Spell::CalculateDamageDoneForAllTargets()
{
    ...
    for(std::list<TargetInfo>::iterator ihit= m_UniqueTargetInfo.begin(); ihit != m_UniqueTargetInfo.end(); ++ihit)
    {
        ...
        if(!mask || !m_caster)
            continue;
        Unit* pUnit = m_caster->GetGUID() == target.targetGUID ? m_caster : ObjectAccessor::GetUnit(*m_caster, target.targetGUID);
        if(!pUnit) // || !unit->isAlive()) do we need to check alive here?
            continue;
        ...
        {
            target.damage += CalculateDamageDone(pUnit, mask, multiplier);
            target.crit = m_caster->isSpellCrit(pUnit, m_spellInfo, m_spellSchoolMask, m_attackType);
        }
