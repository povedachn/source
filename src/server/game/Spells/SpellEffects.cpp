        ...
        case SPELLFAMILY_DEATHKNIGHT:
            ...
            switch(m_spellInfo->Id)
            {
                case 49560: // Death Grip
					...
					/*else if(unitTarget->HasAuraType(SPELL_AURA_REFLECT_SPELLS))
					{
						unitTarget->CastSpell(m_caster, 49560, true);
						if (unitTarget->HasAura(49560))
							unitTarget->RemoveAurasDueToSpell(49560); // Quitar el efecto de taunt sobre el warr...
						if (unitTarget->HasAuraType(SPELL_AURA_REFLECT_SPELLS))
							unitTarget->RemoveAurasByType(SPELL_AURA_REFLECT_SPELLS); // Quitar el efecto de reflejo de hechizos que como no se "refleja" no se quita el efecto
					}*/
...
void Spell::CalculateJumpSpeeds(uint8 i, float dist, float & speedXY, float & speedZ)
{
    if(m_spellInfo->EffectMiscValue[i])
        ...
        speedZ = float(m_spellInfo->EffectMiscValueB[i])/80;
    else
        speedZ = 10.0f;
...
void Spell::SpellDamageHeal(SpellEffIndex /*effIndex*/)
{
    if(unitTarget && unitTarget->isAlive() && damage >= 0)
    {
		...
		//Custom sistema de sanacion en rachas
		/*if(caster->GetTypeId() == TYPEID_PLAYER)
		{
			if(int32 gain = unitTarget->GetHealthGain(int32(addhealth)))
				sScriptMgr->OnPVPHeal(caster->ToPlayer(), unitTarget, uint32(gain));
		}*/
    }
}
...
void Spell::EffectPersistentAA(SpellEffIndex effIndex)
{
    if(!m_spellAura)
    {
        ...
        if(!caster->IsInWorld())
            return;
        DynamicObject* pDO = new DynamicObject(false);
        if(!pDO->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_DYNAMICOBJECT), caster, m_spellInfo->Id, *m_targets.GetDst(), radius, false, DYNAMIC_OBJECT_AREA_SPELL))
        {
            delete pDO;
            return;
        }
        ...
        if(Aura* pAura = Aura::TryCreate(m_spellInfo, MAX_EFFECT_MASK, pDO, caster, &m_spellValue->EffectBasePoints[0]))
        {
            m_spellAura = pAura;
...
void Spell::EffectOpenLock(SpellEffIndex effIndex)
{
    ...
    if(gameObjTarget)
    {
            ...
            (goInfo->type == GAMEOBJECT_TYPE_GOOBER && goInfo->goober.losOK))
        {
            ...
            {
                pBG->EventPlayerClickedOnFlag(player, gameObjTarget);
                return;
            }
        }
        else if(goInfo->type == GAMEOBJECT_TYPE_FLAGSTAND)
        {
            ...
            if(Battleground* pBG = player->GetBattleground())
            {
                if(pBG->GetTypeID(true) == BATTLEGROUND_EY)
                    pBG->EventPlayerClickedOnFlag(player, gameObjTarget);
                return;
            }
...
void Spell::SpellDamageWeaponDmg(SpellEffIndex effIndex)
{
    ...
    switch(m_spellInfo->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            ...
            // Rune Strike
            if(m_spellInfo->SpellFamilyFlags[EFFECT_1] & 0x20000000)
...
void Spell::EffectSummonObjectWild(SpellEffIndex effIndex)
{
    ...
    if(pGameObj->GetGoType() == GAMEOBJECT_TYPE_FLAGDROP && m_caster->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        switch(pGameObj->GetMapId())
        {
            case 489:                                       //WS
            {
                if(pBG && pBG->GetTypeID(true) == BATTLEGROUND_WS && pBG->GetStatus() == STATUS_IN_PROGRESS)
                {
                    uint32 team = ALLIANCE;

                    if(pPlayer->GetBGTeam() == team)
                        team = HORDE;

                    ((BattlegroundWS*)pBG)->SetDroppedFlagGUID(pGameObj->GetGUID(), team);
                }
                break;
            }
            case 566:                                       //EY
            {
                if(pBG && pBG->GetTypeID(true) == BATTLEGROUND_EY && pBG->GetStatus() == STATUS_IN_PROGRESS)
                {
                    ((BattlegroundEY*)pBG)->SetDroppedFlagGUID(pGameObj->GetGUID());
                }
                break;
            }
        }
            ...
            switch(m_spellInfo->Id)
            {
                ...
                case 64142:                                 // Upper Deck - Create Foam Sword
                {
                    ...
                    for(uint32 const *itr = &itemId[0]; *itr; ++itr)
                        if(pPlayer->HasItemCount(*itr, 1, true))
                            return;
...
void Spell::EffectDuel(SpellEffIndex effIndex)
{
    ...
    if(casterAreaEntry && !(casterAreaEntry->flags & AREA_FLAG_ALLOW_DUELS) || caster->GetAreaId() == 368)
    {
    ...
    if(targetAreaEntry && !(targetAreaEntry->flags & AREA_FLAG_ALLOW_DUELS) || target->GetAreaId() == 368)
    {
        SendCastResult(SPELL_FAILED_NO_DUELING);            // Dueling isn't allowed here
        return;
    }
...
void Spell::EffectTransmitted(SpellEffIndex effIndex)
{
    ...
    sLog->outDebug(DebugLogFilters(LOG_FILTER_SPELLS_AURAS | LOG_FILTER_MAPS), "AddObject at SpellEfects.cpp EffectTransmitted");
    //m_caster->AddGameObject(pGameObj);
...
void Spell::EffectBind(SpellEffIndex effIndex)
{
    ...
    sLog->outDebug(DebugLogFilters(LOG_FILTER_UNITS | LOG_FILTER_SPELLS_AURAS), "New homebind X      : %f", loc.m_positionX);
    sLog->outDebug(DebugLogFilters(LOG_FILTER_UNITS | LOG_FILTER_SPELLS_AURAS), "New homebind Y      : %f", loc.m_positionY);
    sLog->outDebug(DebugLogFilters(LOG_FILTER_UNITS | LOG_FILTER_SPELLS_AURAS), "New homebind Z      : %f", loc.m_positionZ);
    sLog->outDebug(DebugLogFilters(LOG_FILTER_UNITS | LOG_FILTER_SPELLS_AURAS), "New homebind MapId  : %u", loc.m_mapId);
    sLog->outDebug(DebugLogFilters(LOG_FILTER_UNITS | LOG_FILTER_SPELLS_AURAS), "New homebind AreaId : %u", area_id);
