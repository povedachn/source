...
void Aura::UpdateOwner(uint32 diff, WorldObject* pOwner)
{
    ASSERT(pOwner == m_owner);
        ...
        switch(GetSpellProto()->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_ROGUE:
                if(GetSpellProto()->SpellFamilyFlags[0] & 0x40 && GetSpellProto()->Category == 44) {   // Sprint (skip non player casted spells by category)
                    ...
                    {
                        sLog->outErrorDb("IF CRASH NOW, CRASH HAPPEN on SpellAuras.cpp line 1247");
    ...
    else
    {
        switch(GetSpellProto()->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_MAGE:
                ...
                // Ice barrier - solo salta Shattered Barrier con el daño, no dispeleandola
				if(removeMode == AURA_REMOVE_BY_ENEMY_SPELL && GetSpellProto()->SpellFamilyFlags[1] & 0x00000001 && aurApp->GetBase()->GetEffect(EFFECT_0)->GetAmount() <= 0)
                {
					// Shattered Barrier
					if (caster->GetDummyAuraEffect(SPELLFAMILY_MAGE, 2945, EFFECT_0))
                        caster->CastSpell(target, 55080, true, NULL, GetEffect(EFFECT_0));
                }
                break;
            ...
            case SPELLFAMILY_WARLOCK:
                ...
                // Improved Fear
                if(GetMaxDuration() && GetSpellProto()->SpellFamilyFlags[1] & 0x00000400)
                {
