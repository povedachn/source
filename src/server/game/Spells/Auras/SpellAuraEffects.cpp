...
int32 AuraEffect::CalculateAmount(Unit* caster)
{
    ...
    ////mixology s:53042 effect bonus
    if(caster && caster->GetTypeId() == TYPEID_PLAYER && caster->HasAura(53042))
        if(caster->ToPlayer()->IsSpellAffectedByMixology(GetSpellProto()))
            DoneActualBenefit = caster->ApplyEffectModifiers(GetSpellProto(), m_effIndex, amount*0.35f);
        ...
        case SPELL_AURA_PERIODIC_DUMMY:
            switch(GetSpellProto()->SpellFamilyName)
            {
                ...
                case SPELLFAMILY_DEATHKNIGHT:
                    ...
                    {
                        // Get 0 effect aura
                        if(AuraEffect* pAuraEffect = GetBase()->GetEffect(0))
                        {
                            int32 newAmount = pAuraEffect->GetAmount() + GetAmount();
                            if(newAmount > 0)
                                newAmount = 0;
                            pAuraEffect->ChangeAmount(newAmount);
                        }
                        return;
                    }
                    break;
...
void AuraEffect::HandleModPossess(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if(Unit* pCaster = GetCaster())
    {
        ...
        if(apply)
        {
           if(pTarget->SetCharmedBy(pCaster, CHARM_TYPE_POSSESS, aurApp))
               pCaster->ToPlayer()->SetMover(pTarget);
        } else {
            pTarget->RemoveCharmedBy(pCaster);
            pCaster->ToPlayer()->SetMover(pCaster);
            if(pTarget->GetTypeId() == TYPEID_PLAYER)
                pTarget->ToPlayer()->SetMover(pTarget);
        }
    }
}
   ...
   target->ApplySpellImmune(GetId(), IMMUNITY_EFFECT, GetMiscValue(), apply);
        ...
        && (GetSpellProto()->AuraInterruptFlags & AURA_INTERRUPT_FLAG_IMMUNE_OR_LOST_SELECTION))
    {
        if(target->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            {
                if(Battleground* pBG = target->ToPlayer()->GetBattleground())
                    pBG->EventPlayerDroppedFlag(target->ToPlayer());
            }
            else
                ...
                case SPELLFAMILY_GENERIC:
                    switch(GetId())
                    {
                        case 2584: // Waiting to Resurrect
                            ...
                            if(target->GetTypeId() == TYPEID_PLAYER)
                                if(Battleground* pBG = target->ToPlayer()->GetBattleground())
                                    pBG->RemovePlayerFromResurrectQueue(target->GetGUID());
                            break;
            ...
            switch(GetId())
            {
                case 6494: // Sentry Totem (Serverside)
                {
                    ...
                    if(Player* pPlayer = caster->ToPlayer())
                    {
                        ...
                        if(apply)
                        {
                            uint64 guid = pPlayer->m_SummonSlot[4];
                            if(guid)
                            {
                                Creature* pCreature_Totem = pPlayer->GetMap()->GetCreature(guid);
                                if(pCreature_Totem && pCreature_Totem->isTotem())
                                    pPlayer->CastSpell(pCreature_Totem, 6496, true);
                            }
                        }
                        else
                            pPlayer->RemoveAurasDueToSpell(6495);
                    }
                    break;
                }
                ...
                case 6495:
                {
                    if(caster && caster->GetTypeId() == TYPEID_PLAYER && apply)
...
void AuraEffect::HandlePeriodicDamageAurasTick(Unit* target, Unit* caster) const
{
    ...
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "PeriodicTick: %u (TypeId: %u) attacked %u (TypeId: %u) for %u dmg inflicted by %u abs is %u",
...
void AuraEffect::HandlePeriodicHealthLeechAuraTick(Unit* target, Unit* caster) const
{
    ...
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "PeriodicTick: %u (TypeId: %u) health leech of %u (TypeId: %u) for %u dmg inflicted by %u abs is %u",
        GUID_LOPART(GetCasterGUID()), GuidHigh2TypeId(GUID_HIPART(GetCasterGUID())), target->GetGUIDLow(), target->GetTypeId(), damage, GetId(), absorb);
...
void AuraEffect::HandlePeriodicHealAurasTick(Unit* target, Unit* caster) const
{
    ...
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "PeriodicTick: %u (TypeId: %u) heal of %u (TypeId: %u) for %u health inflicted by %u",
...
void AuraEffect::HandlePeriodicManaLeechAuraTick(Unit* target, Unit* caster) const
{
    ...
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "PeriodicTick: %u (TypeId: %u) power leech of %u (TypeId: %u) for %u dmg inflicted by %u",
...
void AuraEffect::HandleObsModPowerAuraTick(Unit* target, Unit* caster) const
{
    ...
    sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "PeriodicTick: %u (TypeId: %u) energize %u (TypeId: %u) for %u dmg inflicted by %u",
