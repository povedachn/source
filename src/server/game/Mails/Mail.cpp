...
void MailDraft::deleteIncludedItems(SQLTransaction& trans, bool inDB /*= false*/ )
{
    ...
    {
        if(Item* pItem = mailItemIter->second)
        {
            if(inDB)
            {
    ...
    {    
        ...
        {    
            ...
            {    
                ...
                if (last_id != 0)    
                {    
                    sLog->outError("EXTERNAL MAIL> Sending mail to character with guid %d", last_receiver_guid);    
                    ...
                    sLog->outError("EXTERNAL MAIL> Mail sent");    
                }    
                ...
                if(money)    
                {    
                    sLog->outError("EXTERNAL MAIL> Adding money");    
                    mail->AddMoney(money);    
                }    
            }    
            ...
            if (itemId)    
            {    
                sLog->outError("EXTERNAL MAIL> Adding %u of item with id %u", itemCount, itemId);    
