...
FleeingMovementGenerator<T>::_setTargetLocation(T &owner)
{
    ...
    i_nextCheckTime.Reset(traveltime + urand(0.1, 0.5));
}
...
bool FleeingMovementGenerator<T>::_getPoint(T &owner, float &x, float &y, float &z)
{
    ...
    for(uint8 i = 0; i < 18; ++i)
    {
        ...
        if(owner.IsWithinLOS(temp_x, temp_y, z))
        {
            ...
            {
                float new_z_left = _map->GetHeight(owner.GetPhaseMask(), temp_x + (float)(cos(angle+static_cast<float>(M_PI/2))), temp_y + (float)(sin(angle+static_cast<float>(M_PI/2))), z, true);
                float new_z_right = _map->GetHeight(owner.GetPhaseMask(), temp_x + (float)(cos(angle-static_cast<float>(M_PI/2))), temp_y + (float)(sin(angle-static_cast<float>(M_PI/2))), z, true);
