...
void WaypointMovementGenerator<Creature>::LoadPath(Creature &creature)
{
    ...
    if(!i_path)
    {
        ...
        sLog->outBasic("WaypointMovementGenerator::LoadPath: creature %s (Entry: %u GUID: %u) doesn't have waypoint path id: %u", creature.GetName(), creature.GetEntry(), creature.GetGUIDLow(), path_id);
        return;
    }
