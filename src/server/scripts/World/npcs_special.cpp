...
public:
    ...
    struct npc_air_force_botsAI : public ScriptedAI
    {
        npc_air_force_botsAI(Creature* pCreature): ScriptedAI(pCreature)
        {
            ...
            {
                CreatureTemplate const* pCreatureTemplate = sObjectMgr->GetCreatureTemplate(m_pSpawnAssoc->m_uiSpawnedCreatureEntry);
...
public:
    ...
    {
        npc_brewfest_revelerAI(Creature* pCreature) : ScriptedAI(pCreature) { }
        void ReceiveEmote(Player* player, uint32 emote)
        {
        ...
        void Reset()
        {
			...
			if(!me->getVictim() && me->isSummon())
                if(Unit* Owner = me->ToTempSummon()->GetSummoner())
                    /*if(Owner->ToPlayer()->GetPet()->getVictim())
						AttackStart(Owner->ToPlayer()->GetPet()->getVictim());
					else if(Owner->getAttackerForHelper())
            ...
            if(SpellTimer <= diff)
            {
                ...
                if(IsViper) //Viper
                {
                    if(urand(0, 2) == 0) //33% chance to cast
                    {
						...
						/*if (Owner->ToPlayer()->GetPet()->getVictim())
							DoCast(Owner->ToPlayer()->GetPet()->getVictim(), spell);
						else if (Owner->getAttackerForHelper())
							DoCast(Owner->getAttackerForHelper(), spell);
						else
                ...
                else //Venomous Snake
                {
					...
					/*{
						if (Owner->ToPlayer()->GetPet()->getVictim())
							DoCast(Owner->ToPlayer()->GetPet()->getVictim(), SPELL_DEADLY_POISON);
						else if (Owner->getAttackerForHelper())
							DoCast(Owner->getAttackerForHelper(), SPELL_DEADLY_POISON);
						else
...
public:
    ...
    {
        mob_mojoAI(Creature* pCreature) : ScriptedAI(pCreature) {Reset();}
        uint32 hearts;
        uint64 victimGUID;
        void Reset()
        {
        ...
        void ReceiveEmote(Player* player, uint32 emote)
        {
            ...
            if(emote == TEXT_EMOTE_KISS)
            {
                ...
                if(victimGUID)
                {
                    Player* pVictim = Unit::GetPlayer(*me, victimGUID);
                    if(pVictim)
                        pVictim->RemoveAura(43906);//remove polymorph frog thing
                }
    ...
    struct npc_ebon_gargoyleAI : CasterAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if (!target)
            {
                ...
                {
                    if (Unit* target = Unit::GetUnit((*me), (*iter)->GetGUID()))
                   {
                        if (target->HasAura(49206, owner->GetGUID()))
                        {
				...
				owner->AddAura(49206, owner->getVictim());
				me->Attack(owner->getVictim(), false);
            }
...
public:
    ...
    {
        npc_eversong_rangerAI(Creature* pCreature) : ScriptedAI(pCreature) { }
...
public:
    ...
    struct npc_spring_rabbitAI : public ScriptedAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            } else if(searchTimer <= diff) {
                if(Creature* pRabbit = me->FindNearestCreature(NPC_SPRING_RABBIT, 10.0f))
                {
                    if(pRabbit == me || pRabbit->HasAura(SPELL_SPRING_RABBIT_IN_LOVE))
                        return;

                    me->AddAura(SPELL_SPRING_RABBIT_IN_LOVE, me);
                    DoAction(1);
                    pRabbit->AddAura(SPELL_SPRING_RABBIT_IN_LOVE, pRabbit);
                    pRabbit->AI()->DoAction(1);
                    pRabbit->CastSpell(pRabbit, SPELL_SPRING_RABBIT_JUMP, true);
                    rabbitGUID = pRabbit->GetGUID();
                }
                searchTimer = urand(5000, 10000);
