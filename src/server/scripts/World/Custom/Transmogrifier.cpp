			...
			case EQUIPMENT_SLOT_END+8: // Save preset
				{
					...
					if (canSave)
						player->ADD_GOSSIP_ITEM_EXTENDED(1, "|TInterface/GuildBankFrame/UI-GuildBankFrame-NewTab:30:30:-15:0|tGuardar Set", 0, 0, "Pon un nombre a tu Set", cost*sT->GetSetCostModifier()+sT->GetSetCopperCost(), true);
...
public:
    ...
    {
        sT->LoadConfig(false);
        sLog->outBasic("Deleting non-existing transmogrification entries...");
