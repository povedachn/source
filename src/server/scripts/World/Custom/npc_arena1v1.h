...
static bool Arena1v1CheckTalents(Player* player)
{
	...
	if(count >= 36)
	{
		ChatHandler(player->GetSession()).SendSysMessage("No puedes unirte porque has usado más de 35 puntos de talentos en una rama prohibida (healer). Por favor, edita tus talentos.");
		return false;
	}
	else
		return true;
