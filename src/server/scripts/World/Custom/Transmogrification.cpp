...
const char * Transmogrification::GetSlotName(uint8 slot, WorldSession* session) const
{
    switch (slot)
    {
		case EQUIPMENT_SLOT_HEAD      : return  "Cabeza";// session->GetTrinityString(LANG_SLOT_NAME_HEAD);
		case EQUIPMENT_SLOT_SHOULDERS : return  "Hombreras";// session->GetTrinityString(LANG_SLOT_NAME_SHOULDERS);
		case EQUIPMENT_SLOT_BACK      : return  "Espalda";// session->GetTrinityString(LANG_SLOT_NAME_BACK);
		case EQUIPMENT_SLOT_CHEST     : return  "Pechera";// session->GetTrinityString(LANG_SLOT_NAME_CHEST);
		case EQUIPMENT_SLOT_BODY      : return  "Camisa";// session->GetTrinityString(LANG_SLOT_NAME_BODY);
		case EQUIPMENT_SLOT_TABARD    : return  "Tabardo";// session->GetTrinityString(LANG_SLOT_NAME_TABARD);
		case EQUIPMENT_SLOT_WRISTS    : return  "Brazales";// session->GetTrinityString(LANG_SLOT_NAME_WRISTS);
		case EQUIPMENT_SLOT_HANDS     : return  "Manos";// session->GetTrinityString(LANG_SLOT_NAME_HANDS);
		case EQUIPMENT_SLOT_WAIST     : return  "Cintura";// session->GetTrinityString(LANG_SLOT_NAME_WAIST);
		case EQUIPMENT_SLOT_LEGS      : return  "Pantalones";// session->GetTrinityString(LANG_SLOT_NAME_LEGS);
		case EQUIPMENT_SLOT_FEET      : return  "Pies";// session->GetTrinityString(LANG_SLOT_NAME_FEET);
		case EQUIPMENT_SLOT_MAINHAND  : return  "Mano principal";// session->GetTrinityString(LANG_SLOT_NAME_MAINHAND);
		case EQUIPMENT_SLOT_OFFHAND   : return  "Mano secundaria";// session->GetTrinityString(LANG_SLOT_NAME_OFFHAND);
		case EQUIPMENT_SLOT_RANGED    : return  "A Distancia";// session->GetTrinityString(LANG_SLOT_NAME_RANGED);
...
void Transmogrification::PresetTransmog(Player* player, Item* itemTransmogrified, uint32 fakeEntry, uint8 slot)
{
    ...
    itemTransmogrified->SetSoulboundTradeable(NULL, player, false);
}
...
TransmogTrinityStrings Transmogrification::Transmogrify(Player* player, uint64 itemGUID, uint8 slot, /*uint32 newEntry, */bool no_cost)
{
    ...
    */
    ...
    else
    {
        ...
        itemTransmogrifier->SetSoulboundTradeable(NULL, player, false);
...
bool Transmogrification::CanTransmogrifyItemWithItem(Player* player, ItemTemplate const* proto2, ItemTemplate const* proto1)
{
	...
	if((proto1->InventoryType == INVTYPE_CHEST && proto2->InventoryType == INVTYPE_ROBE) || (proto1->InventoryType == INVTYPE_ROBE && proto2->InventoryType == INVTYPE_CHEST))
		return true;
	else if (proto1->InventoryType != proto2->InventoryType && 
...
{
#ifdef PRESETS
    EnableSetInfo = sConfig->GetBoolDefault("Transmogrification.EnableSetInfo", true);
    SetNpcText = uint32(sConfig->GetIntDefault("Transmogrification.SetNpcText", 50001));

    EnableSets = sConfig->GetBoolDefault("Transmogrification.EnableSets", true);
    MaxSets = (uint8)sConfig->GetIntDefault("Transmogrification.MaxSets", 10);
    SetCostModifier = sConfig->GetFloatDefault("Transmogrification.SetCostModifier", 3.0f);
    SetCopperCost = sConfig->GetIntDefault("Transmogrification.SetCopperCost", 0);
    ...
    EnableTransmogInfo = sConfig->GetBoolDefault("Transmogrification.EnableTransmogInfo", true);
    TransmogNpcText = uint32(sConfig->GetIntDefault("Transmogrification.TransmogNpcText", 50000));

    std::istringstream issAllowed(sConfig->GetStringDefault("Transmogrification.Allowed", ""));
    std::istringstream issNotAllowed(sConfig->GetStringDefault("Transmogrification.NotAllowed", ""));
    ...
    ScaledCostModifier = sConfig->GetFloatDefault("Transmogrification.ScaledCostModifier", 1.0f);
    CopperCost = sConfig->GetIntDefault("Transmogrification.CopperCost", 0);
    ...
    AllowMixedArmorTypes = sConfig->GetBoolDefault("Transmogrification.AllowMixedArmorTypes", false);
    AllowMixedWeaponTypes = sConfig->GetBoolDefault("Transmogrification.AllowMixedWeaponTypes", false);
