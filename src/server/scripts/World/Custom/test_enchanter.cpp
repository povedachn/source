...
class npc_enchantment : public CreatureScript
{
    ...
    void Enchant(Player* player, uint8 equipSlot, uint32 enchantID)
    {
        ...
        for (i = 0; i < 70000; ++i)
        {
            ...
            if (!spellInfo)
                continue;
            if (spellInfo->Effect[0] != SPELL_EFFECT_ENCHANT_ITEM)
                continue;
            if (spellInfo->EffectMiscValue[0] != enchantID)
                continue;
    ...
    void ShowPage(Player* player, uint32 page, uint8 equipSlot)
    {
        ...
        for (i = 0; i < 70000; ++i)
        {
            ...
            if (spellID == 0)
                continue;
            if (SpellItemEnchantmentEntry const* enchant = sSpellItemEnchantmentStore.LookupEntry(spellInfo->EffectMiscValue[0]))
            {
        ...
        if (currentPos == startPos + 10 && i != 70000)
