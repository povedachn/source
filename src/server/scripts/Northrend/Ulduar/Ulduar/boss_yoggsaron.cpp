...
public:
    ...
    {
        boss_saraAI(Creature* pCreature) : ScriptedAI(pCreature), Summons(me)
        {
        ...
        void Reset()
        {
            ...
            if(pInstance)
                pInstance->SetBossState(TYPE_YOGGSARON,NOT_STARTED);
        ...
        {
            if(pInstance)
                pInstance->SetBossState(TYPE_YOGGSARON,DONE);
        ...
        void DoSpawnKeeperForSupport() // Despawn on Sara Reset
        {
            if(pInstance)
            {
                uint32 supportFlag = pInstance->GetData(DATA_KEEPER_SUPPORT_YOGG);
        ...
        void EnterCombat(Unit* /*who*/)
        {
            ...
            if(pInstance)
                pInstance->SetBossState(TYPE_YOGGSARON,IN_PROGRESS);
        ...
        void ModifySanity(Player* pTarget, int8 amount)
        {
            ...
            {
                int newamount = pAura->GetStackAmount();
                if(newamount > 0)
                    newamount += amount;
                    ...
                    for(Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
                    {
                        if(Player* pPlayer = itr->getSource())
                        {
                            if(pPlayer->isAlive())
                            {
                                if(!remove)
                                {
                                    me->AddAura(SPELL_SANITY, pPlayer);
                                    me->SetAuraStack(SPELL_SANITY, pPlayer, 100);
                                } else pPlayer->RemoveAurasDueToSpell(SPELL_SANITY);
                            }
                        }
                    }
                }
        ...
        void DoMadness()
        {
            if(pInstance)
            {
                pInstance->HandleGameObject(pInstance->GetData64(TYPE_BRAIN_DOOR_1),false);
                pInstance->HandleGameObject(pInstance->GetData64(TYPE_BRAIN_DOOR_2),false);
                pInstance->HandleGameObject(pInstance->GetData64(TYPE_BRAIN_DOOR_3),false);
            }
                        ...
                        if(!me->HasAura(SPELL_SHATTERED_ILLUSIONS))
                        {
                            ...
                            if(uiMindSpell_Timer <= diff)
                            {
                                if(!me->IsNonMeleeSpellCasted(false))
                                {
                                    switch(urand(0,2))
                                    {
                                        case 0: DoCast(SPELL_MALADY_OF_MIND);   break;
                                        case 1: DoCast(SPELL_BRAIN_LINK);       break;
                                        case 2: DoCast(SPELL_DEATH_RAY_SUMMON); break;
                                    }
                            ...
                            if(uiTentacle_Timer <= diff)
                            {
                                ...
                                } else if(Creature* yogg = me->GetCreature(*me,guidYogg))
                        ...
                        if(!me->HasAura(SPELL_SHATTERED_ILLUSIONS))
                        {
                            if(currentBrainEventPhase != PORTAL_PHASE_BRAIN_NONE)
                            {
                                if(AllSpawnsDeadOrDespawned(guidEventTentacles))
                                {
                                    switch(currentBrainEventPhase)
                                    {
                                    case PORTAL_PHASE_DRAGON_SOUL: if(pInstance) pInstance->HandleGameObject(pInstance->GetData64(TYPE_BRAIN_DOOR_1),true); break;
                                    case PORTAL_PHASE_LICH_KING: if(pInstance) pInstance->HandleGameObject(pInstance->GetData64(TYPE_BRAIN_DOOR_2),true); break;
                                    case PORTAL_PHASE_KING_LLIANE: if(pInstance) pInstance->HandleGameObject(pInstance->GetData64(TYPE_BRAIN_DOOR_3),true); break;
                                    }
                                    currentBrainEventPhase = PORTAL_PHASE_BRAIN_NONE;
                                    if(Creature* yogg = me->GetCreature(*me,guidYogg))
        ...
        void SpellHitTarget(Unit* target, const SpellEntry* spell)
        {
            if(target && target->ToPlayer())
            {
                ...
                case SPELL_DARK_VOLLEY:
                    if(Creature* cSara = me->GetCreature(*me,pInstance->GetData64(TYPE_SARA)))
...
public:
    ...
    bool OnGossipHello(Player* pPlayer, Creature* pCreature)
    {
        ...
        {
            BrainEventPhase pTemp = CAST_AI(npc_descend_into_madnessAI, pCreature->AI())->bPhase;
...
public:
    ...
    struct boss_yogg_saronAI : public Scripted_NoMovementAI
    {
            ...
            if(pTarget->ToPlayer())
            {
                switch(pSpell->Id)
                {
                    case SPELL_LUNATIC_GAZE_EFFECT:
                        if(Creature* cSara = me->GetCreature(*me, pInstance->GetData64(TYPE_SARA)))
                            CAST_AI(boss_sara::boss_saraAI, cSara->AI())->ModifySanity(pTarget->ToPlayer(), -4);
                        break;
                }
            }
        }
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if(pInstance->GetBossState(TYPE_YOGGSARON) != IN_PROGRESS)
                return;
...
public:
    ...
    {
        npc_influence_tentacleAI(Creature* pCreature) : Scripted_NoMovementAI(pCreature)
        {
...
public:
    ...
    {
        npc_support_keeperAI(Creature* pCreature) : Scripted_NoMovementAI(pCreature) , Summons(me)
        {
            pInstance = pCreature->GetInstanceScript();
        }
        ...
        void AttackStart(Unit* pAttacker) { }

        void Reset()
        {
            switch(me->GetEntry())
            {
                case ENTRY_KEEPER_HODIR:    DoCast(SPELL_FORTITUDE_OF_FROST);   break;
                case ENTRY_KEEPER_FREYA:    DoCast(SPELL_RESILIENCE_OF_NATURE); break;
                case ENTRY_KEEPER_THORIM:   DoCast(SPELL_FURY_OF_THE_STORM);    break;
                case ENTRY_KEEPER_MIMIRON:  DoCast(SPELL_SPEED_OF_INVENTION);   break;
            }
...
public:
    ...
    struct npc_sanity_wellAI : public Scripted_NoMovementAI
    {
        ...
        void MoveInLineOfSight(Unit* pMover)
        {
            if(!pMover)
                return;
            if(pMover->ToPlayer())
            {
                if(me->IsWithinDist2d(pMover, 6))
                {
                    if(!pMover->HasAura(SPELL_SANITY_WELL_DEBUFF))
                        me->AddAura(SPELL_SANITY_WELL_DEBUFF, pMover);
                } else if(pMover->HasAura(SPELL_SANITY_WELL_DEBUFF))
                    pMover->RemoveAurasDueToSpell(SPELL_SANITY_WELL_DEBUFF);
            }
        }
            ...
            if(uiSanityTick_Timer <= diff)
            {
                ...
                for(std::list<Player*>::const_iterator itr = plrList.begin(); itr != plrList.end(); ++itr)
                {
                    if((*itr))
                    {
                        ...
                        {
                            if(Creature* sara = Creature::GetCreature((*me), pInstance->GetData64(TYPE_SARA)))
...
public:
    ...
    {
        npc_laughting_skullAI(Creature* pCreature) : Scripted_NoMovementAI(pCreature)
        {
            pInstance = pCreature->GetInstanceScript();
        }
            ...
            if(pTarget->ToPlayer())
            {
                switch(pSpell->Id)
                {
                    case SPELL_LS_LUNATIC_GAZE_EFFECT:
                        if(Creature* cSara = me->GetCreature(*me,pInstance->GetData64(TYPE_SARA)))
                            CAST_AI(boss_sara::boss_saraAI, cSara->AI())->ModifySanity(pTarget->ToPlayer(), -2);
                        break;
                }
            }
        }
    ...
    class spell_insane_death_effekt_AuraScript : public AuraScript
    {
        ...
        void HandleEffectRemove(AuraEffect const * /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            if(Unit* pTarget = GetTarget())
                if(pTarget->ToPlayer() && pTarget->isAlive())
                    pTarget->DealDamage(pTarget, pTarget->GetHealth());
        }
            ...
            if(Unit* target = GetHitUnit())
            {
                switch(target->GetMap()->GetDifficulty())
                {
                    case RAID_DIFFICULTY_10MAN_NORMAL:
                        GetCaster()->CastSpell(target, SPELL_EMPOWERING_SHADOWS_HEAL_10, true);
                        break;
                    case RAID_DIFFICULTY_25MAN_NORMAL:
                        GetCaster()->CastSpell(target, SPELL_EMPOWERING_SHADOWS_HEAL_25, true);
                        break;
                }
            }
        }
